...
const BODS_CACHE_SCHEMA_VERSION = "v49-origin-led-bods-dataset-search";
...
const BODS_MAX_TOTAL_DOWNLOAD_URLS = 120;
...
async function buildBodsTimetableDatasetSearchTerms(originCoordinates) {
  ...
  // BODS dataset discovery must be driven by the selected origin, not by a stale
  // project title/planning-authority field. Query nearby physical stops first so
  // the dataset search has local stop names, ATCO/NaPTAN references, route refs
  // and operators before it falls back to wider authority terms.
  const nearbyStopTerms = await fetchNearbyBodsOriginStopSearchTerms(originCoordinates);
  nearbyStopTerms.forEach((term) => addTerm(term.value, term.source, { priority: term.priority ?? 100, keepShortCodes: true }));
...
async function fetchNearbyBodsOriginStopSearchTerms(originCoordinates) {
  ...
  try {
    ...
    // Do not fail BODS because the OSM diagnostic lookup failed. Authority and
    // fallback queries remain available.
  }
  return terms;
}
...
function normaliseNearbyBodsOriginStop(element, originCoordinates) {
  ...
  return {
    ...
    atcoCode: tags["naptan:AtcoCode"] || tags["ref:GB:naptan"] || tags.atco_code || tags.AtcoCode || "",
    naptanCode: tags["naptan:NaptanCode"] || tags.naptan_code || tags.NaptanCode || "",
    ref: tags.ref || "",
    localRef: tags.local_ref || tags.local_ref_name || "",
    routeRef: tags.route_ref || tags["bus:route_ref"] || "",
...
function cleanBodsSearchTerm(value, options = {}) {
  ...
  if (options.keepShortCodes && /^[A-Z0-9]{3,16}$/i.test(raw.replace(/\s+/g, ""))) {
    return raw.replace(/\s+/g, "");
  }
  const cleaned = raw
    .replace(/(city|county|borough|district|metropolitan|unitary|council|local authority)/gi, " ")
    .replace(/(at|land|site|near|north|south|east|west)/gi, " ")
    .replace(/[^a-z0-9&'\-\s]/gi, " ")
    .replace(/\s+/g, " ")
    .trim();
