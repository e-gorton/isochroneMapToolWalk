...
function buildBodsTimetableIsochronesFromSearch(searchResult, configuredBands) {
  ...
  configuredBands.forEach((band) => {
    ...
    if (rings.length === 0) {
      const scheduledStopMetrics = buildBodsScheduledStopArrivalMetrics(searchResult.reachableSegments, bandTime);
      rings = buildBodsScheduledStopArrivalRings(scheduledStopMetrics, searchResult.originCoordinates);
      geometryMode = rings.length > 0 ? "scheduled_stop_arrival_buffers" : "none";
...
function buildBodsScheduledStopArrivalMetrics(reachableSegments = [], bandTime) {
  const seen = new Set();
  const metrics = [];
  (reachableSegments || [])
...
function buildBodsScheduledStopArrivalRings(metrics, originCoordinates = null) {
  ...
  clusters.forEach((cluster) => {
    ...
    const nearestOriginDistance = isValidCoordinateObject(originCoordinates)
      ? getNearestCoordinateDistanceMetres(cluster, originCoordinates)
      : 0;
    if (Number.isFinite(nearestOriginDistance) && nearestOriginDistance <= 120) {
      return;
    }
    const ring = cluster.length === 1
      ? buildCoordinateBufferRing(cluster[0], BODS_TIMETABLE_ZONE_BUFFER_METRES, 20)
      : buildBodsWholeZoneRingForCluster(cluster);
    if (ring && ring.length >= 4 && rings.length < BUS_ROUTE_MAX_BUFFERS_PER_BAND) {
      rings.push(ring);
    }
  });
  return capPolygonRings(rings, BUS_ROUTE_MAX_BUFFERS_PER_BAND);
}
