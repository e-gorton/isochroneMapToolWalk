...
const BODS_CACHE_SCHEMA_VERSION = "v75-bods-query-spec-function-restored";
...
const BODS_STRICT_LOCAL_DISTRIBUTION_RADII_METRES = [1000, 5000, 10000, 15000, 25000];
...
async function buildBodsTimetableDatasetSearchTerms(originCoordinates) {
  ...
  nearbyStops.forEach((stop, index) => {
    const basePriority = Math.max(50, 140 - index * 3);
    // Only true NaPTAN/ATCO codes can drive high-confidence BODS dataset selection.
    addTerm(stop.atcoCode, "nearby-origin-stop-atco", { priority: basePriority + 12, keepShortCodes: true });
    addTerm(stop.naptanCode, "nearby-origin-stop-naptan", { priority: basePriority + 10, keepShortCodes: true });
    // Stop names/localities are allowed as low-priority fallback searches only; generic OSM refs,
    // local refs, route refs and route destinations are deliberately excluded because they caused
    // global false positives such as London/Potters Bar datasets for the Leeds test site.
    addTerm(stop.name, "nearby-origin-stop-name", { priority: basePriority - 34 });
    addTerm(stop.localityName, "nearby-origin-stop-locality", { priority: basePriority - 38 });
  });
  ...
  try {
    ...
    addTerm(authorityName, "mapit-authority", { priority: 90 });
  } catch (error) {
    // Planning authority lookup is helpful for BODS filtering but not required.
  }

  addTerm(elements.planningAuthority?.value, "planning-authority-field", { priority: 42 });
  addTerm(extractLikelyOriginPlaceName(elements.projectName?.value), "project-name-origin", { priority: 24 });
...
function getNearbyOriginStopCodeCandidates(stop) {
  // Strict by design: only actual NaPTAN/ATCO identifiers are safe for BODS matching.
  // OSM ref/local_ref values are often stand letters, route numbers or generic refs and can
  // inject the origin into an unrelated timetable dataset.
  return [
    stop?.atcoCode,
    stop?.naptanCode,
  ]
...
function injectNearbyOriginStopMatchesIntoTxcModel(model, xmlText, nearbyStops = []) {
  ...
  const addOrUpdateMatchedStop = (txcStopId, txcStopName, nearbyStop, source) => {
    ...
    if (!normalisedId || !getNearbyOriginStopCodeCandidates(nearbyStop).includes(normalisedId)) {
      return;
    }
  ...
  matches.forEach(({ candidate, stop }) => {
    ...
    addOrUpdateMatchedStop(
      missingStop?.id || existingStop?.id || stop.atcoCode || stop.naptanCode || candidate,
      missingStop?.name || existingStop?.name || stop.name || candidate,
      stop,
      "nearby_origin_stop_exact_atco_naptan_match"
    );
  });

  missingStops.forEach((missingStop) => {
    const missingId = normaliseBodsStopIdentifier(missingStop.id);
    const codeMatch = nearbyStops.find((nearbyStop) => getNearbyOriginStopCodeCandidates(nearbyStop).includes(missingId));
    if (codeMatch) {
      addOrUpdateMatchedStop(missingStop.id, missingStop.name, codeMatch, "nearby_origin_stop_missing_exact_atco_naptan_match");
    }
  });

  stops.forEach((existingStop) => {
    const existingId = normaliseBodsStopIdentifier(existingStop.id);
    const codeMatch = nearbyStops.find((nearbyStop) => getNearbyOriginStopCodeCandidates(nearbyStop).includes(existingId));
    if (codeMatch) {
      addOrUpdateMatchedStop(existingStop.id, existingStop.name, codeMatch, "nearby_origin_stop_existing_exact_atco_naptan_match");
    }
  });

  model.stops = injectedStops.length > 0 ? [...stops, ...injectedStops] : stops;
...
function buildBodsDatasetQuerySpecs(searchTerms) {
  ...
  const addSearchSpec = (value, source, includeStatus = true) => {
    ...
    // Numeric-only free-text searches are a major source of wrong-area BODS hits (e.g. route ref 905).
    // Numeric location confidence should come through adminArea, not global free-text search.
    if (/^\d+$/.test(cleaned)) {
      return;
    }
  ...
  const addDerivedAdminAreaCode = (value, source) => {
    ...
    // Only actual nearby ATCO/NaPTAN stop-code terms are allowed to derive BODS adminArea.
    // Do not use OSM ref/local_ref/route-ref/destination values here.
    const numericPrefix = compact.match(/^(\d{3})/i)?.[1] || "";
    if (!numericPrefix) {
      return;
    }
    if (!derivedAdminAreaByCode.has(numericPrefix)) {
  ...
  // BODS `adminArea` is a numeric NaPTAN administrative area code. It must be tried first.
  // For Leeds / West Yorkshire this should select adminArea=450 before any broad free-text searches.
  ...
  (searchTerms || []).forEach((term) => {
    if (!shouldUseBodsDatasetFreeTextSearchTerm(term)) {
      return;
    }
...
function isBodsTimetableModelLocalToOrigin(timetable, originCoordinates, maximumWalkToBusStopMetres) {
  ...
  return hasRealStopInsideStrictLocalTolerance && isBodsCredibleLocalStopDistribution(distribution);
}
