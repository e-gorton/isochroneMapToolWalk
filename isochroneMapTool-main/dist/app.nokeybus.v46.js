...
const BUS_ROUTE_MAX_POINTS_PER_RELATION = 520;
const BUS_ROUTE_MAX_STITCH_GAP_METRES = 900;
const BUS_ROUTE_CORRIDOR_SAMPLE_SPACING_METRES = 260;
const BUS_ROUTE_CORRIDOR_SEGMENT_BUFFER_METRES = 300;
...
function renderMethodNote() {
  ...
  const note = {
    ...
    routing_origin_coordinates: scenario ? getRoutingOriginCoordinatesForMode(state.selectedMode, scenario) : "Awaiting valid coordinates",
    routing_origin_note:
      state.selectedMode === "bus"
        ? "Bus mode uses the visible development-site coordinates as the BODS/OSM bus origin. Access coordinates are ignored in bus mode to avoid hidden-origin misalignment."
        : "Walking and cycling use the access coordinates as the routing origin where provided; otherwise they use the site coordinates.",
...
function getRoutingOriginCoordinatesForMode(mode, scenario) {
  ...
  return scenario.accessCoordinates ?? scenario.siteCoordinates;
}
...
async function refreshLiveContext(statusText) {
  ...
  setStatus(
    ...
    state.selectedMode === "bus"
      ? "Fetching bus timetable/route data from the visible site marker origin."
      : state.selectedMode === "walking"
  ...
  const originCoordinates = getRoutingOriginCoordinatesForMode(state.selectedMode, state.generatedScenario);
...
function buildBodsTimetableCacheKey(originCoordinates, configuredBands, maximumWalkToBusStopMetres, departureDate, departureTime) {
  return JSON.stringify({
    schemaVersion: BODS_CACHE_SCHEMA_VERSION,
...
function buildBodsStopCoordinateCacheKey(stopId) {
  return `${BODS_CACHE_SCHEMA_VERSION}:${String(stopId || "").trim()}`;
}
...
async function enrichBodsMissingStopCoordinates(missingStopsById, existingStopsById, options = {}) {
  ...
  const addResolvedStop = (id, coordinate, source) => {
    ...
    if (isPlausibleUkWgs84Coordinate(stop.latitude, stop.longitude)) {
      BODS_STOP_COORDINATE_CACHE.set(buildBodsStopCoordinateCacheKey(id), stop);
  ...
  missingStops.forEach((stop) => {
    const cached = BODS_STOP_COORDINATE_CACHE.get(buildBodsStopCoordinateCacheKey(stop.id));
    if (cached) {
...
function isBodsInitialStopWithinSelectedWalkLimit(straightLineDistanceMetres, maximumWalkToBusStopMetres) {
  return Number.isFinite(straightLineDistanceMetres)
    && straightLineDistanceMetres >= 0
    && Number.isFinite(maximumWalkToBusStopMetres)
    && straightLineDistanceMetres <= maximumWalkToBusStopMetres;
}
...
function buildBodsTimetableSourceNote(metadata = {}) {
  ...
  const originText = metadata.busOriginMode === "site_coordinates_visible_marker" ? " Origin: visible development-site coordinates; access coordinates are ignored in bus mode." : "";
...
function buildSmoothedBusCatchmentRings(metrics) {
  const orderedMetrics = (metrics ?? [])
    .filter((metric) =>
      metric &&
      metric.coordinate &&
      Number.isFinite(metric.coordinate.latitude) &&
      Number.isFinite(metric.coordinate.longitude) &&
      Number.isFinite(metric.minimumTotalBusAccessTimeMinutes)
    )
    .sort((a, b) =>
      String(a.routeId ?? "").localeCompare(String(b.routeId ?? "")) ||
      (Number(a.componentId) || 0) - (Number(b.componentId) || 0) ||
  ...
  orderedMetrics.forEach((metric) => {
    const key = `${metric.routeId ?? "unknown"}|${metric.componentId ?? 0}`;
  ...
  metricGroups.forEach((groupMetrics) => {
    const sortedGroup = groupMetrics.sort((a, b) => (Number(a.index) || 0) - (Number(b.index) || 0));
    ...
    for (let index = 1; index < sortedGroup.length; index += 1) {
      ...
      if (!areRouteMetricsContiguous(previous, current)) {
        continue;
      }
      const segmentRing = buildBufferedRouteSegmentRing(
        previous.coordinate,
        current.coordinate,
        BUS_ROUTE_CORRIDOR_SEGMENT_BUFFER_METRES || BUS_CARTO_EFFECTIVE_BUFFER_METRES
      );
      if (segmentRing && segmentRing.length >= 4) {
        rings.push(segmentRing);
        addedSegment = true;
    ...
    if (!addedSegment) {
      sortedGroup.forEach((metric) => {
        rings.push(buildCoordinateBufferRing(
          metric.coordinate,
          BUS_ROUTE_CORRIDOR_SEGMENT_BUFFER_METRES || BUS_CARTO_EFFECTIVE_BUFFER_METRES,
          16
        ));
...
function areRouteMetricsContiguous(previousMetric, currentMetric) {
  ...
  if (previousMetric.routeId !== undefined && currentMetric.routeId !== undefined && previousMetric.routeId !== currentMetric.routeId) {
    return false;
  }
  if (previousMetric.componentId !== undefined && currentMetric.componentId !== undefined && previousMetric.componentId !== currentMetric.componentId) {
    return false;
  }
  const routeDistanceGap = Math.abs(Number(currentMetric.componentDistanceMetres ?? currentMetric.cumulativeDistance) - Number(previousMetric.componentDistanceMetres ?? previousMetric.cumulativeDistance));
  const straightLineGap = getDistanceMetres(
    previousMetric.coordinate.latitude,
    previousMetric.coordinate.longitude,
    currentMetric.coordinate.latitude,
    currentMetric.coordinate.longitude
  );
  return Number.isFinite(routeDistanceGap) &&
    Number.isFinite(straightLineGap) &&
    routeDistanceGap <= BUS_ROUTE_CORRIDOR_SAMPLE_SPACING_METRES * 1.75 &&
    straightLineGap <= BUS_ROUTE_MAX_STITCH_GAP_METRES;
}
...
function buildRouteDistanceMetrics(route, busSpeedSettings = getSelectedBusSpeedSettings()) {
  ...
  const componentDistances = [0];
  const componentTravelTimes = [0];
  const componentIds = [0];
  ...
  for (let index = 1; index < coordinates.length; index += 1) {
    ...
    if (!Number.isFinite(segmentDistance) || segmentDistance > BUS_ROUTE_MAX_STITCH_GAP_METRES) {
      currentComponentId += 1;
      cumulativeDistances.push(cumulativeDistances[index - 1]);
      cumulativeTravelTimes.push(cumulativeTravelTimes[index - 1]);
      componentDistances.push(0);
      componentTravelTimes.push(0);
      componentIds.push(currentComponentId);
      segmentDiagnostics.push({
        highwayClass: "route_geometry_gap",
        appliedSpeedKph: 0,
        baseSpeedKph: 0,
        maxspeedKph: null,
        maxspeedAppliedAsCap: false,
        usedFallbackHighwayClass: true,
        distanceMetres: 0,
        travelTimeMinutes: 0,
        geometryGapMetres: segmentDistance,
      });
      continue;
    }
    ...
    componentDistances.push(componentDistances[index - 1] + segmentDistance);
    componentTravelTimes.push(componentTravelTimes[index - 1] + segmentTravelTimeMinutes);
    componentIds.push(currentComponentId);
    segmentDiagnostics.push({
  ...
  const accessStates = (route.accessAnchors ?? [])
    .map((anchor) => {
      ...
      return {
        anchor,
        index: nearest.index,
        componentId: componentIds[nearest.index] ?? 0,
        componentDistanceMetres: componentDistances[nearest.index] ?? 0,
        componentTravelTimeMinutes: componentTravelTimes[nearest.index] ?? 0,
        accessWalkTimeMinutes: anchor.accessWalkTimeMinutes,
      };
    })
    .filter((accessState) =>
      accessState && Number.isFinite(accessState.componentDistanceMetres) && Number.isFinite(accessState.componentTravelTimeMinutes) && Number.isFinite(accessState.accessWalkTimeMinutes)
    );

  const metrics = coordinates.map((coordinate, index) => {
    const componentId = componentIds[index] ?? 0;
    let bestAccess = null;
    accessStates
      .filter((accessState) => accessState.componentId === componentId)
      .forEach((accessState) => {
        const routeDistanceFromAccess = Math.abs((componentDistances[index] ?? 0) - accessState.componentDistanceMetres);
        const inVehicleBusMinutes = Math.abs((componentTravelTimes[index] ?? 0) - accessState.componentTravelTimeMinutes);
    ...
    return {
      ...
      componentDistanceMetres: componentDistances[index],
      componentTravelTimeMinutes: componentTravelTimes[index],
  ...
  metrics.speedDiagnostics = summariseBusSegmentSpeedDiagnostics(segmentDiagnostics.filter((segment) => !segment.geometryGapMetres), speedSettings);
  metrics.geometryGapCount = segmentDiagnostics.filter((segment) => segment.geometryGapMetres).length;
  return metrics;
}

function sampleRouteMetricsBySpacing(routeMetrics, spacingMetres) {
  const sampled = [];
  const lastKeptDistanceByComponent = new Map();

  routeMetrics.forEach((metric, index) => {
    const componentKey = metric.componentId ?? 0;
    const componentDistance = Number(metric.componentDistanceMetres ?? metric.cumulativeDistance);
    const lastKeptDistance = lastKeptDistanceByComponent.has(componentKey)
      ? lastKeptDistanceByComponent.get(componentKey)
      : -Infinity;
    if (
      index === 0 ||
      index === routeMetrics.length - 1 ||
      componentDistance - lastKeptDistance >= spacingMetres ||
      metric.distanceFromAccess === 0
    ) {
      sampled.push(metric);
      lastKeptDistanceByComponent.set(componentKey, componentDistance);
    }
  });
