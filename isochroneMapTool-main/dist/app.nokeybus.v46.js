...
const BODS_CACHE_SCHEMA_VERSION = "v56-bods-reachable-link-geometry";
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  if (!timetable) {
    ...
    throw createBodsDiagnosticError(
      ...
      `BODS returned timetable datasets, but none appear local and usable for the selected origin.${nearestText} The app found nearby physical stops, but none of the downloaded TransXChange files contained matching usable ATCO/NaPTAN stop references close enough to the origin. Check whether the BODS dataset includes those ATCO codes and whether the operator has published the service.`,
      {
        datasetQueryParametersUsed: datasetResult.queryParametersUsed,
        datasetRecordCount,
        queryTerms: datasetResult.searchTerms?.map((term) => term.value),
