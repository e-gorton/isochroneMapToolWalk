...
const BODS_CACHE_SCHEMA_VERSION = "v77-bods-naptan-access-service-only-cache";
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  isochrones.metadata = {
    ...
    caveat: "Scheduled timetable output from BODS where TransXChange files could be parsed in-browser. Initial walking access is limited to walk-accessible physical NaPTAN/OSM stops; those physical stops are then cross-referenced into the parsed BODS timetable graph using ATCO/NaPTAN references, stop-name matching or graph-adjacent timetable nodes. The linked BODS graph node does not need to be within the walking threshold. Initial and transfer walking times use straight-line distance multiplied by 1.3 at 4.8 kph / 80 metres per minute. Includes scheduled wait time and bus in-vehicle time from parsed stop-to-stop timetable links. Does not include live disruption, reliability, crowding, cancellations, fare integration or operator-specific journey-planner rules.",
  };
