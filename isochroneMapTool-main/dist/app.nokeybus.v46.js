...
const BODS_CACHE_SCHEMA_VERSION = "v81-bods-partial-segment-clipping-cache";
...
function runBodsEarliestArrivalSearch(timetable, originCoordinates, configuredBands, maximumBandMinutes, maximumWalkToBusStopMetres, departureMinutes, boardingStopContext = null) {
  ...
  consideredConnections.forEach((connection) => {
    ...
    for (let transfersUsed = 0; transfersUsed <= BODS_MAX_TRANSFERS; transfersUsed += 1) {
      ...
      const departsBeforeBandEnds = connection.departureMinutes - departureMinutes <= maximumBandMinutes;
      if (canBoardConnection && departsBeforeBandEnds) {
        ...
        if (connection.arrivalMinutes - departureMinutes <= maximumBandMinutes && connection.arrivalMinutes < label[toIndex][transfersUsed]) {
