...
const BODS_CACHE_SCHEMA_VERSION = "v78-bods-route-level-local-gate";
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  if (!timetable) {
    ...
    throw createBodsDiagnosticError(
      ...
      `BODS returned timetable datasets, but none passed the local-origin validation.${nearestText} A dataset now needs either a credible distribution of real parsed local stops or at least one real/exact ATCO-NaPTAN local proof stop that is actually used by scheduled connections. Check selected and rejected BODS dataset diagnostics.`,
      {
        datasetQueryParametersUsed: datasetResult.queryParametersUsed,
        datasetRecordCount,
        queryTerms: datasetResult.searchTerms?.map((term) => term.value),
