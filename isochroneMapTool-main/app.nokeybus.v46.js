...
const BODS_CACHE_SCHEMA_VERSION = "v50-origin-stop-code-crossmatch";
...
function findNearbyOriginStopMatchesInXml(xmlText, nearbyStops = []) {
  const lowerXml = String(xmlText || "").toLowerCase();
  const matches = [];
  const seen = new Set();
  nearbyStops.forEach((stop) => {
    getNearbyOriginStopCodeCandidates(stop).forEach((candidate) => {
      ...
      if (lowerXml.includes(candidate.toLowerCase())) {
        seen.add(candidate);
        matches.push({ candidate, stop });
      }
    });
  });
  return matches;
...
function injectNearbyOriginStopMatchesIntoTxcModel(model, xmlText, nearbyStops = []) {
  ...
  if (matches.length === 0) {
    model.originMatchedStopCount = 0;
    return model;
  }
  const stopsByNormalisedId = new Map((model.stops || []).map((stop) => [normaliseBodsStopIdentifier(stop.id), stop]));
  const missingByNormalisedId = new Map((model.missingCoordinateStops || []).map((stop) => [normaliseBodsStopIdentifier(stop.id), stop]));
  const injectedStops = [];
  matches.forEach(({ candidate, stop }) => {
    if (stopsByNormalisedId.has(candidate)) {
      return;
    }
    const missingStop = missingByNormalisedId.get(candidate);
    injectedStops.push({
      id: missingStop?.id || stop.atcoCode || stop.naptanCode || stop.ref || candidate,
      name: missingStop?.name || stop.name || stop.atcoCode || stop.naptanCode || candidate,
      latitude: stop.latitude,
      longitude: stop.longitude,
      coordinateSource: "nearby_origin_stop_code_match",
    });
  });
  if (injectedStops.length > 0) {
    model.stops = [...(model.stops || []), ...injectedStops];
  }
  model.originMatchedStopCount = matches.length;
  return model;
}
