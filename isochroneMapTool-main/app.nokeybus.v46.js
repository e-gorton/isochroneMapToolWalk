...
const BODS_CACHE_SCHEMA_VERSION = "v72-bods-local-adminarea-gate";
...
const BODS_ORIGIN_ROUTE_HINT_RADIUS_METRES = 2600;
...
async function buildBodsTimetableDatasetSearchTerms(originCoordinates) {
  ...
  nearbyRouteHints.forEach((route, index) => {
    const basePriority = Math.max(45, 118 - index * 2);
    addTerm(route.ref, "nearby-origin-route-ref", { priority: basePriority + 12, keepShortCodes: true });
    addTerm(route.name, "nearby-origin-route-name", { priority: basePriority + 10 });
    addTerm(route.operator, "nearby-origin-route-operator", { priority: basePriority + 8 });
    addTerm(route.network, "nearby-origin-route-network", { priority: basePriority + 6 });
    addTerm(route.to, "nearby-origin-route-destination", { priority: basePriority + 8 });
    addTerm(route.from, "nearby-origin-route-origin", { priority: basePriority + 5 });
    route.via?.forEach((place) => addTerm(place, "nearby-origin-route-via", { priority: basePriority + 4 }));
  });
...
async function fetchNearbyBodsOriginStops(originCoordinates) {
  ...
  try {
    const payload = await fetchOsmBusRouteOverpassPayload(query, {}, 45000);
    return (payload.elements || [])
