...
const BODS_CACHE_SCHEMA_VERSION = "v77-bods-exact-origin-local-gate";
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  if (!timetable) {
    ...
    throw createBodsDiagnosticError(
      ...
      `BODS returned timetable datasets, but none passed the local-origin validation.${nearestText} A dataset now needs either a credible distribution of real parsed local stops or an exact ATCO/NaPTAN origin stop match with usable scheduled connections. Check the selected BODS query specs, rejected non-local dataset samples and operator coverage.`,
      {
        datasetQueryParametersUsed: datasetResult.queryParametersUsed,
        datasetRecordCount,
        queryTerms: datasetResult.searchTerms?.map((term) => term.value),
...
function isBodsTimetableModelLocalToOrigin(timetable, originCoordinates, maximumWalkToBusStopMetres) {
  ...
  // v77 correction: an exact ATCO/NaPTAN origin match is legitimate local proof.
  // v76 wrongly excluded these exact matched stops as "synthetic", so West Yorkshire datasets
  // with missing embedded stop coordinates were rejected even when the selected origin stop
  // appeared in the timetable graph. Name/ref/local_ref matches are still not accepted.
  const exactOriginMatchCount = Math.max(
    Number(timetable?.originMatchedStopCount) || 0,
    countBodsExactOriginCodeMatchedStops(timetable?.stops || [])
  );
  const connectionCount = getBodsTimetableConnectionCount(timetable);
  const stopCount = getBodsTimetableStopCount(timetable);
  const hasUsableMatchedLocalGraph = exactOriginMatchCount > 0
    && connectionCount > 0
    && stopCount > 1;

  return hasUsableMatchedLocalGraph;
}
