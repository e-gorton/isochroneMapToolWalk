...
const BODS_CACHE_SCHEMA_VERSION = "v67-bods-whole-zone-output";
...
function buildBodsTimetableIsochronesFromSearch(searchResult, configuredBands) {
  ...
  configuredBands.forEach((band) => {
    ...
    let rings = routeMetrics.length > stopMetrics.length
      ? buildBodsWholeZoneCatchmentRings(routeMetrics)
      : [];
    ...
    if (rings.length === 0 && stopMetrics.length > 0) {
      rings = buildBodsWholeZoneCatchmentRings(stopMetrics);
...
function buildBodsWholeZoneCatchmentRings(metrics) {
  ...
  return capPolygonRings(rings, 12);
}
