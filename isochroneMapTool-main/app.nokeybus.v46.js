...
const BODS_CACHE_SCHEMA_VERSION = "v57-bods-strict-local-dataset";
const BODS_MAX_DATASETS = 96;
const BODS_MAX_DATASET_QUERY_RESULTS = 96;
const BODS_MAX_DATASET_QUERY_ATTEMPTS = 22;
const BODS_MAX_DATASETS_PER_QUERY = 48;
...
const BODS_STOP_ENRICHMENT_RADIUS_METRES = 45000;
const BODS_STOP_ENRICHMENT_MAX_IDS = 3000;
...
async function fetchBodsDatasetMetadata(originCoordinates, maximumWalkToBusStopMetres, options = {}) {
  ...
  const cacheKey = JSON.stringify({
    ...
    api: "timetable-v46-adminarea-code-fix",
  });
...
async function buildBodsTimetableDatasetSearchTerms(originCoordinates) {
  ...
  const addTerm = (value, source, options = {}) => {
    ...
    if (!terms.some((term) => term.normalised === normalised)) {
      terms.push({ value: cleaned, normalised, source, priority: Number(options.priority) || 0 });
    }
  };
...
function buildBodsDatasetQuerySpecs(searchTerms) {
  ...
  searchTerms.forEach((term) => {
    ...
    if (/nearby-origin-stop-(atco|naptan|ref)/i.test(source) && /^\d{3,}/.test(compact)) {
  ...
  // admin-area codes derived from nearby ATCO/NaPTAN stop references first, then use free text only
  // through the `search` parameter.
...
function isBodsTimetableModelLocalToOrigin(timetable, originCoordinates, maximumWalkToBusStopMetres) {
  if ((Number(timetable?.originMatchedStopCount) || 0) > 0) {
    return true;
  }
  ...
  return Number.isFinite(nearestStopDistanceMetres)
    && nearestStopDistanceMetres <= localDatasetMaxDistanceMetres;
}
...
async function enrichBodsMissingStopCoordinates(missingStopsById, existingStopsById, options = {}) {
  ...
  if (unresolved.size > 0 && options.allowExternalStopEnrichment !== false) {
    const directIds = Array.from(unresolved.keys()).slice(0, Math.min(450, BODS_STOP_ENRICHMENT_MAX_IDS));
...
async function fetchBodsStopCoordinatesFromOsmByStopIds(stopIds, options = {}) {
  ...
  const originalIdByNormalised = buildBodsStopIdLookup(stopIds);
  const resolved = new Map();
  const regexChunks = getBodsStopIdRegexChunks(stopIds);
  for (const regex of regexChunks.slice(0, 45)) {
    if (options.signal?.aborted) {
      throw createServiceError(BODS_TIMETABLE_SERVICE_NAME, "cancelled", "BODS stop enrichment was cancelled.");
    }
    if (!regex) {
...
[out:json][timeout:45];
(
  node["naptan:AtcoCode"~"^(${regex})$",i];
  node["naptan:NaptanCode"~"^(${regex})$",i];
  node["ref:GB:naptan"~"^(${regex})$",i];
  node["ref"~"^(${regex})$",i];
  way["naptan:AtcoCode"~"^(${regex})$",i];
  way["naptan:NaptanCode"~"^(${regex})$",i];
  way["ref:GB:naptan"~"^(${regex})$",i];
  way["ref"~"^(${regex})$",i];
);
out center tags;
    `;
    const payload = await fetchOsmBusRouteOverpassPayload(query, { signal: options.signal }, 60000);
    (payload.elements || []).forEach((element) => {
      const latitude = Number.isFinite(Number(element.lat)) ? Number(element.lat) : Number(element.center?.lat);
      const longitude = Number.isFinite(Number(element.lon)) ? Number(element.lon) : Number(element.center?.lon);
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
...
function buildBodsTimetableIsochronesFromSearch(searchResult, configuredBands) {
  ...
  configuredBands.forEach((band) => {
    ...
    const metricSet = routeMetrics.length > stopMetrics.length ? routeMetrics : stopMetrics;
    let rings = buildSmoothedBusCatchmentRings(metricSet);
    let usedSparseGeometryFallback = false;
    let sparseGeometryMode = routeMetrics.length > stopMetrics.length ? "reachable_timetable_link_samples" : "smoothed_stop_buffers";

    if (rings.length === 0 && (stopMetrics.length > 0 || bandSegments.length > 0)) {
      rings = buildSparseBodsCatchmentRings(stopMetrics, bandSegments, bandTime);
      usedSparseGeometryFallback = rings.length > 0;
      sparseGeometryMode = getBodsSparseGeometryMode(bandSegments, stopMetrics);
    }
    if (rings.length > 0) {
      outputs.push({
        ...
        properties: {
          ...
          geometryMode: sparseGeometryMode,
...
function buildBodsReachableRouteMetrics(stopMetrics, reachableSegments, bandTime) {
  ...
  const seen = new Set();
  const addMetric = (coordinate, minutes) => {
    if (!coordinate || !Number.isFinite(coordinate.latitude) || !Number.isFinite(coordinate.longitude)) {
      return;
    }
    const key = `${coordinate.latitude.toFixed(5)}|${coordinate.longitude.toFixed(5)}|${Math.round(Number(minutes) || 0)}`;
    if (seen.has(key)) {
      return;
    }
    seen.add(key);
    metrics.push({ coordinate: { latitude: coordinate.latitude, longitude: coordinate.longitude }, minimumTotalBusAccessTimeMinutes: Math.min(Number(minutes) || bandTime, bandTime) });
  };
  ...
  reachableSegments.forEach((segment) => {
    ...
    sampleCoordinatePath(coordinates, 260).forEach(({ coordinate, fraction }) => {
...
function getBodsReachableSegmentGeometryCoordinates(segment) {
  ...
  if (segment?.startCoordinate && segment?.endCoordinate) {
    ...
    if (Number.isFinite(distance) && distance > 0 && distance <= 30000) {
