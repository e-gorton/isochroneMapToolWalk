...
const BODS_CACHE_SCHEMA_VERSION = "v76-bods-naptan-graph-access-cache";
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  const boardingStopContext = buildBodsBoardingStopContext(
    timetable,
  ...
  if (boardingStopContext.matchedBoardingStopCount === 0) {
    throw createBodsDiagnosticError(
      "no_bods_matched_boarding_stops",
      "Nearby physical bus stops were found, but none could be matched to local BODS timetable boarding stops using ATCO / NaPTAN codes or conservative stop-name matching.",
      {
        nearbyPhysicalStopCount: boardingStopContext.nearbyPhysicalStopCount,
        eligiblePhysicalBoardingStopCount: boardingStopContext.eligiblePhysicalBoardingStopCount,
        matchedBoardingStopCount: boardingStopContext.matchedBoardingStopCount,
  ...
  isochrones.metadata = {
    ...
    caveat: "Scheduled timetable output from BODS where TransXChange files could be parsed in-browser. Initial walking access is limited to walk-accessible physical NaPTAN/OSM stops; where the walked-to physical stop is not present in the parsed BODS graph, a conservative local access connector can seed a nearby BODS timetable stop with scheduled departures. Initial and transfer walking times use straight-line distance multiplied by 1.3 at 4.8 kph / 80 metres per minute. Includes scheduled wait time and bus in-vehicle time from parsed stop-to-stop timetable links. Does not include live disruption, reliability, crowding, cancellations, fare integration or operator-specific journey-planner rules.",
  };
...
function buildBodsBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes) {
  ...
  eligiblePhysicalStops.forEach((physicalStop) => {
    ...
    // Do not require the walked-to NaPTAN/physical stop to be present as a parsed BODS node.
    // If the user can walk to a real nearby stop, seed the timetable graph at a conservative
    // nearby BODS stop reached by a local interchange/access connector. This keeps the selected
    // walk limit tied to the physical boarding opportunity while still avoiding remote datasets.
    findBodsTimetableStopsLinkedFromPhysicalStop(timetableStops, physicalStop, directMatchIds, {
      includeStop: (stop) => (departuresByStopId.get(stop.id)?.length || 0) > 0,
    })
      .forEach((linked) => {
        addBoardingStopCandidate(linked.stop, physicalStop, {
          matchType: "access_linked_nearby_timetable_stop",
  ...
  // Final resilience fallback: if a parsed BODS timetable stop with departures is itself
  // within the selected origin-to-stop walk threshold, seed it directly. This handles cases
  // where NaPTAN/OSM stop metadata fails to cross-reference the local timetable stop even
  // though the BODS stop coordinate is plainly walk-accessible from the origin.
  findBodsTimetableStopsReachableFromOrigin(timetableStops, originCoordinates, maximumWalkToBusStopMetres, {
    includeStop: (stop) => (departuresByStopId.get(stop.id)?.length || 0) > 0,
  }).forEach((reachable) => {
    addBoardingStopCandidate(reachable.stop, {
      id: reachable.stop.id,
      name: reachable.stop.name || reachable.stop.id,
      atcoCode: reachable.stop.atcoCode || "",
      naptanCode: reachable.stop.naptanCode || "",
      latitude: reachable.stop.latitude,
      longitude: reachable.stop.longitude,
      distanceMetres: reachable.distanceMetres,
    }, {
      matchType: "origin_walkable_timetable_stop_fallback",
      physicalDistanceMetres: reachable.distanceMetres,
      accessLinkDistanceMetres: 0,
    });
  });
  ...
  const directMatchedBoardingStopCount = boardingStops.filter((stop) => ["direct_stop_reference_match", "naptan_code_or_name_timetable_stop", "origin_walkable_timetable_stop_fallback"].includes(stop.boardingMatchType)).length;
  const accessLinkedBoardingStopCount = boardingStops.filter((stop) => ["access_linked_nearby_timetable_stop", "naptan_graph_adjacent_timetable_stop", "naptan_gated_scheduled_graph_stop"].includes(stop.boardingMatchType)).length;
...
function findBodsTimetableStopsLinkedFromNaptanAccessStop(timetable, physicalStop = {}, directMatches = [], departuresByStopId = new Map(), departureMinutes = 0, excludedStopIds = new Set()) {
  ...
  // Last-resort but still NaPTAN-gated fallback: if a real physical NaPTAN stop is walkable but
  // the local timetable graph has no exact/code-adjacent scheduled node, use scheduled nodes from
  // the parsed local BODS graph without applying the user walking-distance threshold to the BODS
  // node. This is deliberately only reached after the physical NaPTAN stop has passed the walk gate.
  if (candidatesById.size === 0) {
    timetableStops
      .filter((stop) => stop?.id && isValidCoordinateObject(stop) && (departuresByStopId.get(stop.id)?.length || 0) > 0)
      .map((stop) => ({
        stop,
        distanceMetres: isValidCoordinateObject(physicalStop)
          ? getDistanceMetres(physicalStop.latitude, physicalStop.longitude, stop.latitude, stop.longitude)
          : 0,
      }))
      .sort((a, b) => a.distanceMetres - b.distanceMetres)
      ...
      .forEach((entry) => {
        addCandidate(entry.stop, "naptan_gated_scheduled_graph_stop");
      });
  }
...
function rankBodsNaptanGraphCandidate(candidate = {}) {
  const matchRank = {
    ...
    naptan_gated_scheduled_graph_stop: 2,
  ...
  if (searchResult.initialStopsWithinMaxWalkCount === 0) {
    ...
    if (Number(searchResult.matchedBoardingStopCount || 0) > 0 && Number(searchResult.boardingStopsWithDeparturesAfterSelectedTime || 0) === 0) {
      return {
        ...
        message: `Nearby physical NaPTAN/OSM stops were found within the selected walk distance, but no scheduled BODS graph boarding nodes could be seeded from those access stops after the selected time.${nearestText} Try a different departure time/date or check whether the downloaded BODS timetable datasets reference those NaPTAN stops.`,
        details,
      };
    }
    return {
...
function buildBodsTimetableSourceNote(metadata = {}) {
  ...
  const precisionWarningText = geometrySummary.straightLineFallbackUsed || geometrySummary.estimatedRuntimeUsed || Number(metadata.bodsEstimatedRuntimeConnectionCount || 0) > 0
    ...
    : "";
  const walkText = Number.isFinite(Number(metadata.maximumWalkToBusStopMetres)) ? ` Initial physical stop access is limited to ${Math.round(metadata.maximumWalkToBusStopMetres).toLocaleString("en-GB")} m from the selected origin; if the walked-to physical stop is not itself present in BODS, a local access connector may link it to a nearby BODS timetable stop. Transfer/access connector walks are limited to ${Math.round(metadata.transferWalkRadiusMetres || BODS_TRANSFER_RADIUS_METRES).toLocaleString("en-GB")} m.` : "";
...
function buildBusMethodLimitationText(metadata = {}) {
  if ((metadata.provider || "").includes("BODS") || getSelectedBusMethod() === BUS_METHOD_BODS) {
    ...
    return `Bus catchments are timetable-based outputs generated from Bus Open Data Service timetable data where local TransXChange files can be parsed. Initial walking access is limited to physical NaPTAN/OSM stops within ${Math.round(maximumWalkToBusStopMetres).toLocaleString("en-GB")} m. Where that physical stop is not present in the parsed BODS graph, the model can use a conservative local access connector to a nearby BODS timetable stop before applying scheduled wait time and bus in-vehicle running time. Initial, connector and transfer walking are estimated using straight-line distance multiplied by 1.3 and ${BUS_ACCESS_WALK_SPEED_KPH} kph / ${Math.round(BUS_ACCESS_WALK_SPEED_METRES_PER_MINUTE)} m per minute walking speed. Outputs do not include live disruption, reliability, cancellations, vehicle crowding or fare integration.`;
  }
