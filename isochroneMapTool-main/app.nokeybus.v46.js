...
function buildBodsBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes) {
  ...
  const addBoardingStopCandidate = (matchedStop, physicalStop, linkContext = {}) => {
    ...
    const physicalDistanceMetres = Number.isFinite(Number(physicalStop.distanceMetres))
    ...
    const accessLinkWalkMinutes = accessLinkDistanceMetres > 0
    ...
    const totalWalkingDistanceMetres = physicalDistanceMetres + accessLinkDistanceMetres;
    ...
    const candidate = {
      ...
      walkingDistanceMetres: Math.round(totalWalkingDistanceMetres),
  ...
  eligiblePhysicalStops.forEach((physicalStop) => {
    ...
    findBodsTimetableStopsLinkedFromPhysicalStop(timetableStops, physicalStop, directMatchIds)
      .filter((linked) => (departuresByStopId.get(linked.stop.id)?.length || 0) > 0)
      .forEach((linked) => {
...
function findBodsTimetableStopsLinkedFromPhysicalStop(timetableStops = [], physicalStop = {}, excludedStopIds = new Set()) {
  ...
  return (timetableStops || [])
    ...
    .filter((entry) => Number.isFinite(entry.distanceMetres) && entry.distanceMetres <= BODS_TRANSFER_RADIUS_METRES)
    .sort((a, b) => a.distanceMetres - b.distanceMetres)
...
function diagnoseBodsTimetableSearchFailure(searchResult) {
  ...
  if (searchResult.initialStopsWithinMaxWalkCount === 0) {
    ...
    return {
      ...
      message: `No walk-accessible physical stop could be linked to a usable BODS timetable boarding stop within the local interchange tolerance.${nearestText} Check local NaPTAN/BODS stop matching, the selected time, or operator coverage.`,
      details,
    };
  }
