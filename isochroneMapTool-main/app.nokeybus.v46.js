...
const NAPTAN_ACCESS_NODES_ENDPOINT = IS_FILE_CONTEXT
  ? "https://naptan.api.dft.gov.uk/v1/access-nodes"
  : resolveHostedAppEndpoint("/api/proxy/naptan/access-nodes");
...
const BODS_CACHE_SCHEMA_VERSION = "v82-bods-no-wait-runtime-cache";
...
const BODS_INITIAL_ACCESS_LINK_TOLERANCE_METRES = 1200;
const BODS_INITIAL_ACCESS_LINK_STOP_CAP = 24;
const BODS_NAPTAN_GRAPH_LINK_STOP_CAP = 96;
const BODS_NAPTAN_GRAPH_LINK_MAX_HOPS = 6;
const BODS_HYBRID_SYNTHETIC_STOP_PREFIX = "hybrid-naptan-osm-bods-";
const BODS_HYBRID_ROUTE_EXPANSION_METRES = 12000;
const BODS_HYBRID_ROUTE_STOP_MATCH_METRES = 220;
const BODS_HYBRID_BODS_ANCHOR_MATCH_METRES = 700;
const BODS_HYBRID_ROUTE_REF_MATCH_REQUIRED_WHEN_AVAILABLE = false;
const BODS_HYBRID_ANCHOR_CAP_PER_ACCESS_ROUTE = 18;
const BODS_HYBRID_MAX_SYNTHETIC_CONNECTIONS = 2800;
const BODS_HYBRID_INTERMEDIATE_STOP_SPACING_METRES = 650;
const BODS_HYBRID_MAX_INTERMEDIATE_STOPS_PER_LINK = 30;
const BODS_HYBRID_MIN_INTERPOLATED_SPEED_KPH = 7;
const BODS_HYBRID_MAX_INTERPOLATED_SPEED_KPH = 45;
const BODS_HYBRID_FALLBACK_SPEED_KPH = 18;
const BODS_HYBRID_TIMING_EPSILON_MINUTES = 0.03;
const BODS_HYBRID_TIMED_GRAPH_FALLBACK_SEARCH_METRES = 10000;
const BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_ANCHORS_PER_ACCESS_STOP = 12;
const BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_SEGMENT_METRES = 12000;
const BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_PROJECTION_METRES = 1200;
...
const BODS_REAL_LOCAL_STOP_RADIUS_METRES = 2500;
...
const BODS_PROGRESS_RENDER_INTERVAL = 12;
const BODS_PROGRESSIVE_MIN_LOCAL_DATASETS_FOR_EARLY_EXIT = 2;
const BODS_PROGRESSIVE_MIN_STOPS_FOR_EARLY_EXIT = 120;
const BODS_PROGRESSIVE_MIN_CONNECTIONS_FOR_EARLY_EXIT = 240;
const BODS_PROGRESSIVE_MIN_MATCHED_CONNECTIONS_FOR_EARLY_EXIT = 8;
...
async function publishBodsProgress(title, text, tone = "running", details = {}) {
  setStatus(title, text, tone);
  if (isBusTimingLogEnabled()) {
    console.debug("[BODS progress]", { title, text, tone, ...details });
  }
  render();
...
async function fetchBodsTimetableIsochronesForScenario(originCoordinates, options = {}) {
  ...
  await publishBodsProgress(
    "Loading BODS timetables",
    "Finding BODS timetable datasets using supported BODS timetable filters for the selected area.",
    "running",
    { phase: "dataset-discovery-start" }
  );
  const datasetResult = await fetchLocalBodsTimetableModelProgressively(originCoordinates, maximumWalkToBusStopMetres, {
    ...options,
    departureDate,
    departureMinutes,
    maximumBandMinutes,
  ...
  if (!timetable) {
    ...
    throw createBodsDiagnosticError(
      ...
      `BODS returned timetable datasets, but none passed the local-origin validation.${nearestText} The app found nearby physical stops, but the downloaded BODS datasets did not contain a usable matched ATCO/NaPTAN stop reference or parsed stop within the strict local tolerance. This prevents geographically wrong datasets around 10 km away being accepted as local. Check the selected BODS query specs and operator coverage.`,
      {
        datasetQueryParametersUsed: datasetResult.queryParametersUsed,
        datasetRecordCount,
        queryTerms: datasetResult.searchTerms?.map((term) => term.value),
  ...
  const boardingStopContext = await buildBodsHybridBoardingStopContext(
    timetable,
    datasetResult.nearbyOriginStops || [],
    originCoordinates,
  ...
  if (boardingStopContext.nearbyPhysicalStopCount === 0) {
    throw createBodsDiagnosticError("no_nearby_boarding_stops", "No nearby physical bus stops were found within the local BODS boarding-stop search area.", {
      nearbyPhysicalStopCount: 0,
      eligiblePhysicalBoardingStopCount: 0,
      matchedBoardingStopCount: 0,
      initialSeededStopCount: 0,
    });
  }
  if (boardingStopContext.eligiblePhysicalBoardingStopCount === 0) {
    throw createBodsDiagnosticError(
      "no_stops_within_max_walk",
      `No nearby physical bus stops were found within the selected maximum walk-to-bus-stop distance of ${Math.round(maximumWalkToBusStopMetres).toLocaleString("en-GB")} m.`,
      {
        nearbyPhysicalStopCount: boardingStopContext.nearbyPhysicalStopCount,
        eligiblePhysicalBoardingStopCount: boardingStopContext.eligiblePhysicalBoardingStopCount,
        matchedBoardingStopCount: boardingStopContext.matchedBoardingStopCount,
        initialSeededStopCount: 0,
      }
    );
  }
  if (boardingStopContext.matchedBoardingStopCount === 0) {
    throw createBodsDiagnosticError(
      "no_hybrid_bods_boarding_nodes",
      "Nearby physical NaPTAN/OSM bus stops were found within the selected walk distance, but no BODS timetable graph node could be seeded from the OSM bus routes serving those stops. The hybrid search looked along mapped bus-route corridors rather than requiring a BODS stop near the site. Check OSM route membership, BODS route/operator coverage and the selected date/time.",
      {
        nearbyPhysicalStopCount: boardingStopContext.nearbyPhysicalStopCount,
        eligiblePhysicalBoardingStopCount: boardingStopContext.eligiblePhysicalBoardingStopCount,
        matchedBoardingStopCount: boardingStopContext.matchedBoardingStopCount,
        hybridOsmRouteRelationCount: boardingStopContext.hybridOsmRouteRelationCount,
        hybridRouteAccessLinkCount: boardingStopContext.hybridRouteAccessLinkCount,
        hybridBodsAnchorCandidateCount: boardingStopContext.hybridBodsAnchorCandidateCount,
        hybridSyntheticConnectionCount: boardingStopContext.hybridSyntheticConnectionCount,
        hybridTimedGraphFallbackCandidateCount: boardingStopContext.hybridTimedGraphFallbackCandidateCount,
        hybridTimedGraphFallbackConnectionCount: boardingStopContext.hybridTimedGraphFallbackConnectionCount,
        hybridWarnings: boardingStopContext.hybridWarnings,
        physicalBoardingStopSamples: boardingStopContext.eligiblePhysicalStops.slice(0, 8),
      }
    );
  }

  await publishBodsProgress(
    "Running timetable search",
    "Calculating earliest scheduled arrivals including initial walk, wait time, bus running time and walking transfers.",
    "running",
    { phase: "earliest-arrival-search" }
  );
  const searchResult = runBodsEarliestArrivalSearch(
    timetable,
    originCoordinates,
    configuredBands,
    maximumBandMinutes,
  ...
  isochrones.metadata = {
    ...
    nearbyPhysicalStopCount: searchResult.nearbyPhysicalStopCount,
    eligiblePhysicalBoardingStopCount: searchResult.eligiblePhysicalBoardingStopCount,
    bodsMatchedBoardingStopCount: searchResult.matchedBoardingStopCount,
    bodsDirectMatchedBoardingStopCount: searchResult.directMatchedBoardingStopCount,
    bodsAccessLinkedBoardingStopCount: searchResult.accessLinkedBoardingStopCount,
    boardingStopsWithDeparturesAfterSelectedTime: searchResult.boardingStopsWithDeparturesAfterSelectedTime,
    hybridOsmRouteRelationCount: searchResult.hybridOsmRouteRelationCount,
    hybridRouteAccessLinkCount: searchResult.hybridRouteAccessLinkCount,
    hybridBodsAnchorCandidateCount: searchResult.hybridBodsAnchorCandidateCount,
    hybridSyntheticStopCount: searchResult.hybridSyntheticStopCount,
    hybridSyntheticConnectionCount: searchResult.hybridSyntheticConnectionCount,
    boardingStopsSeededForGraphRelaxationCount: searchResult.boardingStopsSeededForGraphRelaxationCount,
    hybridWarnings: searchResult.hybridWarnings,
    ...
    bodsSelectedDatasetSummaries: datasetResult.selectedDatasetSummaries,
    bodsRejectedDatasetSamples: datasetResult.rejectedDatasetSamples,
    ...
    reachableStopsByInitialBoardingStop: searchResult.reachableStopsByInitialBoardingStop,
    topRoutesByInitialBoardingStop: searchResult.topRoutesByInitialBoardingStop,
    validPostDepartureConnectionCount: searchResult.validPostDepartureConnectionCount,
    ...
    caveat: "Scheduled timetable output from BODS where TransXChange files could be parsed in-browser. Initial walking access is limited to walk-accessible physical NaPTAN/OSM stops. Those access stops are then linked into the parsed BODS timetable graph using direct ATCO/NaPTAN references where available, and otherwise by following mapped OSM bus-route relations outward to BODS timed anchor stops. BODS timed anchor stops do not need to be within the walking threshold; only the origin-to-physical-stop leg is counted as walking. Initial and transfer walking times use straight-line distance multiplied by 1.3 at 4.8 kph / 80 metres per minute. Includes scheduled wait time and bus in-vehicle time from parsed stop-to-stop timetable links. Does not include live disruption, reliability, crowding, cancellations, fare integration or operator-specific journey-planner rules.",
  };
...
async function fetchLocalBodsTimetableModelProgressively(originCoordinates, maximumWalkToBusStopMetres, options = {}) {
  const progressiveTiming = startBusTiming("BODS progressive dataset discovery", {
    originCoordinates,
    maximumWalkToBusStopMetres,
  });
  ...
  const result = {
    ...
    selectedQuerySpecs: [],
    selectedDatasetSummaries: [],
    rejectedDatasetSamples: [],
    errors: [],
    queryParametersUsed: [],
  ...
  for (let index = 0; index < querySpecs.length; index += 1) {
    ...
    const queryTiming = startBusTiming("BODS dataset query attempt", {
      queryIndex: index + 1,
      queryCount: querySpecs.length,
      querySpec: summariseBodsQuerySpec(querySpec),
    });
    ...
    await publishBodsProgress(
      "Loading BODS timetables",
      `Trying BODS dataset query ${index + 1} of ${querySpecs.length}: ${querySpec.value || querySpec.kind}.`,
      "running",
      { phase: "dataset-query", queryIndex: index + 1, queryCount: querySpecs.length }
    );
    ...
    try {
      ...
      const orderedRecordSummaries = orderedRecords.map((record) => summariseBodsDatasetRecord(record));
      ...
      if (downloadUrls.length === 0) {
        endBusTiming(queryTiming, { status: "no-download-urls", recordCount: newRecords.length });
      ...
      await publishBodsProgress(
        "Parsing BODS timetables",
        `Parsing ${downloadUrls.length} BODS dataset${downloadUrls.length === 1 ? "" : "s"} from query ${index + 1}.`,
        "running",
        { phase: "dataset-parse-start", queryIndex: index + 1, datasetCount: downloadUrls.length }
      );
      const timetable = await buildLocalBodsTimetableModel(downloadUrls, { ...options, originCoordinates, maximumWalkToBusStopMetres, nearbyOriginStops });
      timetable.selectedDatasetSummaries = orderedRecordSummaries.filter((record) =>
        downloadUrls.some((url) => url === record.downloadUrl || url === record.url)
      );
      ...
      if (timetable.localDatasetCount > 0 && timetable.stops.length > 0 && timetable.connections.length > 0) {
        ...
        if (result.timetable.parsedXmlFileCount >= BODS_MAX_XML_FILES || result.timetable.connections.length >= BODS_MAX_CONNECTIONS || result.timetable.stopsById.size >= BODS_MAX_STOPS) {
          ...
          endBusTiming(queryTiming, { status: "capped", localDatasetCount: result.timetable.localDatasetCount, stopCount: result.timetable.stopsById.size, connectionCount: result.timetable.connections.length });
          break;
        }
        if (shouldStopProgressiveBodsDatasetSearch(result, { ...options, originCoordinates, maximumWalkToBusStopMetres, nearbyOriginStops })) {
          result.timetable.warnings.push("BODS progressive search stopped early once a usable local timetable graph had been built.");
          await publishBodsProgress(
            "BODS local graph ready",
            "Stopping early because a usable local timetable graph has already been built for the selected origin.",
            "running",
            {
              phase: "dataset-query-early-exit",
              localDatasetCount: result.timetable.localDatasetCount,
              stopCount: result.timetable.stopsById.size,
              connectionCount: result.timetable.connections.length,
            }
          );
          endBusTiming(queryTiming, { status: "early-exit", localDatasetCount: result.timetable.localDatasetCount, stopCount: result.timetable.stopsById.size, connectionCount: result.timetable.connections.length });
          break;
        }
      }
      endBusTiming(queryTiming, {
        status: "success",
        recordCount: newRecords.length,
        downloadUrlCount: downloadUrls.length,
        localDatasetCount: timetable.localDatasetCount || 0,
        stopCount: timetable.stops?.length || 0,
        connectionCount: timetable.connections?.length || 0,
      });
    } catch (error) {
      if (error?.kind === "cancelled") {
        throw error;
      ...
      endBusTiming(queryTiming, { status: "failed", message: error?.message || String(error) });
    }
  }
  ...
  endBusTiming(progressiveTiming, {
    status: result.timetable ? "usable-local-graph" : "no-usable-local-graph",
    queryCountTried: result.querySpecsTried.length,
    datasetRecordCount: result.datasetRecordCount,
    downloadUrlCount: result.downloadUrlsTried.length,
    localDatasetCount: result.timetable?.localDatasetCount || 0,
    stopCount: result.timetable?.stopsById?.size || 0,
    connectionCount: result.timetable?.connections?.length || 0,
  });
  return result;
}

function shouldStopProgressiveBodsDatasetSearch(result, options = {}) {
  const timetable = result?.timetable;
  if (!timetable) {
    return false;
  }
  if ((timetable.localDatasetCount || 0) < BODS_PROGRESSIVE_MIN_LOCAL_DATASETS_FOR_EARLY_EXIT) {
    return false;
  }
  if ((timetable.stopsById?.size || 0) < BODS_PROGRESSIVE_MIN_STOPS_FOR_EARLY_EXIT) {
    return false;
  }
  if ((timetable.connections?.length || 0) < BODS_PROGRESSIVE_MIN_CONNECTIONS_FOR_EARLY_EXIT) {
    return false;
  }
  const localityDiagnostics = buildBodsDatasetLocalityDiagnostics(timetable, options);
  return localityDiagnostics.hasUsableMatchedBoardingGraph
    && Number(localityDiagnostics.scheduledConnectionsFromMatchedBoardingStops || 0) >= BODS_PROGRESSIVE_MIN_MATCHED_CONNECTIONS_FOR_EARLY_EXIT;
}
...
function mergeProgressiveLocalBodsTimetable(result, timetable, querySpec) {
  ...
  timetable.selectedDatasetSummaries?.forEach((summary) => {
    if (!result.selectedDatasetSummaries.some((entry) => entry.datasetId === summary.datasetId && entry.downloadUrl === summary.downloadUrl)) {
      result.selectedDatasetSummaries.push(summary);
    }
  });
  timetable.rejectedDatasetSamples?.forEach((sample) => {
    if (!result.rejectedDatasetSamples.some((entry) => entry.datasetUrl === sample.datasetUrl && entry.reason === sample.reason)) {
      result.rejectedDatasetSamples.push(sample);
    }
  });
...
async function buildBodsTimetableDatasetSearchTerms(originCoordinates) {
  ...
  await addBodsBaseDatasetSearchTerms(terms, addTerm, originCoordinates);
...
async function addBodsBaseDatasetSearchTerms(terms, addTerm, originCoordinates) {
  try {
    ...
    // Planning authority lookup is helpful for BODS/NaPTAN filtering but not required.
  }
...
async function fetchNearbyBodsOriginStops(originCoordinates) {
  ...
  const naptanStops = await fetchNearbyNaptanOriginStops(originCoordinates);
  if (naptanStops.length > 0) {
    return naptanStops;
  }
  return fetchNearbyBodsOriginStopsFromOsm(originCoordinates);
}

async function fetchNearbyNaptanOriginStops(originCoordinates) {
  const searchTerms = [];
  const addTerm = (value, source, options = {}) => {
    const cleaned = cleanBodsSearchTerm(value, options);
    if (!cleaned) {
    ...
    const normalised = cleaned.toLowerCase();
    if (!searchTerms.some((term) => term.normalised === normalised)) {
      searchTerms.push({ value: cleaned, normalised, source, priority: Number(options.priority) || 0 });
    }
  };
  await addBodsBaseDatasetSearchTerms(searchTerms, addTerm, originCoordinates);
  const adminAreaCodes = getBodsCandidateAdminAreaCodes(searchTerms);
  if (adminAreaCodes.length === 0) {
    return [];
  }

  const nearbyStops = [];
  const seenStopIds = new Set();
  for (const areaCode of adminAreaCodes) {
    const areaStops = await fetchNaptanAccessNodesForAreaCode(areaCode);
    areaStops.forEach((stop) => {
      const distanceMetres = getDistanceMetres(originCoordinates.latitude, originCoordinates.longitude, stop.latitude, stop.longitude);
      if (!Number.isFinite(distanceMetres) || distanceMetres > BODS_ORIGIN_STOP_SEARCH_RADIUS_METRES) {
        return;
      }
      const stopId = normaliseBodsStopIdentifier(stop.atcoCode || stop.naptanCode || stop.id);
      ...
      nearbyStops.push({
        ...stop,
        id: stop.atcoCode || stop.naptanCode || stop.id,
        distanceMetres,
        source: stop.source || "naptan_access_nodes_csv",
      });
    });
    if (nearbyStops.length >= 36) {
      break;
    }
  }
  return nearbyStops
...
function getBodsCandidateAdminAreaCodes(searchTerms = []) {
  const codes = [];
  const seen = new Set();
  const addCode = (value) => {
  ...
  getBodsAuthorityAdminAreaCodeEntries(searchTerms).forEach((entry) => addCode(entry.value));
  codes.slice().forEach((code) => {
    const baseCode = code.slice(0, 3);
    (BODS_ADMIN_AREA_NEIGHBOUR_CODES[baseCode] || []).forEach((neighbourCode) => addCode(neighbourCode));
  });
  return codes;
}

async function fetchNaptanAccessNodesForAreaCode(areaCode) {
  const cacheKey = JSON.stringify({
    api: BODS_CACHE_SCHEMA_VERSION,
    areaCode: String(areaCode || ""),
  });
  const cached = getMapCacheEntry(BODS_NAPTAN_ACCESS_NODES_CACHE, cacheKey, clonePlainValue);
  if (cached) {
    return cached;
  }
  const url = new URL(NAPTAN_ACCESS_NODES_ENDPOINT, window.location.origin);
  url.searchParams.set("atcoAreaCodes", String(areaCode || ""));
  url.searchParams.set("dataFormat", "csv");
  const response = await fetch(url.toString());
  if (!response.ok) {
    throw createServiceError(BODS_TIMETABLE_SERVICE_NAME, "api_outage", `NaPTAN access-nodes lookup returned status ${response.status}.`);
  }
  const text = await response.text();
  const parsedStops = parseNaptanAccessNodesCsv(text);
  setMapCacheEntry(BODS_NAPTAN_ACCESS_NODES_CACHE, cacheKey, parsedStops, clonePlainValue);
  return parsedStops;
}

function parseNaptanAccessNodesCsv(text) {
  return parseGtfsCsvRecords(text)
    .map((record) => normaliseNaptanAccessNodeRecord(record))
    .filter(Boolean);
}

function normaliseNaptanAccessNodeRecord(record = {}) {
  const latitude = parseFiniteCoordinateText(record.Latitude ?? record.latitude);
  const longitude = parseFiniteCoordinateText(record.Longitude ?? record.longitude);
  const converted = (!Number.isFinite(latitude) || !Number.isFinite(longitude))
    ? convertBritishNationalGridToWgs84(
        parseFiniteCoordinateText(record.Easting ?? record.easting),
        parseFiniteCoordinateText(record.Northing ?? record.northing)
      )
    : null;
  const resolvedLatitude = Number.isFinite(latitude) ? latitude : converted?.latitude;
  const resolvedLongitude = Number.isFinite(longitude) ? longitude : converted?.longitude;
  if (!isPlausibleUkWgs84Coordinate(resolvedLatitude, resolvedLongitude)) {
    return null;
  }
  ...
  return {
    id: record.ATCOCode || record.NaptanCode || "",
    name: record.CommonName || record.ShortCommonName || record.Landmark || "",
    street: record.Street || "",
    localityName: record.LocalityName || record.ParentLocalityName || record.Town || "",
    latitude: resolvedLatitude,
    longitude: resolvedLongitude,
    atcoCode: record.ATCOCode || "",
    naptanCode: record.NaptanCode || "",
    ref: record.PlateCode || record.NaptanCode || "",
    localRef: record.Indicator || record.Bearing || "",
    routeRef: "",
    operator: "",
    administrativeAreaCode: record.AdministrativeAreaCode || "",
    source: "naptan_access_nodes_csv",
  };
}

async function fetchNearbyBodsOriginStopsFromOsm(originCoordinates) {
...
function injectNearbyOriginStopMatchesIntoTxcModel(model, xmlText, nearbyStops = []) {
  const matches = findNearbyOriginStopMatchesInXml(xmlText, nearbyStops);
  ...
  const addOrUpdateMatchedStop = (txcStopId, txcStopName, nearbyStop, source) => {
    ...
    const injected = {
      ...
      syntheticNearbyOriginStop: /(?:^nearby_origin_stop_match$|name_match$)/i.test(source),
    };
  ...
  matches.forEach(({ candidate, stop, matchType }) => {
    if (!candidate || String(candidate).startsWith("name:")) {
      return;
    }
    const existingStop = stopsByNormalisedId.get(candidate);
    const missingStop = missingByNormalisedId.get(candidate);
    addOrUpdateMatchedStop(
      missingStop?.id || existingStop?.id || stop.atcoCode || stop.naptanCode || stop.ref || candidate,
      missingStop?.name || existingStop?.name || stop.name || candidate,
      stop,
      matchType === "code_in_xml" ? "nearby_origin_stop_code_match" : "nearby_origin_stop_match"
    );
  });

  missingStops.forEach((missingStop) => {
    const codeMatch = nearbyStops.find((nearbyStop) => getNearbyOriginStopCodeCandidates(nearbyStop).includes(normaliseBodsStopIdentifier(missingStop.id)));
    if (codeMatch) {
      addOrUpdateMatchedStop(missingStop.id, missingStop.name, codeMatch, "nearby_origin_stop_missing_id_match");
      return;
    }
    const nameMatch = nearbyStops.find((nearbyStop) => doBodsStopNamesMatch(missingStop.name, nearbyStop.name));
    if (nameMatch) {
      addOrUpdateMatchedStop(missingStop.id, missingStop.name, nameMatch, "nearby_origin_stop_name_match");
    }
  });

  stops.forEach((existingStop) => {
    const codeMatch = nearbyStops.find((nearbyStop) => getNearbyOriginStopCodeCandidates(nearbyStop).includes(normaliseBodsStopIdentifier(existingStop.id)));
    if (codeMatch) {
      addOrUpdateMatchedStop(existingStop.id, existingStop.name, codeMatch, "nearby_origin_stop_existing_id_match");
      return;
    }
    const nameMatch = nearbyStops.find((nearbyStop) => doBodsStopNamesMatch(existingStop.name, nearbyStop.name));
    if (nameMatch) {
      addOrUpdateMatchedStop(existingStop.id, existingStop.name, nameMatch, "nearby_origin_stop_existing_name_match");
    }
  });
...
function isBodsSyntheticOriginMatchedStop(stop = {}) {
  if (stop?.syntheticNearbyOriginStop === true) {
    return true;
  }
  const source = String(stop.coordinateSource || "");
  // Treat exact/code-matched ATCO or NaPTAN StopPointRefs as real local timetable stops.
  // Only name-only nearby-origin coordinate grafts are synthetic for locality validation.
  return /^nearby_origin_stop_(?:match|name_match|existing_name_match|graph_name_match)$/i.test(source);
}
...
function countBodsRealLocalStopsNearOrigin(stops, originCoordinates, radiusMetres = BODS_REAL_LOCAL_STOP_RADIUS_METRES) {
  ...
  return stops.filter((stop) => {
    if (!stop || isBodsSyntheticOriginMatchedStop(stop)) {
      return false;
    }
...
function buildBodsDatasetQuerySpecs(searchTerms) {
  ...
  const addSearchSpec = (value, source, includeStatus = true) => {
    addSpec("search", value, source, includeStatus);
  };

  const derivedAdminAreaCodes = [];
  const addDerivedAdminAreaCode = (code, source) => {
    const cleaned = String(code || "").replace(/[^0-9]/g, "");
    if (cleaned.length < 3) {
      return;
    }
    const key = `${cleaned}:${source}`;
    if (!derivedAdminAreaCodes.some((entry) => entry.key === key)) {
      derivedAdminAreaCodes.push({ key, value: cleaned, source });
    }
  };

  searchTerms.forEach((term) => {
    const source = String(term?.source || "");
    const compact = String(term?.value || "").replace(/[^A-Za-z0-9]/g, "");
    if (/nearby-origin-stop-(atco|naptan)$/i.test(source) && /^\d{3,}/.test(compact)) {
      addDerivedAdminAreaCode(compact.slice(0, 3), `${source}:admin-area-prefix3`);
      if (compact.length >= 4) {
        addDerivedAdminAreaCode(compact.slice(0, 4), `${source}:admin-area-prefix4`);
      }
    }
  });

  getBodsAuthorityAdminAreaCodeEntries(searchTerms).forEach((entry) => {
    addDerivedAdminAreaCode(entry.value, entry.source);
  });

  const derivedAdminAreaByCode = new Map();
  derivedAdminAreaCodes.forEach((entry) => {
    const code = String(entry.value || "").replace(/[^0-9]/g, "");
    if (!code || derivedAdminAreaByCode.has(code)) {
      return;
    }
    derivedAdminAreaByCode.set(code, entry);
  });

  const neighbourAdminAreaEntries = [];
  derivedAdminAreaByCode.forEach((entry, code) => {
    const baseCode = code.slice(0, 3);
    (BODS_ADMIN_AREA_NEIGHBOUR_CODES[baseCode] || []).forEach((neighbourCode) => {
      if (!derivedAdminAreaByCode.has(neighbourCode) && !neighbourAdminAreaEntries.some((candidate) => candidate.value === neighbourCode)) {
        neighbourAdminAreaEntries.push({
          value: neighbourCode,
          source: `${entry.source}:neighbour-admin-area-${baseCode}`,
        });
      }
    });
  });

  // BODS `adminArea` is an administrative area code, not a free-text council/stop name.
  // Prior versions submitted every term (including "Leeds City Council", "Littleborough" and stop names)
  // as adminArea, which returned plausible-looking but geographically wrong datasets. Try numeric
  // admin-area codes derived from nearby ATCO/NaPTAN stop references first, then neighbouring
  // admin areas before free-text searches. This avoids exhausting preview caps on Greater Manchester
  // only at cross-boundary sites such as Littleborough.
  derivedAdminAreaByCode.forEach((entry) => {
    addAdminAreaSpec(entry.value, entry.source, true);
  });
  neighbourAdminAreaEntries.forEach((entry) => {
    addAdminAreaSpec(entry.value, entry.source, true);
  });
  derivedAdminAreaByCode.forEach((entry) => {
  ...
  // Authority fields can still be useful as free-text search terms, but should not be used as
  // adminArea values unless we have a numeric NaPTAN administrative area code.
...
function summariseBodsDatasetRecord(record) {
  const datasetId = firstNonEmpty(record?.id, record?.dataset_id, record?.datasetID, record?.datasetId);
  const name = firstNonEmpty(record?.name, record?.description, record?.title, record?.dataset_name);
  const operator = firstNonEmpty(record?.operator_name, record?.operator, record?.noc, record?.publisher_name);
  const downloadUrl = extractBodsDatasetDownloadUrls({ results: [record] })[0] || "";
  return {
    datasetId: datasetId ? String(datasetId) : "",
    name: name ? String(name) : "",
    operator: operator ? String(operator) : "",
    adminArea: firstNonEmpty(record?.admin_area, record?.adminArea, record?.admin_area_code, record?.adminAreaCode) || "",
    downloadUrl,
    url: firstNonEmpty(record?.url, record?.download_url, record?.downloadUrl) || downloadUrl,
  };
}
...
async function buildLocalBodsTimetableModel(datasetUrls, options = {}) {
  const timing = startBusTiming("BODS local timetable merge", { datasetCount: datasetUrls.length });
  ...
  for (let index = 0; index < datasetUrls.length; index += 1) {
    ...
    const datasetTiming = startBusTiming("BODS dataset parse", {
    ...
    const localityDiagnostics = buildBodsDatasetLocalityDiagnostics(model, options);
    const nearestRealStopDistanceMetres = localityDiagnostics.nearestRealStopDistanceMetres;
    const realLocalStopCount = localityDiagnostics.realLocalStopCount;
    model.nearestRealStopDistanceMetres = nearestRealStopDistanceMetres;
    model.realLocalStopCount = realLocalStopCount;
    model.localBoardingStopDiagnostics = {
      nearbyPhysicalStopCount: localityDiagnostics.boardingStopContext?.nearbyPhysicalStopCount || 0,
      eligiblePhysicalBoardingStopCount: localityDiagnostics.boardingStopContext?.eligiblePhysicalBoardingStopCount || 0,
      matchedBoardingStopCount: localityDiagnostics.boardingStopContext?.matchedBoardingStopCount || 0,
      directMatchedBoardingStopCount: localityDiagnostics.boardingStopContext?.directMatchedBoardingStopCount || 0,
      accessLinkedBoardingStopCount: localityDiagnostics.boardingStopContext?.accessLinkedBoardingStopCount || 0,
      boardingStopsWithDeparturesAfterSelectedTime: localityDiagnostics.boardingStopContext?.boardingStopsWithDeparturesAfterSelectedTime || 0,
      scheduledConnectionsFromMatchedBoardingStops: localityDiagnostics.scheduledConnectionsFromMatchedBoardingStops || 0,
      hasUsableMatchedBoardingGraph: localityDiagnostics.hasUsableMatchedBoardingGraph,
      hasMultiStopLocalBoardingGraph: localityDiagnostics.hasMultiStopLocalBoardingGraph,
    };
    mergeBodsTimetableDiagnostics(combined, model);

    if (!isBodsTimetableModelLocalToOrigin(model, options.originCoordinates, options.maximumWalkToBusStopMetres, options)) {
      const rejectionDistanceMetres = Number.isFinite(nearestRealStopDistanceMetres)
        ? nearestRealStopDistanceMetres
        : nearestStopDistanceMetres;
      if (Number.isFinite(rejectionDistanceMetres)) {
        combined.rejectedNonLocalDatasetCount += 1;
        combined.nearestRejectedStopDistanceMetres = Math.min(combined.nearestRejectedStopDistanceMetres ?? Infinity, rejectionDistanceMetres);
        if (combined.nonLocalDatasetSamples.length < 8) {
          const rejectionSample = {
            datasetUrl,
            reason: "non_local_dataset",
            nearestStopDistanceMetres: Number.isFinite(nearestStopDistanceMetres) ? Math.round(nearestStopDistanceMetres) : null,
            ...
            matchedBoardingStopCount: model.localBoardingStopDiagnostics?.matchedBoardingStopCount || 0,
            boardingStopsWithDeparturesAfterSelectedTime: model.localBoardingStopDiagnostics?.boardingStopsWithDeparturesAfterSelectedTime || 0,
            scheduledConnectionsFromMatchedBoardingStops: model.localBoardingStopDiagnostics?.scheduledConnectionsFromMatchedBoardingStops || 0,
            stopCount: model.stops.length,
            parsedXmlFileCount: model.parsedXmlFileCount,
          };
          combined.nonLocalDatasetSamples.push(rejectionSample);
          combined.rejectedDatasetSamples.push(rejectionSample);
        }
        const nearestParsedText = Number.isFinite(nearestStopDistanceMetres)
          ? `nearest parsed/matched stop was ${Math.round(nearestStopDistanceMetres).toLocaleString("en-GB")} m`
          : "nearest parsed/matched stop distance was unavailable";
        const nearestRealText = Number.isFinite(nearestRealStopDistanceMetres)
          ? `nearest accepted local timetable stop was ${Math.round(nearestRealStopDistanceMetres).toLocaleString("en-GB")} m`
          : "no accepted local timetable stop with usable coordinates was found";
        combined.warnings.push(`Rejected non-local BODS dataset ${index + 1}: ${nearestParsedText} from the selected origin; ${nearestRealText}; ${realLocalStopCount} non-synthetic/code-matched stop${realLocalStopCount === 1 ? "" : "s"} were within ${BODS_REAL_LOCAL_STOP_RADIUS_METRES.toLocaleString("en-GB")} m; ${model.localBoardingStopDiagnostics?.matchedBoardingStopCount || 0} matched local boarding stop${model.localBoardingStopDiagnostics?.matchedBoardingStopCount === 1 ? "" : "s"} and ${model.localBoardingStopDiagnostics?.scheduledConnectionsFromMatchedBoardingStops || 0} post-departure scheduled connection${model.localBoardingStopDiagnostics?.scheduledConnectionsFromMatchedBoardingStops === 1 ? "" : "s"} were found from them.`);
      }
      endBusTiming(datasetTiming, {
        status: "rejected-non-local",
        nearestStopDistanceMetres: Number.isFinite(nearestStopDistanceMetres) ? Math.round(nearestStopDistanceMetres) : null,
        nearestRealStopDistanceMetres: Number.isFinite(nearestRealStopDistanceMetres) ? Math.round(nearestRealStopDistanceMetres) : null,
        realLocalStopCount,
        matchedBoardingStopCount: model.localBoardingStopDiagnostics?.matchedBoardingStopCount || 0,
  ...
  endBusTiming(timing, {
    localDatasetCount: combined.localDatasetCount,
    rejectedNonLocalDatasetCount: combined.rejectedNonLocalDatasetCount,
    stopCount: combined.stopsById.size,
    connectionCount: combined.connections.length,
    parsedXmlFileCount: combined.parsedXmlFileCount,
  });
  return combined;
}
...
function buildBodsDatasetLocalityDiagnostics(timetable, options = {}) {
  const stops = timetable?.stops || [];
  const originCoordinates = options.originCoordinates;
  const maximumWalkToBusStopMetres = options.maximumWalkToBusStopMetres;
  const departureMinutes = Number(options.departureMinutes);
  const maximumBandMinutes = Number.isFinite(Number(options.maximumBandMinutes))
    ? Number(options.maximumBandMinutes)
    : getMaximumBusBandMinutes(getConfiguredBandsForMode("bus"));
  const localDatasetMaxDistanceMetres = getBodsLocalDatasetMaxDistanceMetres(maximumWalkToBusStopMetres);
  const nearestRealStopDistanceMetres = getNearestRealBodsStopDistanceMetres(stops, originCoordinates);
  const realLocalStopCount = countBodsRealLocalStopsNearOrigin(stops, originCoordinates);
  const boardingStopContext = originCoordinates && Array.isArray(options.nearbyOriginStops) && Number.isFinite(departureMinutes)
    ? buildBodsBoardingStopContext(
        timetable,
        options.nearbyOriginStops,
        originCoordinates,
        maximumWalkToBusStopMetres,
        departureMinutes,
        maximumBandMinutes
    ...
    : null;
  const matchedBoardingStopIds = new Set((boardingStopContext?.boardingStops || []).map((stop) => stop.id));
  const scheduledConnectionsFromMatchedBoardingStops = (timetable?.connections || [])
    .map((connection) => normaliseBodsConnectionForDepartureWindow(connection, departureMinutes))
    .filter((connection) =>
      matchedBoardingStopIds.has(connection.fromStopId)
      && Number.isFinite(Number(connection.departureMinutes))
      && connection.departureMinutes >= departureMinutes
    ).length;
  const hasUsableMatchedBoardingGraph = Number(boardingStopContext?.matchedBoardingStopCount || 0) > 0
    && Number(boardingStopContext?.boardingStopsWithDeparturesAfterSelectedTime || 0) > 0
    && scheduledConnectionsFromMatchedBoardingStops > 0;
  const hasMultiStopLocalBoardingGraph = Number(boardingStopContext?.eligiblePhysicalBoardingStopCount || 0) <= 1
    ? hasUsableMatchedBoardingGraph
    : Number(boardingStopContext?.boardingStopsWithDeparturesAfterSelectedTime || 0) > 1
      && scheduledConnectionsFromMatchedBoardingStops > 1;
  ...
  return {
    ...
    realLocalStopCount,
    boardingStopContext,
    scheduledConnectionsFromMatchedBoardingStops,
    hasUsableMatchedBoardingGraph,
    hasMultiStopLocalBoardingGraph,
  };
}

function isBodsTimetableModelLocalToOrigin(timetable, originCoordinates, maximumWalkToBusStopMetres, options = {}) {
  const locality = buildBodsDatasetLocalityDiagnostics(timetable, {
    ...options,
    originCoordinates,
    maximumWalkToBusStopMetres,
  });
  ...
  // Do not accept a dataset solely because one nearby OSM stop name/code was injected into it.
  // That is how remote Potters Bar / South Mimms datasets were being treated as if they belonged
  // to the Leeds origin. A local BODS dataset must contain real, non-injected parsed stops close
  // to the origin.
  if (realLocalStopCount >= BODS_MIN_REAL_LOCAL_STOPS_FOR_ACCEPTANCE) {
    return true;
  }
  if (hasMultiStopLocalBoardingGraph || hasUsableMatchedBoardingGraph) {
    return true;
  }
  return Number.isFinite(nearestRealStopDistanceMetres)
    && nearestRealStopDistanceMetres <= localDatasetMaxDistanceMetres;
}

async function buildBodsTimetableModel(datasetUrls, options = {}) {
  const cacheKey = buildBodsTimetableModelCacheKey(datasetUrls, options);
  const cachedModel = getMapCacheEntry(BODS_TIMETABLE_MODEL_CACHE, cacheKey, cloneBodsTimetableModelResult);
  if (cachedModel) {
    if (isBusTimingLogEnabled()) {
      console.debug("[BODS progress]", {
        phase: "parsed-model-cache-hit",
        datasetUrls,
        stopCount: cachedModel.stops.length,
        connectionCount: cachedModel.connections.length,
      });
    }
    return cachedModel;
  }
  const timing = startBusTiming("BODS timetable model build", { datasetCount: datasetUrls.length, datasetUrls });
  ...
  for (let index = 0; index < datasetUrls.length; index += 1) {
    ...
    await publishBodsProgress(
      "Parsing BODS timetables",
      `Parsing dataset ${index + 1} of ${datasetUrls.length}.`,
      "running",
      { phase: "dataset-file-fetch", datasetIndex: index + 1, datasetCount: datasetUrls.length }
    );
    try {
      ...
      if ((options.nearbyOriginStops || []).length > 0 && !datasetFilesContainNearbyOriginStopReferences(files, options.nearbyOriginStops || [])) {
        warnings.push(`Dataset ${index + 1} was skipped because no nearby NaPTAN/ATCO stop reference was found in its timetable files.`);
        continue;
      }
      ...
      const remainingXmlCapacity = Math.max(1, BODS_MAX_XML_FILES - parsedXmlFileCount);
      const cappedTxcFiles = txcFiles.slice(0, Math.min(BODS_MAX_XML_FILES_PER_DATASET, remainingXmlCapacity));
      for (let fileIndex = 0; fileIndex < cappedTxcFiles.length; fileIndex += 1) {
        const file = cappedTxcFiles[fileIndex];
        if (fileIndex === 0 || (fileIndex + 1) % BODS_PROGRESS_RENDER_INTERVAL === 0) {
          await publishBodsProgress(
            "Parsing BODS timetables",
            `Parsing XML file ${fileIndex + 1} of ${cappedTxcFiles.length} in dataset ${index + 1}.`,
            "running",
            {
              phase: "txc-file-parse",
              datasetIndex: index + 1,
              datasetCount: datasetUrls.length,
              fileIndex: fileIndex + 1,
              fileCount: cappedTxcFiles.length,
            }
          );
        }
        try {
  ...
  setMapCacheEntry(BODS_TIMETABLE_MODEL_CACHE, cacheKey, model, cloneBodsTimetableModelResult);
  endBusTiming(timing, {
    datasetCount: datasetUrls.length,
    downloadedFileCount,
    parsedXmlFileCount,
    stopCount: model.stops.length,
    connectionCount: model.connections.length,
    unresolvedCoordinateStopCount: model.unresolvedCoordinateStopCount,
  });
  return model;
}
...
function normaliseBodsTimetableGraphAfterStopEnrichment(connections, stopsById, missingStopsById, options = {}) {
  ...
  const resolveByNearbyPhysicalStop = (rawId, missingStop) => {
    ...
    if (nearbyByCode) {
      return addSyntheticStop(rawId, missingStop?.name || nearbyByCode.name || rawId, nearbyByCode, "nearby_origin_stop_graph_id_match");
    }
    const nearbyByName = (options.nearbyOriginStops || []).find((nearbyStop) =>
      doBodsStopNamesMatch(missingStop?.name || rawId, nearbyStop.name)
    );
    if (nearbyByName) {
      return addSyntheticStop(rawId, missingStop?.name || nearbyByName.name || rawId, nearbyByName, "nearby_origin_stop_graph_name_match");
    }
    return null;
  };
...
async function buildBodsHybridBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes, options = {}) {
  resetBodsHybridSyntheticTimetableAdditions(timetable);
  const physicalStops = Array.isArray(nearbyOriginStops) ? nearbyOriginStops.filter(Boolean) : [];
  const timetableStops = timetable?.stops || [];
  const eligiblePhysicalStops = physicalStops
    .filter((stop) => Number.isFinite(Number(stop.distanceMetres)) && Number(stop.distanceMetres) <= maximumWalkToBusStopMetres)
    .map((stop) => {
      const walkingDistanceMetres = Math.round(Number(stop.distanceMetres) || 0);
      return {
        ...stop,
        walkingDistanceMetres,
        walkingTimeMinutes: estimateBusWalkTimeMinutes(walkingDistanceMetres),
      };
    })
    .filter((stop) => Number.isFinite(Number(stop.walkingTimeMinutes)));

  const boardingStopsById = new Map();
  const departuresByStopId = buildBodsDeparturesByStopId((timetable?.connections || []).map((connection) =>
    normaliseBodsConnectionForDepartureWindow(connection, departureMinutes)
  ));

  const addBoardingStopCandidate = (matchedStop, physicalStop, linkContext = {}) => {
    if (!matchedStop?.id || !isValidCoordinateObject(matchedStop) || !physicalStop) {
      return;
    }
    const physicalDistanceMetres = Number.isFinite(Number(linkContext.physicalDistanceMetres))
      ? Math.max(0, Number(linkContext.physicalDistanceMetres))
      : Number.isFinite(Number(physicalStop.walkingDistanceMetres))
        ? Number(physicalStop.walkingDistanceMetres)
        : Number.isFinite(Number(physicalStop.distanceMetres))
          ? Number(physicalStop.distanceMetres)
          : getDistanceMetres(originCoordinates.latitude, originCoordinates.longitude, physicalStop.latitude, physicalStop.longitude);
    const accessLinkDistanceMetres = Number.isFinite(Number(linkContext.accessLinkDistanceMetres))
      ? Math.max(0, Number(linkContext.accessLinkDistanceMetres))
      : 0;
    const accessLinkCountsAsWalk = linkContext.countAccessLinkAsWalk === true;
    const physicalWalkMinutes = Number.isFinite(Number(physicalStop.walkingTimeMinutes))
      ? Number(physicalStop.walkingTimeMinutes)
      : estimateBusWalkTimeMinutes(physicalDistanceMetres);
    const accessLinkWalkMinutes = accessLinkCountsAsWalk && accessLinkDistanceMetres > 0
      ? estimateBusWalkTimeMinutes(accessLinkDistanceMetres)
      : 0;
    if (!Number.isFinite(physicalWalkMinutes) || !Number.isFinite(accessLinkWalkMinutes)) {
      return;
    }
    const departuresAfterSelectedTime = Number.isFinite(Number(linkContext.departuresAfterSelectedTime))
      ? Number(linkContext.departuresAfterSelectedTime)
      : departuresByStopId.get(matchedStop.id)?.length || 0;
    const modelledWalkingDistanceMetres = physicalDistanceMetres + (accessLinkCountsAsWalk ? accessLinkDistanceMetres : 0);
    const totalWalkingTimeMinutes = physicalWalkMinutes + accessLinkWalkMinutes;
    const candidate = {
      id: matchedStop.id,
      name: matchedStop.name || physicalStop.name || matchedStop.id,
      latitude: matchedStop.latitude,
      longitude: matchedStop.longitude,
      atcoCode: physicalStop.atcoCode || matchedStop.atcoCode || "",
      naptanCode: physicalStop.naptanCode || matchedStop.naptanCode || "",
      physicalStopName: physicalStop.name || "",
      physicalStopId: physicalStop.id || physicalStop.atcoCode || physicalStop.naptanCode || "",
      selectedWalkDistanceMetres: Math.round(physicalDistanceMetres),
      accessLinkDistanceMetres: Math.round(accessLinkDistanceMetres),
      accessLinkCountsAsWalk,
      walkingDistanceMetres: Math.round(modelledWalkingDistanceMetres),
      walkingTimeMinutes: totalWalkingTimeMinutes,
      boardingMatchType: linkContext.matchType || "direct_stop_reference_match",
      departuresAfterSelectedTime,
      hybridRouteName: linkContext.hybridRouteName || "",
      hybridRouteRef: linkContext.hybridRouteRef || "",
      hybridAnchorStopName: linkContext.hybridAnchorStopName || "",
    };
    const existing = boardingStopsById.get(matchedStop.id);
    if (!existing || candidate.walkingTimeMinutes < existing.walkingTimeMinutes || candidate.departuresAfterSelectedTime > existing.departuresAfterSelectedTime) {
      boardingStopsById.set(matchedStop.id, candidate);
    }
  };

  eligiblePhysicalStops.forEach((physicalStop) => {
    const directMatches = matchBodsTimetableStopsToPhysicalStop(timetableStops, physicalStop);
    const directMatchIds = new Set();
    directMatches.forEach((matchedStop) => {
      directMatchIds.add(matchedStop.id);
      addBoardingStopCandidate(matchedStop, physicalStop, {
        matchType: "direct_stop_reference_match",
        accessLinkDistanceMetres: 0,
      });
    });
    findBodsTimetableStopsLinkedFromNaptanAccessStop(
      timetable,
      physicalStop,
      directMatches,
      departuresByStopId,
      ...
      addBoardingStopCandidate(linked.stop, physicalStop, {
        matchType: linked.matchType,
        accessLinkDistanceMetres: linked.distanceMetres,
        countAccessLinkAsWalk: false,
      });
    });
  });

  const hybridResult = await buildBodsHybridNaptanOsmRouteAccessLinks(
    timetable,
    eligiblePhysicalStops,
    originCoordinates,
    maximumWalkToBusStopMetres,
    departureMinutes,
    maximumBandMinutes,
  ...
  hybridResult.boardingStops.forEach((boardingStop) => {
    addBoardingStopCandidate(boardingStop.syntheticStop, boardingStop.physicalStop, {
      matchType: "hybrid_naptan_osm_route_to_bods_timed_anchor",
      accessLinkDistanceMetres: boardingStop.accessLinkDistanceMetres,
      countAccessLinkAsWalk: false,
      departuresAfterSelectedTime: boardingStop.departuresAfterSelectedTime,
      hybridRouteName: boardingStop.hybridRouteName,
      hybridRouteRef: boardingStop.hybridRouteRef,
      hybridAnchorStopName: boardingStop.hybridAnchorStopName,
    });
  });

  const boardingStops = Array.from(boardingStopsById.values())
    .map((stop) => ({
      ...stop,
      seedArrivalMinutes: departureMinutes + stop.walkingTimeMinutes,
    }))
    .sort((a, b) => a.walkingTimeMinutes - b.walkingTimeMinutes || b.departuresAfterSelectedTime - a.departuresAfterSelectedTime);
  // Seed the timetable search from every validated walk-accessible physical/BODS-linked stop.
  // Do not require that seed node itself to expose an outgoing BODS departure: in TransXChange
  // the physical boarding point may be represented as an arrival-only timing point, a split node,
  // or a NaPTAN/OSM access stop which then needs graph/connector relaxation to reach the first
  // scheduled BODS departure. Actual bus movement is still only created from BODS timetable
  // connections with parsed departure/arrival times.
  const seedStops = boardingStops.filter((stop) =>
    Number.isFinite(Number(stop.seedArrivalMinutes))
    && stop.walkingTimeMinutes < maximumBandMinutes
  );
  const seedStopsWithDirectDepartures = seedStops.filter((stop) => stop.departuresAfterSelectedTime > 0);
  const directMatchedBoardingStopCount = boardingStops.filter((stop) => ["direct_stop_reference_match", "naptan_code_or_name_timetable_stop"].includes(stop.boardingMatchType)).length;
  const accessLinkedBoardingStopCount = boardingStops.filter((stop) => ["naptan_graph_adjacent_timetable_stop", "hybrid_naptan_osm_route_to_bods_timed_anchor"].includes(stop.boardingMatchType)).length;

  return {
    nearbyPhysicalStopCount: physicalStops.length,
    eligiblePhysicalBoardingStopCount: eligiblePhysicalStops.length,
    matchedBoardingStopCount: boardingStops.length,
    directMatchedBoardingStopCount,
    accessLinkedBoardingStopCount,
    boardingStopsWithDeparturesAfterSelectedTime: seedStopsWithDirectDepartures.length,
    boardingStopsSeededForGraphRelaxationCount: seedStops.length,
    nearbyPhysicalStops: physicalStops,
    eligiblePhysicalStops,
    boardingStops,
    seedStops,
    hybridOsmRouteRelationCount: hybridResult.routeCount,
    hybridRouteAccessLinkCount: hybridResult.routeAccessLinkCount,
    hybridBodsAnchorCandidateCount: hybridResult.bodsAnchorCandidateCount,
    hybridSyntheticStopCount: hybridResult.syntheticStopCount,
    hybridSyntheticConnectionCount: hybridResult.syntheticConnectionCount,
    hybridTimedGraphFallbackCandidateCount: hybridResult.timedGraphFallbackCandidateCount || 0,
    hybridTimedGraphFallbackConnectionCount: hybridResult.timedGraphFallbackConnectionCount || 0,
    hybridWarnings: hybridResult.warnings,
  };
}

function resetBodsHybridSyntheticTimetableAdditions(timetable) {
  if (!timetable || typeof timetable !== "object") {
    return;
  }
  timetable.stops = (timetable.stops || []).filter((stop) => !String(stop?.id || "").startsWith(BODS_HYBRID_SYNTHETIC_STOP_PREFIX));
  timetable.connections = (timetable.connections || []).filter((connection) => !String(connection?.timingSource || "").startsWith("hybrid_naptan_osm_bods"));
  timetable.stopsById = new Map((timetable.stops || []).map((stop) => [stop.id, stop]));
}

async function buildBodsHybridNaptanOsmRouteAccessLinks(timetable, eligiblePhysicalStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes, options = {}) {
  const result = {
    routeCount: 0,
    routeAccessLinkCount: 0,
    bodsAnchorCandidateCount: 0,
    syntheticStopCount: 0,
    syntheticConnectionCount: 0,
    timedGraphFallbackCandidateCount: 0,
    timedGraphFallbackConnectionCount: 0,
    boardingStops: [],
    warnings: [],
  };
  if (!timetable || eligiblePhysicalStops.length === 0 || !isValidCoordinateObject(originCoordinates)) {
    return result;
  }
  ...
  try {
    await publishBodsProgress(
      "Linking NaPTAN access to BODS",
      "Finding mapped OSM bus routes serving the walk-accessible NaPTAN/OSM stops, then searching along those routes for BODS timed anchor stops.",
      "running",
      { phase: "hybrid-naptan-osm-route-link-start" }
    );
    payload = await fetchOsmBusRouteRelationPayload(originCoordinates, options, {
      maximumBandMinutes,
      stopSearchRadiusMetres: getBusStopSearchRadiusMetres(maximumWalkToBusStopMetres),
      maximumWalkToBusStopMetres,
      computationDeadlineMs: getBusComputationDeadlineMs(performance.now(), 90000),
    });
  } catch (error) {
    if (error?.kind === "cancelled") {
      throw error;
    }
    result.warnings.push(`OSM route lookup for hybrid BODS access failed: ${error?.message || String(error)}`);
    payload = { elements: [] };
  }

  const routes = extractBodsHybridOsmAccessRoutesFromPayload(payload, eligiblePhysicalStops);
  result.routeCount = routes.length;
  result.routeAccessLinkCount = routes.reduce((total, route) => total + route.accessAnchors.length, 0);
  if (routes.length === 0) {
    result.warnings.push("No mapped OSM bus route relation could be linked to the walk-accessible physical stops. The app will still try the BODS timed-graph fallback without requiring OSM route membership.");
  }

  const existingStopsById = timetable.stopsById instanceof Map
    ? timetable.stopsById
    : new Map((timetable.stops || []).map((stop) => [stop.id, stop]));
  timetable.stopsById = existingStopsById;
  const existingConnections = timetable.connections || [];
  const normalisedConnections = existingConnections.map((connection) => normaliseBodsConnectionForDepartureWindow(connection, departureMinutes));
  const departuresByStopId = buildBodsDeparturesByStopId(normalisedConnections);
  const syntheticStopIds = new Set();
  const syntheticConnectionKeys = new Set();
  let syntheticSequence = 1;

  routes.forEach((route, routeIndex) => {
    const projectedBodsStops = projectBodsTimetableStopsOntoHybridRoute(timetable, route, departuresByStopId);
    route.projectedBodsStops = projectedBodsStops;
    route.projectedBodsStopById = new Map(projectedBodsStops.map((entry) => [entry.stop.id, entry]));
    enrichBodsConnectionsWithHybridOsmRouteGeometry(timetable, route, projectedBodsStops, normalisedConnections);
    result.bodsAnchorCandidateCount += projectedBodsStops.length;

    route.accessAnchors.forEach((accessAnchor, accessIndex) => {
      const anchorCandidates = mergeBodsHybridAnchorCandidates([
        ...buildBodsHybridAnchorCandidatesForAccess(route, accessAnchor, departuresByStopId, departureMinutes, maximumBandMinutes),
        ...buildBodsHybridConnectionSpanCandidatesForAccess(route, accessAnchor, normalisedConnections, departureMinutes, maximumBandMinutes),
      ]);
      if (anchorCandidates.length === 0) {
        return;
      }
      const syntheticAccessStop = buildBodsHybridSyntheticAccessStop(accessAnchor, route, routeIndex, accessIndex);
      ensureBodsHybridTimetableStop(timetable, syntheticAccessStop, syntheticStopIds);
      let departuresFromSyntheticAccess = 0;
      let firstAnchorName = "";

      anchorCandidates.slice(0, BODS_HYBRID_ANCHOR_CAP_PER_ACCESS_ROUTE).forEach((anchorCandidate, anchorIndex) => {
        if (result.syntheticConnectionCount >= BODS_HYBRID_MAX_SYNTHETIC_CONNECTIONS) {
          return;
        }
        const pathResult = appendBodsHybridSyntheticInterpolatedPath(
          timetable,
          route,
          accessAnchor,
          syntheticAccessStop,
          anchorCandidate,
          {
            syntheticSequence,
            syntheticStopIds,
            syntheticConnectionKeys,
          }
        );
        syntheticSequence += pathResult.syntheticStopsAdded + 1;
        result.syntheticStopCount += pathResult.syntheticStopsAdded;
        result.syntheticConnectionCount += pathResult.syntheticConnectionsAdded;
        if (pathResult.startsAtSyntheticAccess) {
          departuresFromSyntheticAccess += 1;
          if (!firstAnchorName) {
            firstAnchorName = anchorCandidate.stop.name || anchorCandidate.stop.id;
          }
        }
      });

      if (departuresFromSyntheticAccess > 0) {
        result.boardingStops.push({
          syntheticStop: syntheticAccessStop,
          physicalStop: accessAnchor.physicalStop,
          accessLinkDistanceMetres: Math.round(Math.min(...anchorCandidates.map((candidate) => candidate.routeDistanceMetres).filter((value) => Number.isFinite(value)))),
          departuresAfterSelectedTime: departuresFromSyntheticAccess,
          hybridRouteName: route.name || "",
          hybridRouteRef: route.ref || "",
          hybridAnchorStopName: firstAnchorName,
        });
      }
    });
  });

  if (result.boardingStops.length === 0) {
    const fallbackResult = buildBodsHybridTimedGraphFallbackAccessLinks(
      timetable,
      eligiblePhysicalStops,
      originCoordinates,
      departureMinutes,
    ...
    result.timedGraphFallbackCandidateCount = fallbackResult.candidateCount;
    result.timedGraphFallbackConnectionCount = fallbackResult.syntheticConnectionCount;
    result.syntheticStopCount += fallbackResult.syntheticStopCount;
    result.syntheticConnectionCount += fallbackResult.syntheticConnectionCount;
    result.boardingStops.push(...fallbackResult.boardingStops);
    result.warnings.push(...fallbackResult.warnings);
  }

  if (result.syntheticConnectionCount === 0) {
    result.warnings.push("Mapped OSM routes were found from the access stops, but no catchable BODS timed anchor could be created after the selected time. A secondary timed-graph fallback also failed to seed from nearby NaPTAN/OSM stops.");
  }
  return result;
}


function buildBodsHybridTimedGraphFallbackAccessLinks(timetable, eligiblePhysicalStops, originCoordinates, departureMinutes, maximumBandMinutes, context = {}) {
  const result = {
    candidateCount: 0,
    syntheticStopCount: 0,
    syntheticConnectionCount: 0,
  ...
  if (!timetable || !Array.isArray(eligiblePhysicalStops) || eligiblePhysicalStops.length === 0) {
    return result;
  }
  if (!isValidCoordinateObject(originCoordinates)) {
    return result;
  }
  if (!(timetable.stopsById instanceof Map)) {
    timetable.stopsById = new Map((timetable.stops || []).map((stop) => [stop.id, stop]));
  }

  const syntheticStopIds = context.syntheticStopIds || new Set();
  const syntheticConnectionKeys = context.syntheticConnectionKeys || new Set();
  let syntheticSequence = Number(context.syntheticSequence) || 1;
  const stopById = timetable.stopsById;
  const normalisedConnections = (timetable.connections || [])
    .map((connection) => normaliseBodsConnectionForDepartureWindow(connection, departureMinutes))
    .filter((connection) =>
      connection?.fromStopId &&
      connection?.toStopId &&
      Number.isFinite(Number(connection.departureMinutes)) &&
      Number.isFinite(Number(connection.arrivalMinutes)) &&
      Number(connection.arrivalMinutes) > Number(connection.departureMinutes) &&
      Number(connection.departureMinutes) <= departureMinutes + maximumBandMinutes + BODS_LOOKAHEAD_MINUTES
    )
    .sort((a, b) => Number(a.departureMinutes) - Number(b.departureMinutes));

  if (normalisedConnections.length === 0) {
    result.warnings.push("Timed-graph fallback could not run because no normalised BODS timetable connections were available after the selected time.");
    return result;
  }

  eligiblePhysicalStops.forEach((physicalStop, accessIndex) => {
    if (!isValidCoordinateObject(physicalStop)) {
      return;
    }
    const walkingTimeMinutes = Number.isFinite(Number(physicalStop.walkingTimeMinutes))
      ? Number(physicalStop.walkingTimeMinutes)
      : estimateBusWalkTimeMinutes(Number(physicalStop.walkingDistanceMetres) || Number(physicalStop.distanceMetres) || 0);
    if (!Number.isFinite(walkingTimeMinutes)) {
      return;
    }
    const seedArrivalMinutes = departureMinutes + walkingTimeMinutes;
    const syntheticAccessStop = buildBodsHybridTimedGraphFallbackAccessStop(physicalStop, accessIndex);
    ensureBodsHybridTimetableStop(timetable, syntheticAccessStop, syntheticStopIds);

    const candidates = buildBodsHybridTimedGraphFallbackCandidatesForAccess(
      physicalStop,
      syntheticAccessStop,
      stopById,
      normalisedConnections,
      seedArrivalMinutes,
      departureMinutes,
      maximumBandMinutes
    );
    ...
    let departuresFromSyntheticAccess = 0;
    let firstAnchorName = "";
    candidates.slice(0, BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_ANCHORS_PER_ACCESS_STOP).forEach((candidate) => {
      if (result.syntheticConnectionCount >= BODS_HYBRID_MAX_SYNTHETIC_CONNECTIONS) {
        return;
      }
      const appendResult = appendBodsHybridTimedGraphFallbackPath(
        timetable,
        syntheticAccessStop,
        candidate,
        {
      ...
      syntheticSequence += 1;
      result.syntheticConnectionCount += appendResult.syntheticConnectionsAdded;
      result.syntheticStopCount += appendResult.syntheticStopsAdded;
      if (appendResult.startsAtSyntheticAccess) {
        departuresFromSyntheticAccess += 1;
        if (!firstAnchorName) {
          firstAnchorName = candidate.anchorStop.name || candidate.anchorStop.id;
        }
      }
    });
    ...
    if (departuresFromSyntheticAccess > 0) {
      result.boardingStops.push({
        syntheticStop: syntheticAccessStop,
        physicalStop,
        accessLinkDistanceMetres: Math.round(Math.min(...candidates.map((candidate) => candidate.routeDistanceMetres).filter((value) => Number.isFinite(value)))),
        departuresAfterSelectedTime: departuresFromSyntheticAccess,
        hybridRouteName: "BODS timed graph fallback",
        hybridRouteRef: candidates[0]?.connection?.routeRef || candidates[0]?.connection?.routeName || "",
        hybridAnchorStopName: firstAnchorName,
      });
    }
  });
  ...
  if (result.syntheticConnectionCount === 0) {
    result.warnings.push("Timed-graph fallback searched BODS timetable connections up to 10 km from the walk-accessible NaPTAN/OSM stops, but no catchable timed segment could be joined after the selected time.");
  }
  return result;
}

function buildBodsHybridTimedGraphFallbackCandidatesForAccess(physicalStop, syntheticAccessStop, stopById, normalisedConnections, seedArrivalMinutes, departureMinutes, maximumBandMinutes) {
  const candidatesByKey = new Map();
  normalisedConnections.forEach((connection) => {
    const fromStop = stopById.get(connection.fromStopId);
    const toStop = stopById.get(connection.toStopId);
    if (!isValidCoordinateObject(fromStop) || !isValidCoordinateObject(toStop)) {
      return;
    }
    const runtime = Number(connection.arrivalMinutes) - Number(connection.departureMinutes);
    if (!Number.isFinite(runtime) || runtime <= 0) {
      return;
    }

    const scheduledAnchorConnectorCandidate = buildBodsHybridTimedGraphFallbackScheduledAnchorConnectorCandidate(
      physicalStop,
      syntheticAccessStop,
      fromStop,
      connection,
    ...
    if (scheduledAnchorConnectorCandidate) {
      const key = `${scheduledAnchorConnectorCandidate.anchorStop.id}|${connection.tripId || connection.routeId || ""}|${Math.round(scheduledAnchorConnectorCandidate.syntheticDepartureMinutes * 10)}|connector`;
      const existing = candidatesByKey.get(key);
      if (!existing || scheduledAnchorConnectorCandidate.routeDistanceMetres < existing.routeDistanceMetres) {
        candidatesByKey.set(key, scheduledAnchorConnectorCandidate);
      }
    }

    const directAnchorCandidate = buildBodsHybridTimedGraphFallbackDirectAnchorCandidate(
      physicalStop,
      syntheticAccessStop,
      fromStop,
      connection,
    ...
    if (directAnchorCandidate) {
      const key = `${directAnchorCandidate.anchorStop.id}|${connection.tripId || connection.routeId || ""}|${Math.round(directAnchorCandidate.syntheticDepartureMinutes * 10)}|direct`;
      const existing = candidatesByKey.get(key);
      if (!existing || directAnchorCandidate.routeDistanceMetres < existing.routeDistanceMetres) {
        candidatesByKey.set(key, directAnchorCandidate);
      }
    }

    const spanCandidate = buildBodsHybridTimedGraphFallbackSpanCandidate(
      physicalStop,
      syntheticAccessStop,
      fromStop,
      toStop,
    ...
    if (spanCandidate) {
      const key = `${spanCandidate.anchorStop.id}|${connection.tripId || connection.routeId || ""}|${Math.round(spanCandidate.syntheticDepartureMinutes * 10)}|span`;
      const existing = candidatesByKey.get(key);
      if (!existing || spanCandidate.routeDistanceMetres < existing.routeDistanceMetres) {
        candidatesByKey.set(key, spanCandidate);
      }
    }
  });
  ...
  return Array.from(candidatesByKey.values())
    .sort((a, b) =>
      Number(a.syntheticDepartureMinutes) - Number(b.syntheticDepartureMinutes) ||
      Number(a.routeDistanceMetres) - Number(b.routeDistanceMetres)
    )
    .slice(0, BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_ANCHORS_PER_ACCESS_STOP * 2);
}

function buildBodsHybridTimedGraphFallbackScheduledAnchorConnectorCandidate(physicalStop, syntheticAccessStop, anchorStop, connection, seedArrivalMinutes, departureMinutes, maximumBandMinutes) {
  if (!isValidCoordinateObject(physicalStop) || !isValidCoordinateObject(anchorStop)) {
    return null;
  }
  const straightDistanceMetres = getDistanceMetres(physicalStop.latitude, physicalStop.longitude, anchorStop.latitude, anchorStop.longitude);
  if (!Number.isFinite(straightDistanceMetres) || straightDistanceMetres > BODS_HYBRID_TIMED_GRAPH_FALLBACK_SEARCH_METRES) {
    return null;
  }
  const connectionDeparture = Number(connection.departureMinutes);
  const connectionArrival = Number(connection.arrivalMinutes);
  if (!Number.isFinite(connectionDeparture) || !Number.isFinite(connectionArrival) || connectionArrival <= connectionDeparture) {
    return null;
  }
  // This is a data/graph connector to the first catchable BODS departure anchor. It does not
  // invent a bus running time: the connector simply places the search at the BODS from-stop just
  // before the parsed BODS departure so that the next real BODS connection supplies the actual
  // scheduled bus movement and elapsed time.
  const anchorArrivalMinutes = connectionDeparture - BODS_HYBRID_TIMING_EPSILON_MINUTES;
  if (
    !Number.isFinite(anchorArrivalMinutes) ||
    anchorArrivalMinutes < seedArrivalMinutes ||
    anchorArrivalMinutes - departureMinutes > maximumBandMinutes + BODS_LOOKAHEAD_MINUTES ||
    connectionArrival - departureMinutes > maximumBandMinutes + BODS_LOOKAHEAD_MINUTES
  ) {
    return null;
  }
  const syntheticDepartureMinutes = Math.min(seedArrivalMinutes, anchorArrivalMinutes - BODS_HYBRID_TIMING_EPSILON_MINUTES);
  if (!isBodsHybridTimedGraphFallbackCandidateTimingUsable(syntheticDepartureMinutes, anchorArrivalMinutes, seedArrivalMinutes, departureMinutes, maximumBandMinutes)) {
    return null;
  }
  return {
    candidateType: "timed_graph_scheduled_anchor_connector",
    accessStop: syntheticAccessStop,
    anchorStop,
    connection,
    routeDistanceMetres: straightDistanceMetres * BUS_ACCESS_WALK_DETOUR_FACTOR,
    travelMinutes: anchorArrivalMinutes - syntheticDepartureMinutes,
    syntheticDepartureMinutes,
    anchorArrivalMinutes,
    isHybridAccessConnectorOnly: true,
    geometry: [
      { latitude: physicalStop.latitude, longitude: physicalStop.longitude },
      { latitude: anchorStop.latitude, longitude: anchorStop.longitude },
    ],
  };
}

function buildBodsHybridTimedGraphFallbackDirectAnchorCandidate(physicalStop, syntheticAccessStop, anchorStop, connection, seedArrivalMinutes, departureMinutes, maximumBandMinutes) {
  const straightDistanceMetres = getDistanceMetres(physicalStop.latitude, physicalStop.longitude, anchorStop.latitude, anchorStop.longitude);
  if (!Number.isFinite(straightDistanceMetres) || straightDistanceMetres > BODS_HYBRID_TIMED_GRAPH_FALLBACK_SEARCH_METRES) {
    return null;
  }
  const routeDistanceMetres = straightDistanceMetres * BUS_ACCESS_WALK_DETOUR_FACTOR;
  const travelMinutes = estimateBodsHybridTimedGraphFallbackTravelMinutes(routeDistanceMetres, connection);
  const syntheticDepartureMinutes = Number(connection.departureMinutes) - travelMinutes;
  const anchorArrivalMinutes = Number(connection.departureMinutes) - BODS_HYBRID_TIMING_EPSILON_MINUTES;
  if (!isBodsHybridTimedGraphFallbackCandidateTimingUsable(syntheticDepartureMinutes, anchorArrivalMinutes, seedArrivalMinutes, departureMinutes, maximumBandMinutes)) {
    return null;
  }
  return {
    candidateType: "timed_graph_direct_anchor",
    accessStop: syntheticAccessStop,
    anchorStop,
    connection,
    routeDistanceMetres,
...
function buildBodsHybridTimedGraphFallbackSpanCandidate(physicalStop, syntheticAccessStop, fromStop, toStop, connection, seedArrivalMinutes, departureMinutes, maximumBandMinutes) {
  const segmentDistanceMetres = getDistanceMetres(fromStop.latitude, fromStop.longitude, toStop.latitude, toStop.longitude) * BUS_ACCESS_WALK_DETOUR_FACTOR;
  if (!Number.isFinite(segmentDistanceMetres) || segmentDistanceMetres < 50 || segmentDistanceMetres > BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_SEGMENT_METRES) {
    return null;
  }
  const projection = projectCoordinateToLineSegmentFraction(physicalStop, fromStop, toStop);
  if (!projection || !Number.isFinite(Number(projection.fraction))) {
    return null;
  }
  if (projection.fraction <= 0.02 || projection.fraction >= 0.98 || projection.distanceMetres > BODS_HYBRID_TIMED_GRAPH_FALLBACK_MAX_PROJECTION_METRES) {
    return null;
  }
  const runtime = Number(connection.arrivalMinutes) - Number(connection.departureMinutes);
  if (!Number.isFinite(runtime) || runtime <= 0) {
  ...
  const syntheticDepartureMinutes = Number(connection.departureMinutes) + runtime * projection.fraction;
  const anchorArrivalMinutes = Number(connection.arrivalMinutes);
  const routeDistanceMetres = segmentDistanceMetres * (1 - projection.fraction);
  if (!isBodsHybridTimedGraphFallbackCandidateTimingUsable(syntheticDepartureMinutes, anchorArrivalMinutes, seedArrivalMinutes, departureMinutes, maximumBandMinutes)) {
    return null;
  }
  return {
    candidateType: "timed_graph_interpolated_span",
    accessStop: syntheticAccessStop,
    anchorStop: toStop,
    connection,
    routeDistanceMetres,
...
function isBodsHybridTimedGraphFallbackCandidateTimingUsable(syntheticDepartureMinutes, anchorArrivalMinutes, seedArrivalMinutes, departureMinutes, maximumBandMinutes) {
  return Number.isFinite(Number(syntheticDepartureMinutes)) &&
    Number.isFinite(Number(anchorArrivalMinutes)) &&
    anchorArrivalMinutes > syntheticDepartureMinutes &&
    syntheticDepartureMinutes + 0.05 >= seedArrivalMinutes &&
    syntheticDepartureMinutes - departureMinutes <= maximumBandMinutes + BODS_LOOKAHEAD_MINUTES &&
    anchorArrivalMinutes - departureMinutes <= maximumBandMinutes + BODS_LOOKAHEAD_MINUTES;
}

function estimateBodsHybridTimedGraphFallbackTravelMinutes(routeDistanceMetres, connection = {}) {
  let speedKph = Number(connection.impliedSpeedKph);
  if (!Number.isFinite(speedKph) || speedKph <= 0) {
    const runtime = Number(connection.arrivalMinutes) - Number(connection.departureMinutes);
    const impliedDistance = Number(connection.distanceMetres);
    if (Number.isFinite(runtime) && runtime > 0 && Number.isFinite(impliedDistance) && impliedDistance > 0) {
      speedKph = (impliedDistance / 1000) / (runtime / 60);
    }
  }
  if (!Number.isFinite(speedKph) || speedKph <= 0) {
    speedKph = BODS_HYBRID_FALLBACK_SPEED_KPH;
  }
  speedKph = clamp(speedKph, BODS_HYBRID_MIN_INTERPOLATED_SPEED_KPH, BODS_HYBRID_MAX_INTERPOLATED_SPEED_KPH);
  return (Math.max(0, Number(routeDistanceMetres) || 0) / 1000) / speedKph * 60;
}

function buildBodsHybridTimedGraphFallbackAccessStop(physicalStop, accessIndex) {
  const rawId = physicalStop.id || physicalStop.atcoCode || physicalStop.naptanCode || `fallback-${accessIndex}`;
  const stopKey = normaliseBodsStopIdentifier(rawId) || `fallback-${accessIndex}`;
  return {
    id: `${BODS_HYBRID_SYNTHETIC_STOP_PREFIX}timed-graph-access-${stopKey}-${accessIndex}`,
    name: physicalStop.name || `Timed graph access stop ${accessIndex + 1}`,
    latitude: Number(physicalStop.latitude),
    longitude: Number(physicalStop.longitude),
    atcoCode: physicalStop.atcoCode || "",
    naptanCode: physicalStop.naptanCode || "",
    coordinateSource: "hybrid_timed_graph_physical_access_stop",
    isHybridSyntheticStop: true,
  };
}

function appendBodsHybridTimedGraphFallbackPath(timetable, syntheticAccessStop, candidate, context = {}) {
  const syntheticConnectionKeys = context.syntheticConnectionKeys || new Set();
  const departure = Number(candidate.syntheticDepartureMinutes);
  const arrival = Number(candidate.anchorArrivalMinutes);
  if (!Number.isFinite(departure) || !Number.isFinite(arrival) || arrival <= departure || !candidate?.anchorStop?.id) {
    return { syntheticConnectionsAdded: 0, syntheticStopsAdded: 0, startsAtSyntheticAccess: false };
  }
  const key = `${syntheticAccessStop.id}|${candidate.anchorStop.id}|${departure.toFixed(2)}|${candidate.connection.tripId || candidate.connection.routeId || "timed-graph-fallback"}`;
  if (syntheticConnectionKeys.has(key)) {
    return { syntheticConnectionsAdded: 0, syntheticStopsAdded: 0, startsAtSyntheticAccess: false };
  }
  syntheticConnectionKeys.add(key);
  timetable.connections.push({
    fromStopId: syntheticAccessStop.id,
    toStopId: candidate.anchorStop.id,
    departureMinutes: departure,
    arrivalMinutes: arrival,
    routeId: candidate.connection.routeId || `timed-graph-fallback-${context.syntheticSequence || 1}`,
    routeRef: candidate.connection.routeRef || candidate.connection.routeName || "Bus",
    routeName: candidate.connection.routeName || "BODS timed graph fallback",
    tripId: `hybrid-timed-graph-${candidate.connection.tripId || candidate.connection.routeId || context.syntheticSequence || 1}`,
    linkIndex: -1,
    estimatedRuntime: true,
    impliedSpeedKph: candidate.routeDistanceMetres > 0 && arrival > departure ? (candidate.routeDistanceMetres / 1000) / ((arrival - departure) / 60) : null,
    geometry: Array.isArray(candidate.geometry) && candidate.geometry.length >= 2 ? candidate.geometry : [
      { latitude: syntheticAccessStop.latitude, longitude: syntheticAccessStop.longitude },
      { latitude: candidate.anchorStop.latitude, longitude: candidate.anchorStop.longitude },
    ],
    geometrySource: "hybrid_timed_graph_fallback_geometry",
    timingSource: candidate.candidateType === "timed_graph_interpolated_span"
      ? "hybrid_naptan_bods_interpolated_between_timed_anchors"
      : candidate.candidateType === "timed_graph_scheduled_anchor_connector"
        ? "hybrid_naptan_bods_scheduled_anchor_connector"
        : "hybrid_naptan_bods_timed_anchor_direct_fallback",
    isHybridAccessConnectorOnly: candidate.isHybridAccessConnectorOnly === true,
    hybridAnchorStopId: candidate.anchorStop.id,
  });
  return { syntheticConnectionsAdded: 1, syntheticStopsAdded: 0, startsAtSyntheticAccess: true };
}

function projectCoordinateToLineSegmentFraction(point, start, end) {
  if (!isValidCoordinateObject(point) || !isValidCoordinateObject(start) || !isValidCoordinateObject(end)) {
    return null;
  }
  const metresPerDegreeLat = 111320;
  const meanLatitudeRadians = ((Number(start.latitude) + Number(end.latitude)) / 2) * Math.PI / 180;
  const metresPerDegreeLon = metresPerDegreeLat * Math.max(Math.cos(meanLatitudeRadians), 0.2);
  const px = (Number(point.longitude) - Number(start.longitude)) * metresPerDegreeLon;
  const py = (Number(point.latitude) - Number(start.latitude)) * metresPerDegreeLat;
  const ex = (Number(end.longitude) - Number(start.longitude)) * metresPerDegreeLon;
  const ey = (Number(end.latitude) - Number(start.latitude)) * metresPerDegreeLat;
  const lengthSquared = ex * ex + ey * ey;
  if (!Number.isFinite(lengthSquared) || lengthSquared <= 0) {
    return null;
  }
  const rawFraction = (px * ex + py * ey) / lengthSquared;
  const fraction = clamp(rawFraction, 0, 1);
  const projectedX = ex * fraction;
  const projectedY = ey * fraction;
  const distanceMetres = Math.hypot(px - projectedX, py - projectedY);
  return { fraction, distanceMetres };
}

function extractBodsHybridOsmAccessRoutesFromPayload(payload, eligiblePhysicalStops = []) {
  ...
  return relationElements
    .map((relation) => {
      const coordinates = buildRouteCoordinateSequenceFromRelation(relation, wayById);
      if (coordinates.length < 2) {
        return null;
      }
      const measures = buildBodsHybridRouteMeasures(coordinates);
      const routeStops = extractBodsHybridOsmRouteStopCandidates(relation, nodeById, coordinates, measures);
      const accessAnchors = eligiblePhysicalStops
        .map((physicalStop) => {
          if (!isValidCoordinateObject(physicalStop)) {
            return null;
          }
          const projection = projectCoordinateToBodsHybridRouteMeasure(physicalStop, coordinates, measures);
          const nearestRelationStopDistance = getNearestHybridRouteStopDistance(physicalStop, routeStops);
          const acceptableByRouteGeometry = projection && projection.distanceMetres <= BODS_HYBRID_ROUTE_STOP_MATCH_METRES;
          const acceptableByRelationStop = Number.isFinite(nearestRelationStopDistance) && nearestRelationStopDistance <= BODS_HYBRID_ROUTE_STOP_MATCH_METRES * 1.2;
          if (!acceptableByRouteGeometry && !acceptableByRelationStop) {
            return null;
          }
          return {
            physicalStop,
            routeMeasureMetres: projection.routeMeasureMetres,
            projection,
            walkingDistanceMetres: Number(physicalStop.walkingDistanceMetres),
            walkingTimeMinutes: Number(physicalStop.walkingTimeMinutes),
            seedArrivalMinutes: null,
            nearestRouteDistanceMetres: projection.distanceMetres,
            nearestRelationStopDistanceMetres: nearestRelationStopDistance,
          };
        })
        .filter(Boolean)
        .sort((a, b) => a.walkingTimeMinutes - b.walkingTimeMinutes || a.nearestRouteDistanceMetres - b.nearestRouteDistanceMetres)
      ...
      return {
        ...
        tags: relation.tags || {},
        ref: firstNonEmpty(relation.tags?.ref, relation.tags?.route_ref, relation.tags?.name),
        name: deriveOsmBusRouteName(relation.tags || {}),
        coordinates,
        measures,
        lengthMetres: measures[measures.length - 1] || 0,
        routeStops,
        accessAnchors,
      };
    })
    .filter(Boolean)
    .sort((a, b) => Math.min(...a.accessAnchors.map((anchor) => anchor.walkingTimeMinutes)) - Math.min(...b.accessAnchors.map((anchor) => anchor.walkingTimeMinutes)))
    .slice(0, BUS_ROUTE_MAX_RELATIONS);
}

function extractBodsHybridOsmRouteStopCandidates(relation, nodeById, routeCoordinates, routeMeasures) {
  const candidates = [];
  (relation.members || []).forEach((member) => {
    ...
    const coordinate = { latitude: Number(node.lat), longitude: Number(node.lon) };
    if (!isValidCoordinateObject(coordinate)) {
      return;
    }
    const projection = projectCoordinateToBodsHybridRouteMeasure(coordinate, routeCoordinates, routeMeasures);
    if (!projection || projection.distanceMetres > BODS_HYBRID_ROUTE_STOP_MATCH_METRES * 1.5) {
      return;
    }
    const tags = node.tags || {};
    candidates.push({
      id: Number(node.id),
      name: tags.name || tags["naptan:CommonName"] || tags.ref || "Route stop",
      latitude: coordinate.latitude,
      longitude: coordinate.longitude,
      atcoCode: tags["naptan:AtcoCode"] || tags["ref:GB:naptan"] || "",
      naptanCode: tags["naptan:NaptanCode"] || "",
      routeMeasureMetres: projection.routeMeasureMetres,
      projectionDistanceMetres: projection.distanceMetres,
      source: "osm_route_relation_stop",
    });
  });
  return candidates
    .sort((a, b) => a.routeMeasureMetres - b.routeMeasureMetres)
    .filter((candidate, index, array) => index === 0 || Math.abs(candidate.routeMeasureMetres - array[index - 1].routeMeasureMetres) > 25);
}

function projectBodsTimetableStopsOntoHybridRoute(timetable, route, departuresByStopId = new Map()) {
  return (timetable?.stops || [])
    .filter((stop) => stop?.id && !String(stop.id).startsWith(BODS_HYBRID_SYNTHETIC_STOP_PREFIX) && isValidCoordinateObject(stop))
    .map((stop) => {
      const projection = projectCoordinateToBodsHybridRouteMeasure(stop, route.coordinates, route.measures);
      if (!projection || projection.distanceMetres > BODS_HYBRID_BODS_ANCHOR_MATCH_METRES) {
        return null;
      }
      return {
        stop,
        projection,
        routeMeasureMetres: projection.routeMeasureMetres,
        routeDistanceFromAccessMinMetres: Math.min(...route.accessAnchors.map((anchor) => Math.abs(anchor.routeMeasureMetres - projection.routeMeasureMetres))),
        departuresAfterSelectedTime: departuresByStopId.get(stop.id)?.length || 0,
      };
    })
    .filter(Boolean)
    .filter((entry) => entry.routeDistanceFromAccessMinMetres <= BODS_HYBRID_ROUTE_EXPANSION_METRES)
    .sort((a, b) => a.routeDistanceFromAccessMinMetres - b.routeDistanceFromAccessMinMetres);
}

function buildBodsHybridAnchorCandidatesForAccess(route, accessAnchor, departuresByStopId, departureMinutes, maximumBandMinutes) {
  const seedArrivalMinutes = departureMinutes + accessAnchor.walkingTimeMinutes;
  accessAnchor.seedArrivalMinutes = seedArrivalMinutes;
  const candidates = [];
  (route.projectedBodsStops || []).forEach((projectedStop) => {
    const routeDistanceMetres = Math.abs(projectedStop.routeMeasureMetres - accessAnchor.routeMeasureMetres);
    if (!Number.isFinite(routeDistanceMetres) || routeDistanceMetres > BODS_HYBRID_ROUTE_EXPANSION_METRES) {
      return;
    }
    const departures = departuresByStopId.get(projectedStop.stop.id) || [];
    for (const connection of departures) {
      if (!doesBodsHybridRouteMatchConnection(route, connection)) {
        continue;
      }
      if (!isBodsHybridConnectionDirectionConsistent(route, accessAnchor, projectedStop, connection)) {
        continue;
      }
      const travelMinutes = estimateBodsHybridAccessToAnchorMinutes(route, accessAnchor, projectedStop, connection, routeDistanceMetres);
      if (!Number.isFinite(travelMinutes) || travelMinutes < 0) {
        continue;
      }
      const anchorArrivalMinutes = Number(connection.departureMinutes) - BODS_HYBRID_TIMING_EPSILON_MINUTES;
      const syntheticDepartureMinutes = anchorArrivalMinutes - travelMinutes;
      if (syntheticDepartureMinutes + 0.05 < seedArrivalMinutes) {
        continue;
      }
      if (anchorArrivalMinutes - departureMinutes > maximumBandMinutes + BODS_LOOKAHEAD_MINUTES) {
        continue;
      }
      candidates.push({
        stop: projectedStop.stop,
        projectedStop,
        connection,
        routeDistanceMetres,
        travelMinutes,
        syntheticDepartureMinutes,
        anchorArrivalMinutes,
        speedKph: routeDistanceMetres > 0 && travelMinutes > 0 ? (routeDistanceMetres / 1000) / (travelMinutes / 60) : null,
      });
      break;
    }
  });
  return candidates
    .sort((a, b) => a.syntheticDepartureMinutes - b.syntheticDepartureMinutes || a.routeDistanceMetres - b.routeDistanceMetres)
    .slice(0, BODS_HYBRID_ANCHOR_CAP_PER_ACCESS_ROUTE);
}

function mergeBodsHybridAnchorCandidates(candidates = []) {
  const candidateByKey = new Map();
  candidates
    .filter((candidate) => candidate?.stop?.id && candidate?.projectedStop && Number.isFinite(Number(candidate.syntheticDepartureMinutes)))
    .forEach((candidate) => {
      const key = [
        candidate.stop.id,
        candidate.connection?.tripId || candidate.connection?.routeId || "",
        Math.round(Number(candidate.syntheticDepartureMinutes) * 10),
        Math.round(Number(candidate.anchorArrivalMinutes) * 10),
      ].join("|");
      const existing = candidateByKey.get(key);
      if (!existing || Number(candidate.routeDistanceMetres) < Number(existing.routeDistanceMetres)) {
        candidateByKey.set(key, candidate);
      }
    });
  return Array.from(candidateByKey.values())
    .sort((a, b) => Number(a.syntheticDepartureMinutes) - Number(b.syntheticDepartureMinutes) || Number(a.routeDistanceMetres) - Number(b.routeDistanceMetres))
    .slice(0, BODS_HYBRID_ANCHOR_CAP_PER_ACCESS_ROUTE);
}

function buildBodsHybridConnectionSpanCandidatesForAccess(route, accessAnchor, normalisedConnections = [], departureMinutes, maximumBandMinutes) {
  const seedArrivalMinutes = departureMinutes + accessAnchor.walkingTimeMinutes;
  accessAnchor.seedArrivalMinutes = seedArrivalMinutes;
  const accessMeasure = Number(accessAnchor.routeMeasureMetres);
  if (!Number.isFinite(accessMeasure) || !(route?.projectedBodsStopById instanceof Map)) {
    return [];
  }

  const candidates = [];
  normalisedConnections.forEach((connection) => {
    if (!connection?.fromStopId || !connection?.toStopId) {
      return;
    }
    if (!doesBodsHybridRouteMatchConnection(route, connection)) {
      return;
    }
    const fromProjection = route.projectedBodsStopById.get(connection.fromStopId);
    const toProjection = route.projectedBodsStopById.get(connection.toStopId);
    if (!fromProjection || !toProjection) {
      return;
    }
    const fromMeasure = Number(fromProjection.routeMeasureMetres);
    const toMeasure = Number(toProjection.routeMeasureMetres);
    const connectionDeparture = Number(connection.departureMinutes);
    const connectionArrival = Number(connection.arrivalMinutes);
    const segmentRuntime = connectionArrival - connectionDeparture;
    const segmentDelta = toMeasure - fromMeasure;
    const segmentDistanceMetres = Math.abs(segmentDelta);
    if (
      !Number.isFinite(fromMeasure) ||
      !Number.isFinite(toMeasure) ||
      !Number.isFinite(connectionDeparture) ||
      !Number.isFinite(connectionArrival) ||
      !Number.isFinite(segmentRuntime) ||
      segmentRuntime <= 0 ||
      segmentDistanceMetres < 60 ||
      segmentDistanceMetres > BODS_HYBRID_ROUTE_EXPANSION_METRES * 1.35
    ) {
      return;
    }

    const direction = Math.sign(segmentDelta);
    if (direction === 0) {
      return;
    }

    let projectedStop = null;
    let routeDistanceMetres = null;
    let syntheticDepartureMinutes = null;
    let anchorArrivalMinutes = null;
    let candidateType = "";

    const segmentSpeedKph = clamp(
      (segmentDistanceMetres / 1000) / (segmentRuntime / 60),
      BODS_HYBRID_MIN_INTERPOLATED_SPEED_KPH,
      BODS_HYBRID_MAX_INTERPOLATED_SPEED_KPH
    );

    const isBeforeFromInDirection = direction > 0
      ? accessMeasure <= fromMeasure
      : accessMeasure >= fromMeasure;
    const isBetweenTimedAnchors = direction > 0
      ? accessMeasure > fromMeasure && accessMeasure < toMeasure
      : accessMeasure < fromMeasure && accessMeasure > toMeasure;

    if (isBeforeFromInDirection) {
      routeDistanceMetres = Math.abs(fromMeasure - accessMeasure);
      if (routeDistanceMetres > BODS_HYBRID_ROUTE_EXPANSION_METRES) {
        return;
      }
      const travelToFromMinutes = (routeDistanceMetres / 1000) / segmentSpeedKph * 60;
      projectedStop = fromProjection;
      syntheticDepartureMinutes = connectionDeparture - travelToFromMinutes;
      anchorArrivalMinutes = connectionDeparture - BODS_HYBRID_TIMING_EPSILON_MINUTES;
      candidateType = "hybrid_access_before_bods_timed_anchor";
    } else if (isBetweenTimedAnchors) {
      const fractionFrom = Math.abs(accessMeasure - fromMeasure) / segmentDistanceMetres;
      routeDistanceMetres = Math.abs(toMeasure - accessMeasure);
      if (routeDistanceMetres > BODS_HYBRID_ROUTE_EXPANSION_METRES) {
        return;
      }
      projectedStop = toProjection;
      syntheticDepartureMinutes = connectionDeparture + segmentRuntime * fractionFrom;
      anchorArrivalMinutes = connectionArrival;
      candidateType = "hybrid_access_between_bods_timed_anchors";
    } else {
      return;
    }

    if (
      !projectedStop?.stop?.id ||
      !Number.isFinite(routeDistanceMetres) ||
      !Number.isFinite(syntheticDepartureMinutes) ||
      !Number.isFinite(anchorArrivalMinutes) ||
      anchorArrivalMinutes <= syntheticDepartureMinutes ||
      syntheticDepartureMinutes + 0.05 < seedArrivalMinutes ||
      syntheticDepartureMinutes - departureMinutes > maximumBandMinutes + BODS_LOOKAHEAD_MINUTES ||
    ...
    const travelMinutes = anchorArrivalMinutes - syntheticDepartureMinutes;
    candidates.push({
      stop: projectedStop.stop,
      projectedStop,
      connection,
      ...
      syntheticDepartureMinutes,
      anchorArrivalMinutes,
      speedKph: routeDistanceMetres > 0 && travelMinutes > 0 ? (routeDistanceMetres / 1000) / (travelMinutes / 60) : segmentSpeedKph,
      candidateType,
    });
  });
...
function doesBodsHybridRouteMatchConnection(route, connection = {}) {
  const routeTokens = getBodsHybridRouteTokens(route?.tags || route || {});
  const connectionTokens = getBodsHybridConnectionTokens(connection);
  if (routeTokens.size === 0 || connectionTokens.size === 0) {
    return true;
  }
  const hasSharedToken = Array.from(routeTokens).some((token) => connectionTokens.has(token));
  return hasSharedToken || !BODS_HYBRID_ROUTE_REF_MATCH_REQUIRED_WHEN_AVAILABLE;
}

function getBodsHybridRouteTokens(tags = {}) {
  return normaliseBodsHybridRouteTokenSet([
    tags.ref,
    tags.route_ref,
    tags["ref:route"],
    tags.name,
...
function getBodsHybridConnectionTokens(connection = {}) {
  return normaliseBodsHybridRouteTokenSet([
    connection.routeRef,
...
function normaliseBodsHybridRouteTokenSet(values = []) {
  const genericTokens = new Set(["bus", "route", "service", "services", "line", "the", "and", "for", "from", "to", "via", "towards", "city", "centre", "center", "first", "arriva", "transdev", "stagecoach"]);
  ...
  values.forEach((value) => {
    String(value || "")
      ...
      .filter((token) => token.length > 0 && token.length <= 12 && !genericTokens.has(token))
      .forEach((token) => {
        tokens.add(token);
        const numeric = token.replace(/[^0-9]/g, "");
        if (numeric && numeric.length <= 4) {
          tokens.add(numeric);
        }
      });
  });
...
function isBodsHybridConnectionDirectionConsistent(route, accessAnchor, projectedAnchor, connection = {}) {
  const toProjection = route.projectedBodsStopById?.get(connection.toStopId);
  if (!toProjection) {
    return true;
  }
  const onwardDelta = toProjection.routeMeasureMetres - projectedAnchor.routeMeasureMetres;
  if (Math.abs(onwardDelta) < 50) {
    return true;
  }
  const accessToAnchorDelta = projectedAnchor.routeMeasureMetres - accessAnchor.routeMeasureMetres;
  if (Math.abs(accessToAnchorDelta) < BODS_HYBRID_ROUTE_STOP_MATCH_METRES) {
    return true;
  }
  return Math.sign(onwardDelta) === Math.sign(accessToAnchorDelta);
}

function estimateBodsHybridAccessToAnchorMinutes(route, accessAnchor, projectedAnchor, connection = {}, routeDistanceMetres = null) {
  const distanceMetres = Number.isFinite(Number(routeDistanceMetres))
    ? Number(routeDistanceMetres)
    : Math.abs(Number(projectedAnchor.routeMeasureMetres) - Number(accessAnchor.routeMeasureMetres));
  if (!Number.isFinite(distanceMetres) || distanceMetres < 0) {
    return null;
  }
  if (distanceMetres < 35) {
    return 0.05;
  }
  let speedKph = null;
  const onwardProjection = route.projectedBodsStopById?.get(connection.toStopId);
  const runtime = Number(connection.arrivalMinutes) - Number(connection.departureMinutes);
  if (onwardProjection && Number.isFinite(runtime) && runtime > 0) {
    const onwardDistance = Math.abs(onwardProjection.routeMeasureMetres - projectedAnchor.routeMeasureMetres);
    if (Number.isFinite(onwardDistance) && onwardDistance > 150) {
      speedKph = (onwardDistance / 1000) / (runtime / 60);
    }
  }
  if (!Number.isFinite(speedKph) && Number.isFinite(Number(connection.impliedSpeedKph))) {
    speedKph = Number(connection.impliedSpeedKph);
  }
  if (!Number.isFinite(speedKph)) {
    speedKph = BODS_HYBRID_FALLBACK_SPEED_KPH;
  }
  speedKph = clamp(speedKph, BODS_HYBRID_MIN_INTERPOLATED_SPEED_KPH, BODS_HYBRID_MAX_INTERPOLATED_SPEED_KPH);
  return (distanceMetres / 1000) / speedKph * 60;
}

function buildBodsHybridSyntheticAccessStop(accessAnchor, route, routeIndex, accessIndex) {
  const physicalStop = accessAnchor.physicalStop;
  const rawId = physicalStop.id || physicalStop.atcoCode || physicalStop.naptanCode || `${routeIndex}-${accessIndex}`;
  const stopKey = normaliseBodsStopIdentifier(rawId) || `${routeIndex}-${accessIndex}`;
  return {
    id: `${BODS_HYBRID_SYNTHETIC_STOP_PREFIX}access-${route.id}-${stopKey}`,
    name: physicalStop.name || `Hybrid access stop ${accessIndex + 1}`,
    latitude: Number(physicalStop.latitude),
    longitude: Number(physicalStop.longitude),
    atcoCode: physicalStop.atcoCode || "",
    naptanCode: physicalStop.naptanCode || "",
    coordinateSource: "hybrid_naptan_osm_access_stop",
    isHybridSyntheticStop: true,
    osmRouteId: route.id,
    osmRouteName: route.name || "",
  };
}

function appendBodsHybridSyntheticInterpolatedPath(timetable, route, accessAnchor, syntheticAccessStop, anchorCandidate, context = {}) {
  const syntheticStopIds = context.syntheticStopIds || new Set();
  const syntheticConnectionKeys = context.syntheticConnectionKeys || new Set();
  const startMeasure = accessAnchor.routeMeasureMetres;
  const endMeasure = anchorCandidate.projectedStop.routeMeasureMetres;
  const runtime = Math.max(BODS_HYBRID_TIMING_EPSILON_MINUTES, anchorCandidate.anchorArrivalMinutes - anchorCandidate.syntheticDepartureMinutes);
  const pathCoordinates = sliceBodsHybridRouteCoordinatesBetweenMeasures(route.coordinates, route.measures, startMeasure, endMeasure);
  const intermediateStops = buildBodsHybridIntermediateRouteStops(route, startMeasure, endMeasure, pathCoordinates, context.syntheticSequence || 1);
  const nodes = [
    {
      stop: syntheticAccessStop,
      routeMeasureMetres: startMeasure,
      fraction: 0,
      coordinate: { latitude: syntheticAccessStop.latitude, longitude: syntheticAccessStop.longitude },
    },
    ...intermediateStops,
    {
      stop: anchorCandidate.stop,
      routeMeasureMetres: endMeasure,
      fraction: 1,
      coordinate: { latitude: anchorCandidate.stop.latitude, longitude: anchorCandidate.stop.longitude },
    },
  ]
    .filter((node) => node?.stop?.id && isValidCoordinateObject(node.coordinate || node.stop))
    .sort((a, b) => (startMeasure <= endMeasure ? a.routeMeasureMetres - b.routeMeasureMetres : b.routeMeasureMetres - a.routeMeasureMetres));

  let syntheticStopsAdded = 0;
  let syntheticConnectionsAdded = 0;
  let startsAtSyntheticAccess = false;
  intermediateStops.forEach((node) => {
    ensureBodsHybridTimetableStop(timetable, node.stop, syntheticStopIds);
    syntheticStopsAdded += 1;
  });

  for (let index = 1; index < nodes.length; index += 1) {
    const fromNode = nodes[index - 1];
    const toNode = nodes[index];
    const departure = anchorCandidate.syntheticDepartureMinutes + runtime * fromNode.fraction;
    const arrival = anchorCandidate.syntheticDepartureMinutes + runtime * toNode.fraction;
    if (!Number.isFinite(departure) || !Number.isFinite(arrival) || arrival <= departure) {
      continue;
    }
    const key = `${fromNode.stop.id}|${toNode.stop.id}|${departure.toFixed(2)}|${anchorCandidate.connection.tripId || anchorCandidate.connection.routeId || route.id}`;
    if (syntheticConnectionKeys.has(key)) {
      continue;
    }
    syntheticConnectionKeys.add(key);
    const segmentGeometry = sliceBodsHybridRouteCoordinatesBetweenMeasures(route.coordinates, route.measures, fromNode.routeMeasureMetres, toNode.routeMeasureMetres);
    const segmentDistance = Math.abs(toNode.routeMeasureMetres - fromNode.routeMeasureMetres);
    timetable.connections.push({
      fromStopId: fromNode.stop.id,
      toStopId: toNode.stop.id,
      departureMinutes: departure,
      arrivalMinutes: arrival,
      routeId: anchorCandidate.connection.routeId || String(route.id),
      routeRef: anchorCandidate.connection.routeRef || route.ref || route.name || "Bus",
      routeName: anchorCandidate.connection.routeName || route.name || "Hybrid BODS bus service",
      tripId: `hybrid-access-${anchorCandidate.connection.tripId || anchorCandidate.connection.routeId || route.id}-${context.syntheticSequence || 1}-${index}`,
      linkIndex: -1,
      estimatedRuntime: true,
      impliedSpeedKph: segmentDistance > 0 && arrival > departure ? (segmentDistance / 1000) / ((arrival - departure) / 60) : anchorCandidate.speedKph,
      geometry: segmentGeometry.length >= 2 ? segmentGeometry : [fromNode.coordinate, toNode.coordinate],
      geometrySource: "hybrid_osm_route_geometry",
      timingSource: "hybrid_naptan_osm_bods_interpolated_access_to_timed_anchor",
      hybridAccessRouteId: route.id,
      hybridAnchorStopId: anchorCandidate.stop.id,
    });
    syntheticConnectionsAdded += 1;
    if (fromNode.stop.id === syntheticAccessStop.id) {
      startsAtSyntheticAccess = true;
    }
  }

  return { syntheticStopsAdded, syntheticConnectionsAdded, startsAtSyntheticAccess };
}

function buildBodsHybridIntermediateRouteStops(route, startMeasure, endMeasure, pathCoordinates, syntheticSequence) {
  const minMeasure = Math.min(startMeasure, endMeasure);
  const maxMeasure = Math.max(startMeasure, endMeasure);
  const directionForward = endMeasure >= startMeasure;
  let candidates = (route.routeStops || [])
    .filter((stop) => stop.routeMeasureMetres > minMeasure + 40 && stop.routeMeasureMetres < maxMeasure - 40)
    .sort((a, b) => directionForward ? a.routeMeasureMetres - b.routeMeasureMetres : b.routeMeasureMetres - a.routeMeasureMetres)
    .slice(0, BODS_HYBRID_MAX_INTERMEDIATE_STOPS_PER_LINK)
    .map((stop, index) => ({
      stop: {
        id: `${BODS_HYBRID_SYNTHETIC_STOP_PREFIX}osm-stop-${route.id}-${normaliseBodsStopIdentifier(stop.id || stop.name || index)}-${syntheticSequence}-${index}`,
        name: stop.name || "Interpolated NaPTAN/OSM route stop",
        latitude: stop.latitude,
        longitude: stop.longitude,
        atcoCode: stop.atcoCode || "",
        naptanCode: stop.naptanCode || "",
        coordinateSource: "hybrid_osm_route_stop_intermediate",
        isHybridSyntheticStop: true,
        osmRouteId: route.id,
      },
      routeMeasureMetres: stop.routeMeasureMetres,
      fraction: Math.abs(stop.routeMeasureMetres - startMeasure) / Math.max(1, Math.abs(endMeasure - startMeasure)),
      coordinate: { latitude: stop.latitude, longitude: stop.longitude },
    }));

  if (candidates.length === 0 && Array.isArray(pathCoordinates) && pathCoordinates.length >= 2) {
    const samples = sampleCoordinatePath(pathCoordinates, BODS_HYBRID_INTERMEDIATE_STOP_SPACING_METRES)
      .filter((sample) => sample.fraction > 0.05 && sample.fraction < 0.95)
      .slice(0, Math.min(10, BODS_HYBRID_MAX_INTERMEDIATE_STOPS_PER_LINK));
    candidates = samples.map((sample, index) => ({
      stop: {
        id: `${BODS_HYBRID_SYNTHETIC_STOP_PREFIX}route-sample-${route.id}-${syntheticSequence}-${index}`,
        name: "Interpolated route timing point",
        latitude: sample.coordinate.latitude,
        longitude: sample.coordinate.longitude,
        atcoCode: "",
        naptanCode: "",
        coordinateSource: "hybrid_osm_route_sample_intermediate",
        isHybridSyntheticStop: true,
        osmRouteId: route.id,
      },
      routeMeasureMetres: startMeasure + (endMeasure - startMeasure) * sample.fraction,
      fraction: sample.fraction,
      coordinate: sample.coordinate,
    }));
  }
  return candidates.filter((candidate) => candidate.fraction > 0 && candidate.fraction < 1);
}

function ensureBodsHybridTimetableStop(timetable, stop, syntheticStopIds = new Set()) {
  if (!stop?.id || !isValidCoordinateObject(stop)) {
    return false;
  }
  if (!(timetable.stopsById instanceof Map)) {
    timetable.stopsById = new Map((timetable.stops || []).map((candidate) => [candidate.id, candidate]));
  }
  if (timetable.stopsById.has(stop.id)) {
    return false;
  }
  timetable.stopsById.set(stop.id, stop);
  timetable.stops.push(stop);
  syntheticStopIds.add(stop.id);
  return true;
}

function enrichBodsConnectionsWithHybridOsmRouteGeometry(timetable, route, projectedBodsStops, normalisedConnections = []) {
  const projectionByStopId = new Map(projectedBodsStops.map((entry) => [entry.stop.id, entry]));
  (timetable.connections || []).forEach((connection) => {
    if (Array.isArray(connection.geometry) && connection.geometry.length >= 2 && connection.geometrySource !== "stop_to_stop") {
      return;
    }
    if (!doesBodsHybridRouteMatchConnection(route, connection)) {
      return;
    }
    const fromProjection = projectionByStopId.get(connection.fromStopId);
    const toProjection = projectionByStopId.get(connection.toStopId);
    if (!fromProjection || !toProjection) {
      return;
    }
    const routeDistance = Math.abs(toProjection.routeMeasureMetres - fromProjection.routeMeasureMetres);
    if (!Number.isFinite(routeDistance) || routeDistance <= 25 || routeDistance > BODS_HYBRID_ROUTE_EXPANSION_METRES) {
      return;
    }
    const geometry = sliceBodsHybridRouteCoordinatesBetweenMeasures(route.coordinates, route.measures, fromProjection.routeMeasureMetres, toProjection.routeMeasureMetres);
    if (geometry.length < 2) {
      return;
    }
    connection.geometry = geometry;
    connection.geometrySource = "hybrid_osm_route_geometry_between_bods_anchors";
    connection.hybridOsmRouteId = route.id;
  });
}

function buildBodsHybridRouteMeasures(coordinates = []) {
  const measures = [0];
  for (let index = 1; index < coordinates.length; index += 1) {
    const previous = coordinates[index - 1];
    const current = coordinates[index];
    const distance = getDistanceMetres(previous.latitude, previous.longitude, current.latitude, current.longitude);
    measures[index] = measures[index - 1] + (Number.isFinite(distance) ? distance : 0);
  }
  return measures;
}

function projectCoordinateToBodsHybridRouteMeasure(coordinate, routeCoordinates = [], routeMeasures = []) {
  if (!isValidCoordinateObject(coordinate) || routeCoordinates.length < 2) {
    return null;
  }
  const referenceLatitude = coordinate.latitude * Math.PI / 180;
  const metresPerDegreeLat = 111320;
  const metresPerDegreeLon = 111320 * Math.max(Math.cos(referenceLatitude), 0.2);
  const toXY = (point) => ({
    x: (point.longitude - coordinate.longitude) * metresPerDegreeLon,
    y: (point.latitude - coordinate.latitude) * metresPerDegreeLat,
  });
  let best = null;
  for (let index = 1; index < routeCoordinates.length; index += 1) {
    const a = routeCoordinates[index - 1];
    const b = routeCoordinates[index];
    if (!isValidCoordinateObject(a) || !isValidCoordinateObject(b)) {
      continue;
    }
    const ax = toXY(a).x;
    const ay = toXY(a).y;
    ...
    const t = lengthSq > 0 ? clamp((0 - ax) * vx / lengthSq + (0 - ay) * vy / lengthSq, 0, 1) : 0;
    const px = ax + vx * t;
    const py = ay + vy * t;
    const distanceMetres = Math.sqrt(px * px + py * py);
    const segmentLength = Math.max(0, (routeMeasures[index] || 0) - (routeMeasures[index - 1] || 0));
    const routeMeasureMetres = (routeMeasures[index - 1] || 0) + segmentLength * t;
    const projectedCoordinate = interpolateCoordinate(a, b, t);
    if (!best || distanceMetres < best.distanceMetres) {
      best = { distanceMetres, routeMeasureMetres, coordinate: projectedCoordinate, segmentIndex: index - 1, fraction: t };
    }
  }
  return best;
}
...
function getCoordinateAtBodsHybridRouteMeasure(routeCoordinates, routeMeasures, measureMetres) {
  if (!Array.isArray(routeCoordinates) || routeCoordinates.length === 0) {
    return null;
  }
  const target = clamp(Number(measureMetres) || 0, 0, routeMeasures[routeMeasures.length - 1] || 0);
  for (let index = 1; index < routeMeasures.length; index += 1) {
    if (routeMeasures[index] >= target) {
      const previousMeasure = routeMeasures[index - 1] || 0;
      const segmentLength = Math.max(1, routeMeasures[index] - previousMeasure);
      return interpolateCoordinate(routeCoordinates[index - 1], routeCoordinates[index], (target - previousMeasure) / segmentLength);
    }
  }
  return routeCoordinates[routeCoordinates.length - 1];
}

function sliceBodsHybridRouteCoordinatesBetweenMeasures(routeCoordinates = [], routeMeasures = [], fromMeasure, toMeasure) {
  if (routeCoordinates.length < 2 || routeMeasures.length !== routeCoordinates.length) {
    return [];
  }
  const start = clamp(Number(fromMeasure) || 0, 0, routeMeasures[routeMeasures.length - 1] || 0);
  const end = clamp(Number(toMeasure) || 0, 0, routeMeasures[routeMeasures.length - 1] || 0);
  const minMeasure = Math.min(start, end);
  const maxMeasure = Math.max(start, end);
  const coordinates = [];
  const startCoordinate = getCoordinateAtBodsHybridRouteMeasure(routeCoordinates, routeMeasures, minMeasure);
  if (startCoordinate) {
    coordinates.push(startCoordinate);
  }
  routeCoordinates.forEach((coordinate, index) => {
    const measure = routeMeasures[index];
    if (measure > minMeasure && measure < maxMeasure) {
      coordinates.push({ latitude: coordinate.latitude, longitude: coordinate.longitude });
    }
  });
  const endCoordinate = getCoordinateAtBodsHybridRouteMeasure(routeCoordinates, routeMeasures, maxMeasure);
  if (endCoordinate) {
    coordinates.push(endCoordinate);
  }
  const deduped = dedupeAdjacentCoordinates(coordinates, 6);
  return start <= end ? deduped : deduped.reverse();
}

function getNearestHybridRouteStopDistance(coordinate, routeStops = []) {
  if (!isValidCoordinateObject(coordinate) || routeStops.length === 0) {
    return Infinity;
  }
  let nearest = Infinity;
  routeStops.forEach((stop) => {
    const distance = getDistanceMetres(coordinate.latitude, coordinate.longitude, stop.latitude, stop.longitude);
...
function buildBodsBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes) {
  const physicalStops = Array.isArray(nearbyOriginStops) ? nearbyOriginStops.filter(Boolean) : [];
  const timetableStops = timetable?.stops || [];
  const eligiblePhysicalStops = physicalStops
    .filter((stop) => Number.isFinite(Number(stop.distanceMetres)) && Number(stop.distanceMetres) <= maximumWalkToBusStopMetres)
    .map((stop) => ({
      ...stop,
      walkingDistanceMetres: Math.round(Number(stop.distanceMetres) || 0),
      walkingTimeMinutes: estimateBusWalkTimeMinutes(Number(stop.distanceMetres) || 0),
    }));
  const boardingStopsById = new Map();
  const departuresByStopId = buildBodsDeparturesByStopId((timetable?.connections || []).map((connection) =>
    normaliseBodsConnectionForDepartureWindow(connection, departureMinutes)
  ...
  const addBoardingStopCandidate = (matchedStop, physicalStop, linkContext = {}) => {
    ...
    const accessLinkDistanceMetres = Number.isFinite(Number(linkContext.accessLinkDistanceMetres))
      ...
      : 0;
    const accessLinkCountsAsWalk = linkContext.countAccessLinkAsWalk === true;
    const physicalWalkMinutes = estimateBusWalkTimeMinutes(physicalDistanceMetres);
    const accessLinkWalkMinutes = accessLinkCountsAsWalk && accessLinkDistanceMetres > 0
      ? estimateBusWalkTimeMinutes(accessLinkDistanceMetres)
      : 0;
    if (!Number.isFinite(physicalWalkMinutes) || !Number.isFinite(accessLinkWalkMinutes)) {
    ...
    const existing = boardingStopsById.get(matchedStop.id);
    const departuresAfterSelectedTime = departuresByStopId.get(matchedStop.id)?.length || 0;
    const modelledWalkingDistanceMetres = physicalDistanceMetres + (accessLinkCountsAsWalk ? accessLinkDistanceMetres : 0);
    const totalWalkingTimeMinutes = physicalWalkMinutes + accessLinkWalkMinutes;
    const candidate = {
      id: matchedStop.id,
    ...
    if (!existing || candidate.walkingTimeMinutes < existing.walkingTimeMinutes) {
      boardingStopsById.set(matchedStop.id, candidate);
    }
  };
  ...
  eligiblePhysicalStops.forEach((physicalStop) => {
    ...
    // Do not seed boarding from geographically nearby BODS nodes. The selected walk threshold
    // applies to origin -> physical NaPTAN/OSM stop only. The BODS graph must then be reached
    // through the stop reference/name/graph relationship to that physical stop, not by finding
    // a BODS stop that happens to be near the site.

    // NaPTAN-gated timetable access: once the origin can reach a real NaPTAN/physical
    // stop within the user-selected walk threshold, do not also require the BODS
    // timetable boarding node to be geographically within that walk threshold. BODS
    // StopPointRefs and parsed coordinates can be incomplete, offset or split across
    // timing points. Use the walk-accessible NaPTAN stop as the access gate, then seed
    // the timetable graph at code/name matches, graph-adjacent stops, or finally the
    // nearest scheduled timetable stops in the local parsed graph. The data link is not
    // counted as walking time; only origin -> physical NaPTAN stop is counted as access walk.
    findBodsTimetableStopsLinkedFromNaptanAccessStop(
      timetable,
      physicalStop,
      directMatches,
  ...
  // No direct BODS-near-origin fallback here. A BODS node near the site is not enough for
  // the intended model; a real physical NaPTAN/OSM stop must be walk-accessible and must then
  // cross-reference into the parsed timetable graph.
  ...
  const boardingStops = Array.from(boardingStopsById.values())
    ...
    .sort((a, b) => a.walkingTimeMinutes - b.walkingTimeMinutes);
  // Seed the timetable search from every validated walk-accessible physical/BODS-linked stop.
  // Do not require that seed node itself to expose an outgoing BODS departure: in TransXChange
  // the physical boarding point may be represented as an arrival-only timing point, a split node,
  // or a NaPTAN/OSM access stop which then needs graph/connector relaxation to reach the first
  ...
  const seedStopsWithDirectDepartures = seedStops.filter((stop) => stop.departuresAfterSelectedTime > 0);
  const directMatchedBoardingStopCount = boardingStops.filter((stop) => ["direct_stop_reference_match", "naptan_code_or_name_timetable_stop"].includes(stop.boardingMatchType)).length;
  const accessLinkedBoardingStopCount = boardingStops.filter((stop) => ["naptan_graph_adjacent_timetable_stop"].includes(stop.boardingMatchType)).length;
...
function findBodsTimetableStopsLinkedFromNaptanAccessStop(timetable, physicalStop = {}, directMatches = [], departuresByStopId = new Map(), departureMinutes = 0, excludedStopIds = new Set()) {
  const timetableStops = timetable?.stops || [];
  const stopsById = timetable?.stopsById instanceof Map
    ? timetable.stopsById
    : new Map(timetableStops.map((stop) => [stop.id, stop]));
  const excludedIds = excludedStopIds instanceof Set ? excludedStopIds : new Set(excludedStopIds || []);
  const normalisedExcludedIds = new Set(Array.from(excludedIds).map(normaliseBodsStopIdentifier).filter(Boolean));
  const candidatesById = new Map();
  const addCandidate = (stop, matchType, graphHops = null) => {
    if (!stop?.id || !isValidCoordinateObject(stop)) {
      return;
    }
    const normalisedId = normaliseBodsStopIdentifier(stop.id);
    if (normalisedExcludedIds.has(normalisedId)) {
      return;
    }
    const departuresAfterSelectedTime = departuresByStopId.get(stop.id)?.length || 0;
    if (departuresAfterSelectedTime <= 0) {
      return;
    }
    const distanceMetres = isValidCoordinateObject(physicalStop)
      ? getDistanceMetres(physicalStop.latitude, physicalStop.longitude, stop.latitude, stop.longitude)
      : 0;
    const existing = candidatesById.get(stop.id);
    const candidate = { stop, matchType, graphHops, distanceMetres };
    if (!existing || rankBodsNaptanGraphCandidate(candidate) < rankBodsNaptanGraphCandidate(existing)) {
      candidatesById.set(stop.id, candidate);
    }
  };

  // Exact ATCO/NaPTAN or conservative name matches remain the preferred link. These are
  // not distance-limited because they represent the same boarding reference, not a walk leg.
  (directMatches || []).forEach((stop) => {
    addCandidate(stop, "naptan_code_or_name_timetable_stop");
  });

  // If the exact matched timetable node has no outgoing post-time departures, walk the parsed
  // timetable graph a few hops to find the first usable scheduled boarding nodes. This handles
  // timing-point splits and one-way stop representations in BODS/TransXChange.
  findBodsTimetableStopsByGraphAdjacency(timetable, directMatches, departuresByStopId).forEach((linked) => {
    addCandidate(linked.stop, "naptan_graph_adjacent_timetable_stop", linked.graphHops);
  });

  // Do not fall back to arbitrary scheduled BODS nodes. If the walk-accessible physical
  // NaPTAN/OSM stop cannot be resolved to an exact/name match or a graph-adjacent scheduled
  // node, the timetable graph is not safely boardable from that physical stop.

  return Array.from(candidatesById.values())
    .sort((a, b) => rankBodsNaptanGraphCandidate(a) - rankBodsNaptanGraphCandidate(b))
    .slice(0, BODS_NAPTAN_GRAPH_LINK_STOP_CAP);
}

function rankBodsNaptanGraphCandidate(candidate = {}) {
  const matchRank = {
    naptan_code_or_name_timetable_stop: 0,
    naptan_graph_adjacent_timetable_stop: 1,
  }[candidate.matchType] ?? 9;
  const graphHops = Number.isFinite(Number(candidate.graphHops)) ? Number(candidate.graphHops) : 99;
  const distanceMetres = Number.isFinite(Number(candidate.distanceMetres)) ? Number(candidate.distanceMetres) : Infinity;
  return matchRank * 1e9 + graphHops * 1e6 + distanceMetres;
}

function findBodsTimetableStopsByGraphAdjacency(timetable, startStops = [], departuresByStopId = new Map()) {
  const stops = timetable?.stops || [];
  const stopsById = timetable?.stopsById instanceof Map
    ? timetable.stopsById
    : new Map(stops.map((stop) => [stop.id, stop]));
  const startIds = Array.from(new Set((startStops || []).map((stop) => stop?.id).filter(Boolean)));
  ...
  (timetable?.connections || []).forEach((connection) => {
    ...
    if (!adjacency.has(connection.fromStopId)) adjacency.set(connection.fromStopId, new Set());
    if (!adjacency.has(connection.toStopId)) adjacency.set(connection.toStopId, new Set());
    adjacency.get(connection.fromStopId).add(connection.toStopId);
    adjacency.get(connection.toStopId).add(connection.fromStopId);
  });
  const queue = startIds.map((id) => ({ id, hops: 0 }));
  const visited = new Set(startIds);
  const linked = [];
  while (queue.length > 0 && linked.length < BODS_NAPTAN_GRAPH_LINK_STOP_CAP) {
    const current = queue.shift();
    if (!current || current.hops >= BODS_NAPTAN_GRAPH_LINK_MAX_HOPS) {
      continue;
    }
    const neighbours = Array.from(adjacency.get(current.id) || []);
    neighbours.forEach((nextId) => {
      if (visited.has(nextId)) {
        return;
      }
      visited.add(nextId);
      const nextStop = stopsById.get(nextId);
      const nextHops = current.hops + 1;
      if (nextStop && (departuresByStopId.get(nextId)?.length || 0) > 0) {
        linked.push({ stop: nextStop, graphHops: nextHops });
      }
      if (nextHops < BODS_NAPTAN_GRAPH_LINK_MAX_HOPS) {
...
function findBodsTimetableStopsLinkedFromPhysicalStop(timetableStops = [], physicalStop = {}, excludedStopIds = new Set(), options = {}) {
  if (!isValidCoordinateObject(physicalStop)) {
    return [];
  }
  const excludedIds = excludedStopIds instanceof Set ? excludedStopIds : new Set(excludedStopIds || []);
  const normalisedExcludedIds = new Set(Array.from(excludedIds).map(normaliseBodsStopIdentifier).filter(Boolean));
  const maximumLinkDistanceMetres = Number.isFinite(Number(options.maxDistanceMetres))
    ? Number(options.maxDistanceMetres)
    : BODS_INITIAL_ACCESS_LINK_TOLERANCE_METRES;
  const stopCap = Number.isFinite(Number(options.stopCap))
    ? Number(options.stopCap)
    : BODS_INITIAL_ACCESS_LINK_STOP_CAP;
  const includeStop = typeof options.includeStop === "function" ? options.includeStop : () => true;
  return (timetableStops || [])
    .filter((stop) => stop?.id && isValidCoordinateObject(stop))
    .filter((stop) => !normalisedExcludedIds.has(normaliseBodsStopIdentifier(stop.id)))
    .filter((stop) => includeStop(stop))
    .map((stop) => ({
      stop,
      distanceMetres: getDistanceMetres(
        physicalStop.latitude,
        physicalStop.longitude,
        stop.latitude,
        stop.longitude
      ),
    }))
    .filter((entry) => Number.isFinite(entry.distanceMetres) && entry.distanceMetres <= maximumLinkDistanceMetres)
    .sort((a, b) => a.distanceMetres - b.distanceMetres)
    .slice(0, stopCap);
}

function findBodsTimetableStopsReachableFromOrigin(timetableStops = [], originCoordinates = {}, maximumWalkToBusStopMetres, options = {}) {
  if (!isValidCoordinateObject(originCoordinates) || !Number.isFinite(Number(maximumWalkToBusStopMetres))) {
    return [];
  }
  const stopCap = Number.isFinite(Number(options.stopCap))
    ? Number(options.stopCap)
  ...
  return (timetableStops || [])
    ...
    .filter((entry) => Number.isFinite(entry.distanceMetres) && entry.distanceMetres <= Number(maximumWalkToBusStopMetres))
    .sort((a, b) => a.distanceMetres - b.distanceMetres)
    .slice(0, stopCap);
}

function buildBodsDeparturesByStopId(connections = []) {
  const departuresByStopId = new Map();
  connections.forEach((connection) => {
    if (!connection?.fromStopId || !Number.isFinite(Number(connection.departureMinutes))) {
      return;
    }
    if (!departuresByStopId.has(connection.fromStopId)) {
      departuresByStopId.set(connection.fromStopId, []);
    }
    departuresByStopId.get(connection.fromStopId).push(connection);
  });
  return departuresByStopId;
}

function matchBodsTimetableStopsToPhysicalStop(timetableStops = [], physicalStop = {}) {
  const physicalCodes = new Set(getNearbyOriginStopCodeCandidates(physicalStop));
  return timetableStops.filter((stop) => {
    const stopCodes = [
      stop?.id,
      stop?.atcoCode,
      stop?.naptanCode,
      stop?.ref,
    ].map(normaliseBodsStopIdentifier).filter(Boolean);
    if (stopCodes.some((candidate) => physicalCodes.has(candidate))) {
      return true;
    }
    return doBodsStopNamesMatch(stop?.name, physicalStop?.name);
  });
}

function datasetFilesContainNearbyOriginStopReferences(files = [], nearbyOriginStops = []) {
  const nearbyCodes = Array.from(new Set((nearbyOriginStops || []).flatMap(getNearbyOriginStopCodeCandidates)))
  ...
  return (files || []).some((file) => {
    ...
    const compactText = normaliseBodsStopIdentifier(text);
    return nearbyCodes.some((code) => compactText.includes(code));
  });
}
...
async function fetchBodsDatasetTextFiles(datasetUrl, options = {}, depth = 0) {
  ...
  const cacheKey = buildBodsDatasetTextCacheKey(datasetUrl);
  if (depth === 0) {
    const cachedFiles = getMapCacheEntry(BODS_DATASET_TEXT_CACHE, cacheKey, cloneBodsDatasetTextFiles);
    if (cachedFiles) {
      if (isBusTimingLogEnabled()) {
        console.debug("[BODS progress]", {
          phase: "dataset-text-cache-hit",
          datasetUrl,
  ...
  const timing = startBusTiming("BODS dataset text download", { datasetUrl, depth });
  ...
  if (contentType.includes("zip") || isZipArrayBuffer(arrayBuffer)) {
    const files = await unzipTextFiles(arrayBuffer, [".xml", ".txc", ".txt", ".csv"]).then((unpackedFiles) => {
      const relevant = unpackedFiles.filter((file) => isTransXChangeTextFile(file.name) || isGtfsStaticTextFile(file.name));
      return relevant.slice(0, BODS_MAX_XML_FILES_PER_DATASET);
    });
    if (depth === 0) {
      setMapCacheEntry(BODS_DATASET_TEXT_CACHE, cacheKey, files, cloneBodsDatasetTextFiles);
    }
    endBusTiming(timing, { datasetUrl, depth, fileCount: files.length, source: "zip" });
    return files;
  }
  ...
  if (contentType.includes("json") || /^[\s\r\n]*[\[{]/.test(text)) {
    try {
      ...
      if (files.length > 0) {
        ...
        endBusTiming(timing, { datasetUrl, depth, fileCount: files.length, source: "nested-json" });
        return files;
      }
    } catch (error) {
  ...
  const files = [{ name: datasetUrl.split("/").pop() || "bods-timetable.xml", text }];
  if (depth === 0) {
    setMapCacheEntry(BODS_DATASET_TEXT_CACHE, cacheKey, files, cloneBodsDatasetTextFiles);
  }
  endBusTiming(timing, { datasetUrl, depth, fileCount: files.length, source: "plain-text" });
  return files;
}
...
async function fetchBodsStopCoordinateFromNaptanApi(stopId, options = {}) {
  const candidateAreaCodes = deriveNaptanAreaCodeCandidatesFromStopId(stopId);
  for (const areaCode of candidateAreaCodes) {
    try {
      const areaStops = await fetchNaptanAccessNodesForAreaCode(areaCode);
      const matchedStop = areaStops.find((stop) => {
        const stopCodes = [
          stop?.id,
          stop?.atcoCode,
          stop?.naptanCode,
        ...
        return stopCodes.includes(normaliseBodsStopIdentifier(stopId));
      });
      if (matchedStop && isPlausibleUkWgs84Coordinate(matchedStop.latitude, matchedStop.longitude)) {
        return { latitude: matchedStop.latitude, longitude: matchedStop.longitude };
      }
    } catch (error) {
      if (error?.kind === "cancelled") {
        throw error;
...
function deriveNaptanAreaCodeCandidatesFromStopId(stopId) {
  const compact = String(stopId || "").replace(/[^0-9A-Za-z]/g, "");
  const candidates = [];
  const add = (value) => {
    const cleaned = String(value || "").replace(/[^0-9]/g, "");
...
function runBodsEarliestArrivalSearch(timetable, originCoordinates, configuredBands, maximumBandMinutes, maximumWalkToBusStopMetres, departureMinutes, boardingStopContext = null) {
  ...
  // `label` stores the absolute clock time at which the traveller is available at each stop.
  // `elapsedLabel` stores the catchment cost used for the contour bands. In BODS mode we use
  // the timetable to obtain actual bus running times, but do not require the timetable wait
  // before the first available service to fall inside the 15/30/45/60 minute bands. This keeps
  // BODS as the timing source while avoiding an empty catchment simply because the first parsed
  // service after the selected clock time is infrequent.
  const elapsedLabel = stops.map(() => new Array(BODS_MAX_TRANSFERS + 1).fill(Infinity));
  const previous = stops.map(() => new Array(BODS_MAX_TRANSFERS + 1).fill(null));
  const seedStops = Array.isArray(boardingStopContext?.seedStops) ? boardingStopContext.seedStops : null;
  const matchedBoardingStopLookup = new Map((boardingStopContext?.boardingStops || []).map((stop) => [stop.id, stop]));
  const initialWalkCandidateStopCount = Number.isFinite(Number(boardingStopContext?.eligiblePhysicalBoardingStopCount))
    ? Number(boardingStopContext.eligiblePhysicalBoardingStopCount)
    : stops.length;
  ...
  stops.forEach((stop, index) => {
    const matchedBoardingStop = matchedBoardingStopLookup.get(stop.id);
    const distance = matchedBoardingStop
      ? Number(matchedBoardingStop.walkingDistanceMetres)
      : getDistanceMetres(originCoordinates.latitude, originCoordinates.longitude, stop.latitude, stop.longitude);
    const estimatedWalkDistance = matchedBoardingStop
      ? Number(matchedBoardingStop.walkingDistanceMetres)
      : estimateBusWalkDistanceMetres(distance);
    const walkMinutes = matchedBoardingStop
      ? Number(matchedBoardingStop.walkingTimeMinutes)
      : estimateBusWalkTimeMinutes(distance);
    const isSeededBoardingStop = seedStops
      ? seedStops.some((seedStop) => seedStop.id === stop.id)
      : isBodsInitialStopWithinSelectedWalkLimit(distance, maximumWalkToBusStopMetres);
    ...
    if (isSeededBoardingStop && Number.isFinite(estimatedWalkDistance) && Number.isFinite(walkMinutes)) {
      initialStopsWithinMaxWalkCount += 1;
      if (walkMinutes < maximumBandMinutes) {
        const arrivalMinutes = seedStops
          ? Number(matchedBoardingStop?.seedArrivalMinutes)
          : departureMinutes + walkMinutes;
        label[index][0] = arrivalMinutes;
        elapsedLabel[index][0] = walkMinutes;
        previous[index][0] = {
          type: "initial_walk",
          distanceMetres: estimatedWalkDistance,
          arrivalMinutes,
          physicalStopName: matchedBoardingStop?.physicalStopName || "",
          stopCode: matchedBoardingStop?.atcoCode || matchedBoardingStop?.naptanCode || "",
        };
  ...
  const initialInterchangeRelaxationCount = applyBodsInitialInterchangeRelaxation(label, elapsedLabel, previous, transferEdges, departureMinutes, maximumBandMinutes);
  ...
  consideredConnections.forEach((connection) => {
    ...
    for (let transfersUsed = 0; transfersUsed <= BODS_MAX_TRANSFERS; transfersUsed += 1) {
      const canBoardConnection = label[fromIndex][transfersUsed] <= connection.departureMinutes;
      if (canBoardConnection) {
        const accessArrivalMinutes = label[fromIndex][transfersUsed];
        const accessElapsedMinutes = elapsedLabel[fromIndex][transfersUsed];
        const connectionRuntimeMinutes = Math.max(0, connection.arrivalMinutes - connection.departureMinutes);
        const segmentElapsedStart = Number.isFinite(accessElapsedMinutes) ? accessElapsedMinutes : Math.max(0, accessArrivalMinutes - departureMinutes);
        const segmentElapsedEnd = segmentElapsedStart + connectionRuntimeMinutes;
        const reachableSegment = buildBodsReachableSegment(connection, timetable.stopsById, departureMinutes, transfersUsed, {
          ...
          elapsedTimeAtStart: segmentElapsedStart,
          elapsedTimeAtEnd: segmentElapsedEnd,
          waitTimeCountsInBand: false,
        ...
        if (segmentElapsedEnd <= maximumBandMinutes && segmentElapsedEnd < elapsedLabel[toIndex][transfersUsed]) {
          label[toIndex][transfersUsed] = connection.arrivalMinutes;
          elapsedLabel[toIndex][transfersUsed] = segmentElapsedEnd;
          previous[toIndex][transfersUsed] = { type: "bus", connection, fromIndex, fromTransfersUsed: transfersUsed };
          applyBodsTransferRelaxation(toIndex, transfersUsed, label, elapsedLabel, previous, transferEdges, departureMinutes, maximumBandMinutes);
        }
      }
    }
  });
  const earliestByStop = stops
    .map((stop, index) => {
      const earliestElapsed = Math.min(...elapsedLabel[index]);
      const transfersUsed = elapsedLabel[index].indexOf(earliestElapsed);
      return {
        stop,
        earliestArrival: Math.min(...label[index]),
        earliestElapsed,
        transfersUsed,
      };
    })
    .filter((entry) => Number.isFinite(entry.earliestElapsed));
  const reachableStopsByBand = {};
  const reachableConnectionsByBand = {};
  configuredBands.forEach((band) => {
    const bandTime = Number(band.time);
    reachableStopsByBand[bandTime] = earliestByStop.filter((entry) => entry.earliestElapsed <= bandTime).length;
  ...
  return {
    ...
    earliestByStop,
    reachableSegments,
    nearbyPhysicalStopCount: Number(boardingStopContext?.nearbyPhysicalStopCount) || 0,
    eligiblePhysicalBoardingStopCount: Number(boardingStopContext?.eligiblePhysicalBoardingStopCount) || 0,
    matchedBoardingStopCount: Number(boardingStopContext?.matchedBoardingStopCount) || 0,
    directMatchedBoardingStopCount: Number(boardingStopContext?.directMatchedBoardingStopCount) || 0,
    accessLinkedBoardingStopCount: Number(boardingStopContext?.accessLinkedBoardingStopCount) || 0,
    boardingStopsWithDeparturesAfterSelectedTime: Number(boardingStopContext?.boardingStopsWithDeparturesAfterSelectedTime) || 0,
    boardingStopsSeededForGraphRelaxationCount: Number(boardingStopContext?.boardingStopsSeededForGraphRelaxationCount) || 0,
    hybridOsmRouteRelationCount: Number(boardingStopContext?.hybridOsmRouteRelationCount) || 0,
    hybridRouteAccessLinkCount: Number(boardingStopContext?.hybridRouteAccessLinkCount) || 0,
    hybridBodsAnchorCandidateCount: Number(boardingStopContext?.hybridBodsAnchorCandidateCount) || 0,
    hybridSyntheticStopCount: Number(boardingStopContext?.hybridSyntheticStopCount) || 0,
    hybridSyntheticConnectionCount: Number(boardingStopContext?.hybridSyntheticConnectionCount) || 0,
    hybridWarnings: Array.isArray(boardingStopContext?.hybridWarnings) ? boardingStopContext.hybridWarnings : [],
    physicalBoardingStopSamples: (boardingStopContext?.eligiblePhysicalStops || []).slice(0, 8),
    ...
    validPostDepartureConnectionCount: consideredConnections.length,
    ...
    reachableStopsByInitialBoardingStop: summariseBodsReachableStopsByInitialBoardingStop(earliestByStop, previous, stops),
    topRoutesByInitialBoardingStop: summariseBodsTopRoutesByInitialBoardingStop(reachableSegments),
...
function diagnoseBodsTimetableSearchFailure(searchResult) {
  const details = {
    ...
    initialStopsWithinMaxWalkCount: searchResult.initialStopsWithinMaxWalkCount,
    retainedInitialWalkStopCount: searchResult.retainedInitialWalkStopCount,
    directMatchedBoardingStopCount: searchResult.directMatchedBoardingStopCount,
    accessLinkedBoardingStopCount: searchResult.accessLinkedBoardingStopCount,
    hybridOsmRouteRelationCount: searchResult.hybridOsmRouteRelationCount,
    hybridRouteAccessLinkCount: searchResult.hybridRouteAccessLinkCount,
    hybridBodsAnchorCandidateCount: searchResult.hybridBodsAnchorCandidateCount,
    hybridSyntheticStopCount: searchResult.hybridSyntheticStopCount,
  ...
  if (searchResult.initialStopsWithinMaxWalkCount === 0) {
    const nearestText = Number.isFinite(Number(searchResult.nearestInitialStopStraightLineMetres))
      ...
      : "";
    if (Number(searchResult.matchedBoardingStopCount || 0) > 0 && Number(searchResult.boardingStopsWithDeparturesAfterSelectedTime || 0) === 0) {
      return {
        stage: "no_departures_from_linked_boarding_stops",
        message: `Nearby physical NaPTAN/OSM stops were found within the selected walk distance, and the app searched mapped OSM bus routes and a 10 km BODS timed-graph fallback for catchable scheduled anchor stops; however no catchable BODS timetable segment could be seeded after the selected time.${nearestText} Try a different departure time/date or check BODS operator coverage for services passing the nearby NaPTAN/OSM stops.`,
        details,
      };
    }
    return {
      stage: "no_stops_within_max_walk",
      message: `No walk-accessible physical stop or directly walkable parsed BODS timetable stop could be used as an initial boarding point.${nearestText} Check local NaPTAN/BODS stop matching, the selected time, or operator coverage.`,
      details,
    };
  }
  ...
  if (searchResult.consideredConnectionCount === 0) {
    return {
      ...
      message: "Walk-accessible physical NaPTAN/OSM stops were found, but the parsed BODS graph did not expose valid scheduled departures after the selected time within the configured look-ahead window. Try a different time/date or check BODS operator coverage.",
      details,
    };
  }
  ...
  if (searchResult.reachableSegments.length === 0) {
    return {
      ...
      message: "BODS timetable departures were found after the selected time, but none could be boarded before the end of the configured bus time bands. Check the selected departure time/date or increase the maximum band if the first service is later than the current bands.",
      details,
    };
  }
  ...
  for (let guard = 0; guard < 16; guard += 1) {
    ...
    if (entry.type === "initial_walk") {
      steps.push({
        ...
        physicalStopName: entry.physicalStopName || "",
...
function summariseBodsReachableStopsByInitialBoardingStop(earliestByStop = [], previous = [], stops = []) {
  const stopIndexById = new Map(stops.map((stop, index) => [stop.id, index]));
  const counts = new Map();
  (earliestByStop || []).forEach((entry) => {
    const index = stopIndexById.get(entry.stop.id);
    if (index === undefined) {
      return;
    }
    const chain = buildBodsJourneyChainSnapshot(previous, stops, stopIndexById, index, entry.transfersUsed, 0);
    const initialStep = chain.find((step) => step.type === "initial_walk");
    if (!initialStep?.toStopId) {
      return;
    }
    if (!counts.has(initialStep.toStopId)) {
      counts.set(initialStep.toStopId, {
        stop_id: initialStep.toStopId,
        stop_name: initialStep.toStopName || initialStep.toStopId,
        stop_code: initialStep.stopCode || "",
        physical_stop_name: initialStep.physicalStopName || "",
        reachable_stop_count: 0,
      });
    }
    counts.get(initialStep.toStopId).reachable_stop_count += 1;
  });
  return Array.from(counts.values()).sort((a, b) => b.reachable_stop_count - a.reachable_stop_count);
}

function summariseBodsTopRoutesByInitialBoardingStop(reachableSegments = []) {
  const grouped = new Map();
  (reachableSegments || []).forEach((segment) => {
    const initialStep = Array.isArray(segment.journeyChain)
      ? segment.journeyChain.find((step) => step.type === "initial_walk")
      : null;
    if (!initialStep?.toStopId) {
      return;
    }
    if (!grouped.has(initialStep.toStopId)) {
      grouped.set(initialStep.toStopId, {
        stop_id: initialStep.toStopId,
        stop_name: initialStep.toStopName || initialStep.toStopId,
        stop_code: initialStep.stopCode || "",
        physical_stop_name: initialStep.physicalStopName || "",
    ...
    const routeKey = normaliseBodsReachableRouteKey(segment);
    const groupedStop = grouped.get(initialStep.toStopId);
    if (!groupedStop.routes.has(routeKey)) {
      groupedStop.routes.set(routeKey, {
        route_ref: segment.routeRef || "",
        route_name: segment.routeName || "",
        route_id: segment.routeId || "",
        reachable_link_count: 0,
        sample_stop_names: new Set(),
      });
    }
    const routeEntry = groupedStop.routes.get(routeKey);
    routeEntry.reachable_link_count += 1;
    [segment.fromStopName, segment.toStopName].filter(Boolean).forEach((name) => {
      if (routeEntry.sample_stop_names.size < 6) {
        routeEntry.sample_stop_names.add(name);
      }
    });
  });
  return Array.from(grouped.values()).map((entry) => ({
    ...
    physical_stop_name: entry.physical_stop_name,
    routes: Array.from(entry.routes.values())
      .sort((a, b) => b.reachable_link_count - a.reachable_link_count)
      .slice(0, 6)
      .map((route) => ({
        route_ref: route.route_ref,
        route_name: route.route_name,
        route_id: route.route_id,
        reachable_link_count: route.reachable_link_count,
        sample_stop_names: Array.from(route.sample_stop_names),
      })),
  }));
}
...
function buildBusIsochroneDiagnosticsForMethodNote(metadata = {}) {
  ...
  return {
    ...
    bods_direct_matched_boarding_stop_count: metadata.bodsDirectMatchedBoardingStopCount,
    bods_access_linked_boarding_stop_count: metadata.bodsAccessLinkedBoardingStopCount,
...
async function runBodsRealityTest(options = {}) {
  const originCoordinates = options.originCoordinates
    || parseCoordinatePair(elements.accessCoordinates?.value || "", true)
    || parseCoordinatePair(DEFAULT_ACCESS_COORDINATES, true)
    || parseCoordinatePair(DEFAULT_SITE_COORDINATES);
  ...
  try {
    ...
    console.info("[BODS reality test]", result);
    return result;
  } catch (error) {
    ...
    console.warn("[BODS reality test]", result);
    return result;
  } finally {
...
function buildBodsLeedsRealityDiagnostics({ originCoordinates, departureDate, departureTime, maximumWalkToBusStopMetres, isochrones, metadata }) {
  ...
  return {
    ...
    bodsSelectedDatasetSummaries: metadata.bodsSelectedDatasetSummaries || [],
    bodsRejectedDatasetSamples: metadata.bodsRejectedDatasetSamples || [],
    ...
    nearbyPhysicalStopCount: metadata.nearbyPhysicalStopCount ?? 0,
    eligiblePhysicalBoardingStopCount: metadata.eligiblePhysicalBoardingStopCount ?? 0,
    bodsMatchedBoardingStopCount: metadata.bodsMatchedBoardingStopCount ?? 0,
    bodsDirectMatchedBoardingStopCount: metadata.bodsDirectMatchedBoardingStopCount ?? 0,
    bodsAccessLinkedBoardingStopCount: metadata.bodsAccessLinkedBoardingStopCount ?? 0,
    boardingStopsWithDeparturesAfterSelectedTime: metadata.boardingStopsWithDeparturesAfterSelectedTime ?? 0,
    ...
    validPostDepartureConnectionCount: metadata.validPostDepartureConnectionCount ?? 0,
    ...
    reachableStopsByInitialBoardingStop: metadata.reachableStopsByInitialBoardingStop || [],
    topRoutesByInitialBoardingStop: metadata.topRoutesByInitialBoardingStop || [],
...
function getBodsLeedsRealityFailures(diagnostics) {
  ...
  const wrongAreaPattern = /\b(potters bar|south mimms|borehamwood|elstree|london)\b/i;
  ...
  if ((diagnostics.bodsSelectedDatasetSummaries || []).some((sample) => wrongAreaPattern.test(`${sample.name || ""} ${sample.operator || ""}`))) failures.push("Selected datasets include obvious wrong-area samples.");
  if (Number(diagnostics.nearbyPhysicalStopCount) <= 0) failures.push("No nearby physical stops were recorded.");
  if (Number(diagnostics.eligiblePhysicalBoardingStopCount) > 1 && Number(diagnostics.initialStopCount) <= 1) failures.push("Multiple nearby boarding stops exist but only one was seeded into the search.");
  if (Number(diagnostics.bodsMatchedBoardingStopCount) <= 0) failures.push("No BODS-matched boarding stops were recorded.");
...
window.runBodsRealityTest = runBodsRealityTest;
window.runBodsLeedsRealityTest = runBodsRealityTest;
if (typeof window.addEventListener === "function") {
  ...
  try {
    ...
    const result = await runBodsRealityTest(options);
...
function createBodsLeedsRealityTestHarness() {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const result = await runBodsRealityTest({
...
function applyBodsInitialInterchangeRelaxation(label, elapsedLabel, previous, transferEdges, departureMinutes, maximumBandMinutes) {
  let relaxedCount = 0;
  label.forEach((entries, fromIndex) => {
    const fromArrival = entries?.[0];
    const fromElapsed = elapsedLabel[fromIndex]?.[0];
    if (!Number.isFinite(fromArrival) || !Number.isFinite(fromElapsed)) {
      return;
    }
    (transferEdges[fromIndex] || []).forEach((edge) => {
      const nextArrival = fromArrival + edge.travelTimeMinutes;
      const nextElapsed = fromElapsed + edge.travelTimeMinutes;
      if (nextElapsed <= maximumBandMinutes && nextElapsed < elapsedLabel[edge.toIndex][1]) {
        label[edge.toIndex][1] = nextArrival;
        elapsedLabel[edge.toIndex][1] = nextElapsed;
...
function applyBodsTransferRelaxation(fromIndex, transfersUsed, label, elapsedLabel, previous, transferEdges, departureMinutes, maximumBandMinutes) {
  if (transfersUsed >= BODS_MAX_TRANSFERS) {
    return;
  }
  const fromArrival = label[fromIndex][transfersUsed];
  const fromElapsed = elapsedLabel[fromIndex][transfersUsed];
  if (!Number.isFinite(fromArrival) || !Number.isFinite(fromElapsed)) {
    return;
  }
  transferEdges[fromIndex].forEach((edge) => {
    const nextArrival = fromArrival + edge.travelTimeMinutes;
    const nextElapsed = fromElapsed + edge.travelTimeMinutes;
    if (nextElapsed <= maximumBandMinutes && nextElapsed < elapsedLabel[edge.toIndex][transfersUsed + 1]) {
      label[edge.toIndex][transfersUsed + 1] = nextArrival;
      elapsedLabel[edge.toIndex][transfersUsed + 1] = nextElapsed;
...
function buildBodsReachableSegment(connection, stopsById, departureMinutes, transfersUsed, context = {}) {
  ...
  return {
    ...
    elapsedTimeAtStart: Number.isFinite(Number(context.elapsedTimeAtStart))
      ? Number(context.elapsedTimeAtStart)
      : connection.departureMinutes - departureMinutes,
    elapsedTimeAtEnd: Number.isFinite(Number(context.elapsedTimeAtEnd))
      ? Number(context.elapsedTimeAtEnd)
      : connection.arrivalMinutes - departureMinutes,
    ...
    waitTimeCountsInBand: context.waitTimeCountsInBand !== false,
    ...
    transfersUsed,
    isHybridAccessConnectorOnly: connection.isHybridAccessConnectorOnly === true,
    reachedVia: connection.isHybridAccessConnectorOnly === true ? "hybrid_access_connector" : "scheduled_bus",
  };
}
...
function buildBodsTimetableIsochronesFromSearch(searchResult, configuredBands) {
  ...
  configuredBands.forEach((band) => {
    ...
    const bandSegments = searchResult.reachableSegments
      ...
      .filter(Boolean)
      .filter((segment) => segment.isHybridAccessConnectorOnly !== true);

    // Do not draw a bus catchment from initial walking/access seeding alone. A BODS bus band
    // must include at least one scheduled bus movement within that time band. Without this
    // guard the map can show a small circular blob around the site, because the polygon builder
    // buffers the origin/boarding seed even when no bus leg has contributed to the catchment.
    if (bandSegments.length === 0) {
      return;
    }

    const scheduledStopIds = new Set();
    bandSegments.forEach((segment) => {
      if (segment.fromStopId) scheduledStopIds.add(segment.fromStopId);
      if (segment.toStopId) scheduledStopIds.add(segment.toStopId);
    ...
    searchResult.earliestByStop
      .filter((entry) => scheduledStopIds.has(entry.stop?.id))
      .filter((entry) => Number(entry.earliestElapsed) <= bandTime)
      .forEach((entry) => stopMetrics.push({ coordinate: { latitude: entry.stop.latitude, longitude: entry.stop.longitude }, minimumTotalBusAccessTimeMinutes: Number(entry.earliestElapsed) }));
    if (stopMetrics.length > 0 && isValidCoordinateObject(searchResult.originCoordinates)) {
      stopMetrics.push({
        coordinate: {
...
function buildBodsTimetableSourceNote(metadata = {}) {
  ...
  const boardingStopText = Number.isFinite(Number(metadata.nearbyPhysicalStopCount)) || Number.isFinite(Number(metadata.eligiblePhysicalBoardingStopCount)) || Number.isFinite(Number(metadata.bodsMatchedBoardingStopCount))
    ? ` Boarding-stop audit: ${Number(metadata.nearbyPhysicalStopCount || 0).toLocaleString("en-GB")} nearby physical stop${Number(metadata.nearbyPhysicalStopCount || 0) === 1 ? "" : "s"} found; ${Number(metadata.eligiblePhysicalBoardingStopCount || 0).toLocaleString("en-GB")} within the selected walk limit; ${Number(metadata.bodsMatchedBoardingStopCount || 0).toLocaleString("en-GB")} usable BODS boarding stop${Number(metadata.bodsMatchedBoardingStopCount || 0) === 1 ? "" : "s"}, including ${Number(metadata.bodsDirectMatchedBoardingStopCount || 0).toLocaleString("en-GB")} direct ATCO/NaPTAN/name match${Number(metadata.bodsDirectMatchedBoardingStopCount || 0) === 1 ? "" : "es"} and ${Number(metadata.bodsAccessLinkedBoardingStopCount || 0).toLocaleString("en-GB")} local access-linked timetable stop${Number(metadata.bodsAccessLinkedBoardingStopCount || 0) === 1 ? "" : "s"}; ${Number(metadata.boardingStopsWithDeparturesAfterSelectedTime || 0).toLocaleString("en-GB")} with scheduled departures after the selected time.`
    : "";
  ...
  const precisionWarningText = geometrySummary.straightLineFallbackUsed || geometrySummary.estimatedRuntimeUsed || Number(metadata.bodsEstimatedRuntimeConnectionCount || 0) > 0
    ...
    : "";
  const walkText = Number.isFinite(Number(metadata.maximumWalkToBusStopMetres)) ? ` Initial physical stop access is limited to ${Math.round(metadata.maximumWalkToBusStopMetres).toLocaleString("en-GB")} m from the selected origin; the walked-to NaPTAN/OSM stop is then cross-referenced into the BODS graph and the linked BODS graph node does not need to be within walking distance. Transfer walks are limited to ${Math.round(metadata.transferWalkRadiusMetres || BODS_TRANSFER_RADIUS_METRES).toLocaleString("en-GB")} m.` : "";
...
function buildBusMethodLimitationText(metadata = {}) {
  if ((metadata.provider || "").includes("BODS") || getSelectedBusMethod() === BUS_METHOD_BODS) {
    ...
    return `Bus catchments are timetable-based outputs generated from Bus Open Data Service timetable data where local TransXChange files can be parsed. Initial walking access is limited to physical NaPTAN/OSM stops within ${Math.round(maximumWalkToBusStopMetres).toLocaleString("en-GB")} m. Those physical stops are then cross-referenced into the parsed BODS graph using ATCO/NaPTAN references, stop-name matching or graph-adjacent scheduled nodes; the linked BODS graph node does not need to be within walking distance. Initial and transfer walking are estimated using straight-line distance multiplied by 1.3 and ${BUS_ACCESS_WALK_SPEED_KPH} kph / ${Math.round(BUS_ACCESS_WALK_SPEED_METRES_PER_MINUTE)} m per minute walking speed. Outputs do not include live disruption, reliability, cancellations, vehicle crowding or fare integration.`;
  }
...
function buildBodsDatasetTextCacheKey(datasetUrl) {
  return JSON.stringify({
    api: BODS_CACHE_SCHEMA_VERSION,
...
function buildBodsTimetableModelCacheKey(datasetUrls, options = {}) {
  const nearbyOriginStopKey = Array.from(new Set((options.nearbyOriginStops || []).flatMap(getNearbyOriginStopCodeCandidates)))
    .sort()
    .slice(0, 128)
    .join(",");
  return JSON.stringify({
    api: BODS_CACHE_SCHEMA_VERSION,
    datasetUrls: (datasetUrls || []).map((url) => String(url || "")).sort(),
    departureDate: String(options.departureDate || ""),
    nearbyOriginStopKey,
    allowExternalStopEnrichment: options.allowExternalStopEnrichment !== false,
  });
}
...
function cloneBodsDatasetTextFiles(files) {
...
function cloneBodsTimetableModelResult(model) {
  if (!model) {
    return model;
  }
  const clonedStops = (model.stops || []).map((stop) => clonePlainValue(stop));
  const clonedWarnings = [...(model.warnings || [])];
  const clonedConnections = (model.connections || []).map((connection) => ({
    ...clonePlainValue(connection),
    geometry: Array.isArray(connection?.geometry)
      ? connection.geometry.map((coordinate) => clonePlainValue(coordinate))
      : null,
  }));
  const cloned = {
    stops: clonedStops,
    stopsById: new Map(clonedStops.map((stop) => [stop.id, stop])),
    connections: clonedConnections,
    downloadedFileCount: Number(model.downloadedFileCount) || 0,
    parsedXmlFileCount: Number(model.parsedXmlFileCount) || 0,
    parseFailureCount: Number(model.parseFailureCount) || 0,
    stopReferenceCount: Number(model.stopReferenceCount) || 0,
    directCoordinateStopCount: Number(model.directCoordinateStopCount) || 0,
    routeLinkCoordinateStopCount: Number(model.routeLinkCoordinateStopCount) || 0,
    parsedRuntimeConnectionCount: Number(model.parsedRuntimeConnectionCount) || 0,
    ...
    rejectedImplausibleConnectionCount: Number(model.rejectedImplausibleConnectionCount) || 0,
    originMatchedStopCount: Number(model.originMatchedStopCount) || 0,
    impliedSpeedStats: clonePlainValue(model.impliedSpeedStats) || { count: 0, min: null, median: null, max: null },
    impliedSpeedKphValues: [...(model.impliedSpeedKphValues || [])],
    enrichedCoordinateStopCount: Number(model.enrichedCoordinateStopCount) || 0,
    unresolvedCoordinateStopCount: Number(model.unresolvedCoordinateStopCount) || 0,
    sampleMissingStopIds: [...(model.sampleMissingStopIds || [])],
    stopStructureSamples: (model.stopStructureSamples || []).map((sample) => clonePlainValue(sample)),
