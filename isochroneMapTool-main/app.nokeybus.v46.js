...
const BODS_DATASET_ENDPOINT = "https://isochronemaptoolwalk.e-gorton.workers.dev/api/proxy/bods/dataset";
const BODS_DOWNLOAD_ENDPOINT = "https://isochronemaptoolwalk.e-gorton.workers.dev/api/proxy/bods/download";
...
const BODS_CACHE_SCHEMA_VERSION = "v77-bods-earliest-stop-render";
...
const BODS_MAX_STRAIGHT_STOP_LINK_GEOMETRY_METRES = 500;
const BODS_MAX_STRAIGHT_STOP_LINK_GEOMETRY_SPEED_KPH = 80;
const BODS_SCHEDULED_STOP_FALLBACK_BUFFER_METRES = 110;
const BODS_SCHEDULED_STOP_FALLBACK_MAX_RINGS_PER_BAND = 1200;
...
const BODS_STOP_NAME_MATCH_MAX_DISTANCE_METRES = 250;
...
function injectNearbyOriginStopMatchesIntoTxcModel(model, xmlText, nearbyStops = []) {
  ...
  missingStops.forEach((missingStop) => {
    ...
    const nameMatch = nearbyStops.find((nearbyStop) => isBodsStopNameMatchGeographicallySafe(missingStop, nearbyStop));
    if (nameMatch) {
  ...
  stops.forEach((existingStop) => {
    ...
    const nameMatch = nearbyStops.find((nearbyStop) => isBodsStopNameMatchGeographicallySafe(existingStop, nearbyStop));
    if (nameMatch) {
...
async function buildLocalBodsTimetableModel(datasetUrls, options = {}) {
  ...
  for (let index = 0; index < datasetUrls.length; index += 1) {
    ...
    const nearestRealStopDistanceMetres = getNearestRealBodsStopDistanceMetres(model.stops, options.originCoordinates);
    const realLocalStopCount = countBodsRealLocalStopsNearOrigin(model.stops, options.originCoordinates);
...
function countNearbyOriginMatchesInStops(stopsById, nearbyStops = []) {
  ...
  stopsById.forEach((stop, id) => {
    ...
    if (idCandidates.some((candidate) => nearbyCodes.has(candidate)) || nearbyStops.some((nearbyStop) => isBodsStopNameMatchGeographicallySafe(stop, nearbyStop))) {
      count += 1;
    }
  });
  return count;
...
function buildBodsBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes) {
  ...
  const boardingStopsWithDeparturesAfterSelectedTime = boardingStops.filter((stop) => stop.departuresAfterSelectedTime > 0).length;
  ...
  return {
    ...
    boardingStopsWithDeparturesAfterSelectedTime,
...
function matchBodsTimetableStopsToPhysicalStop(timetableStops = [], physicalStop = {}) {
  ...
  return timetableStops.filter((stop) => {
    ...
    return isBodsStopNameMatchGeographicallySafe(stop, physicalStop);
  });
}

function isBodsStopNameMatchGeographicallySafe(bodsStop = {}, physicalStop = {}) {
  if (!doBodsStopNamesMatch(bodsStop?.name, physicalStop?.name)) {
    return false;
  }
  const bodsCoordinate = getBodsStopCoordinateForSafeMatch(bodsStop);
  const physicalCoordinate = getBodsStopCoordinateForSafeMatch(physicalStop);
  if (!bodsCoordinate || !physicalCoordinate) {
    return false;
  }
  const distance = getDistanceMetres(
    bodsCoordinate.latitude,
    bodsCoordinate.longitude,
    physicalCoordinate.latitude,
    physicalCoordinate.longitude
  );
  return Number.isFinite(distance) && distance <= BODS_STOP_NAME_MATCH_MAX_DISTANCE_METRES;
}

function getBodsStopCoordinateForSafeMatch(stop = {}) {
  const latitude = Number(stop.latitude ?? stop.lat ?? stop.coordinate?.latitude);
  const longitude = Number(stop.longitude ?? stop.lon ?? stop.lng ?? stop.coordinate?.longitude);
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    return null;
  }
  return { latitude, longitude };
...
function runBodsEarliestArrivalSearch(timetable, originCoordinates, configuredBands, maximumBandMinutes, maximumWalkToBusStopMetres, departureMinutes, boardingStopContext = null) {
  ...
  const costLabel = stops.map(() => new Array(BODS_MAX_TRANSFERS + 1).fill(Infinity));
  ...
  stops.forEach((stop, index) => {
    ...
    if (isSeededBoardingStop && Number.isFinite(estimatedWalkDistance) && Number.isFinite(walkMinutes)) {
      ...
      if (walkMinutes < maximumBandMinutes) {
        ...
        costLabel[index][0] = walkMinutes;
        previous[index][0] = {
          type: "initial_walk",
          distanceMetres: estimatedWalkDistance,
          arrivalMinutes,
          elapsedCostMinutes: walkMinutes,
  ...
  const initialInterchangeRelaxationCount = applyBodsInitialInterchangeRelaxation(label, costLabel, previous, transferEdges, maximumBandMinutes);
  const reachableSegments = [];
  const reachableSegmentKeys = new Set();
  let firstPostSelectedDeparture = null;
  let firstCandidateSegment = null;
  let firstRejectedCandidate = null;
  ...
  firstPostSelectedDeparture = consideredConnections[0] || null;
  consideredConnections.forEach((connection) => {
    ...
    for (let transfersUsed = 0; transfersUsed <= BODS_MAX_TRANSFERS; transfersUsed += 1) {
      ...
      const accessCostMinutes = costLabel[fromIndex][transfersUsed];
      const segmentRuntimeMinutes = connection.arrivalMinutes - connection.departureMinutes;
      const elapsedCostAtStart = accessCostMinutes;
      const elapsedCostAtEnd = accessCostMinutes + segmentRuntimeMinutes;
      if (
        !firstCandidateSegment
        && Number.isFinite(accessArrivalMinutes)
        && accessArrivalMinutes <= connection.departureMinutes
        && Number.isFinite(segmentRuntimeMinutes)
        && segmentRuntimeMinutes > 0
      ) {
        firstCandidateSegment = buildBodsCandidateSegmentDiagnostic(connection, timetable.stopsById, {
          accessArrivalMinutes,
          accessCostMinutes,
          initialWalkTimeMinutes: getBodsInitialWalkCostMinutes(previous, stops, fromIndex, transfersUsed),
          segmentRuntimeMinutes,
          elapsedCostAtEnd,
        });
      }
      if (accessArrivalMinutes <= connection.departureMinutes && elapsedCostAtEnd <= maximumBandMinutes) {
        const reachableSegment = buildBodsReachableSegment(connection, timetable.stopsById, departureMinutes, transfersUsed, {
          accessArrivalMinutes,
          elapsedTimeAtStart: elapsedCostAtStart,
          elapsedTimeAtEnd: elapsedCostAtEnd,
        ...
        if (elapsedCostAtEnd < costLabel[toIndex][transfersUsed] || (elapsedCostAtEnd === costLabel[toIndex][transfersUsed] && connection.arrivalMinutes < label[toIndex][transfersUsed])) {
          label[toIndex][transfersUsed] = connection.arrivalMinutes;
          costLabel[toIndex][transfersUsed] = elapsedCostAtEnd;
          previous[toIndex][transfersUsed] = { type: "bus", connection, fromIndex, fromTransfersUsed: transfersUsed };
          applyBodsTransferRelaxation(toIndex, transfersUsed, label, costLabel, previous, transferEdges, maximumBandMinutes);
        }
      } else if (!firstRejectedCandidate && Number.isFinite(accessArrivalMinutes) && accessArrivalMinutes <= connection.departureMinutes) {
        firstRejectedCandidate = buildBodsCandidateSegmentDiagnostic(connection, timetable.stopsById, {
          accessArrivalMinutes,
          accessCostMinutes,
          initialWalkTimeMinutes: getBodsInitialWalkCostMinutes(previous, stops, fromIndex, transfersUsed),
          segmentRuntimeMinutes,
          elapsedCostAtEnd,
          rejectionReason: !Number.isFinite(segmentRuntimeMinutes) || segmentRuntimeMinutes <= 0
            ? "invalid_bods_segment_runtime"
            : !Number.isFinite(accessCostMinutes)
              ? "missing_initial_access_cost"
              : `walk_plus_bods_runtime_exceeds_max_band_${maximumBandMinutes}`,
        });
      }
    }
  });
  const earliestByStop = stops
    .map((stop, index) => {
      const minimumCost = Math.min(...costLabel[index]);
      const transfersUsed = costLabel[index].indexOf(minimumCost);
      return {
        stop,
        earliestArrival: label[index][transfersUsed],
        elapsedCostMinutes: minimumCost,
        transfersUsed,
      };
    })
    .filter((entry) => Number.isFinite(entry.elapsedCostMinutes));
  const reachableStopsByBand = {};
  const reachableConnectionsByBand = {};
  configuredBands.forEach((band) => {
    const bandTime = Number(band.time);
    reachableStopsByBand[bandTime] = earliestByStop.filter((entry) => entry.elapsedCostMinutes <= bandTime).length;
  ...
  return {
    ...
    firstPostSelectedDeparture: firstPostSelectedDeparture ? buildBodsCandidateSegmentDiagnostic(firstPostSelectedDeparture, timetable.stopsById) : null,
    firstCandidateSegment,
    firstRejectedCandidate,
    maxConfiguredBandMinutes: maximumBandMinutes,
...
function diagnoseBodsTimetableSearchFailure(searchResult) {
  const drawableReachableSegmentCount = Array.isArray(searchResult.reachableSegments)
    ? searchResult.reachableSegments.filter(isDrawableBodsReachableSegment).length
    : 0;
  const details = {
    initialWalkCandidateStopCount: searchResult.initialWalkCandidateStopCount,
    initialStopsWithinMaxWalkCount: searchResult.initialStopsWithinMaxWalkCount,
    ...
    reachableStopCount: searchResult.reachableStopCount,
    reachableConnectionCount: searchResult.reachableSegments.length,
    drawableReachableSegmentCount,
    rejectedSyntheticOrUndrawableSegmentCount: Math.max(0, searchResult.reachableSegments.length - drawableReachableSegmentCount),
    firstPostSelectedDeparture: searchResult.firstPostSelectedDeparture,
    firstCandidateBodsSegment: searchResult.firstCandidateSegment,
    firstRejectedCandidateBodsSegment: searchResult.firstRejectedCandidate,
    maxConfiguredBandMinutes: searchResult.maxConfiguredBandMinutes,
  };
  ...
  if (drawableReachableSegmentCount === 0) {
    return {
      stage: "reachable_movement_no_route_continuous_drawable_geometry",
      message: "BODS timetable movement was reachable for timing purposes, but no route-continuous drawable bus movement geometry could be established from the walk-accessible boarding stop. Synthetic access or scheduled-anchor connector links are retained for timing only and are not drawn.",
      details,
    };
  }
  ...
  for (let guard = 0; guard < 16; guard += 1) {
    ...
    if (entry.type === "initial_walk") {
      steps.push({
        ...
        elapsedCostMinutes: Number.isFinite(Number(entry.elapsedCostMinutes)) ? Math.round(Number(entry.elapsedCostMinutes) * 10) / 10 : null,
...
function applyBodsInitialInterchangeRelaxation(label, costLabel, previous, transferEdges, maximumBandMinutes) {
  let relaxedCount = 0;
  label.forEach((entries, fromIndex) => {
    const fromArrival = entries?.[0];
    const fromCost = costLabel[fromIndex]?.[0];
    if (!Number.isFinite(fromArrival)) {
      return;
    }
    (transferEdges[fromIndex] || []).forEach((edge) => {
      const nextArrival = fromArrival + edge.travelTimeMinutes;
      const nextCost = fromCost + edge.travelTimeMinutes;
      if (nextCost <= maximumBandMinutes && (nextCost < costLabel[edge.toIndex][1] || (nextCost === costLabel[edge.toIndex][1] && nextArrival < label[edge.toIndex][1]))) {
        label[edge.toIndex][1] = nextArrival;
        costLabel[edge.toIndex][1] = nextCost;
...
function applyBodsTransferRelaxation(fromIndex, transfersUsed, label, costLabel, previous, transferEdges, maximumBandMinutes) {
  if (transfersUsed >= BODS_MAX_TRANSFERS) {
    return;
  }
  const fromArrival = label[fromIndex][transfersUsed];
  const fromCost = costLabel[fromIndex][transfersUsed];
  transferEdges[fromIndex].forEach((edge) => {
    const nextArrival = fromArrival + edge.travelTimeMinutes;
    const nextCost = fromCost + edge.travelTimeMinutes;
    if (nextCost <= maximumBandMinutes && (nextCost < costLabel[edge.toIndex][transfersUsed + 1] || (nextCost === costLabel[edge.toIndex][transfersUsed + 1] && nextArrival < label[edge.toIndex][transfersUsed + 1]))) {
      label[edge.toIndex][transfersUsed + 1] = nextArrival;
      costLabel[edge.toIndex][transfersUsed + 1] = nextCost;
      previous[edge.toIndex][transfersUsed + 1] = { type: "transfer_walk", fromIndex, fromTransfersUsed: transfersUsed, elapsedCostMinutes: nextCost, ...edge };
    }
  });
}
...
function buildBodsReachableSegment(connection, stopsById, departureMinutes, transfersUsed, context = {}) {
  ...
  return {
    ...
    tripId: connection.tripId,
    timingSource: connection.timingSource,
    connectionGeometrySource: connection.geometrySource || "",
    drawableGeometryType: getBodsConnectionDrawableGeometryType(connection, routeGeometry),
    syntheticTimingOnly: isBodsSyntheticTimingConnection(connection),
    fromStopId: connection.fromStopId,
    ...
    elapsedTimeAtStart: Number.isFinite(Number(context.elapsedTimeAtStart)) ? Number(context.elapsedTimeAtStart) : connection.departureMinutes - departureMinutes,
    elapsedTimeAtEnd: Number.isFinite(Number(context.elapsedTimeAtEnd)) ? Number(context.elapsedTimeAtEnd) : connection.arrivalMinutes - departureMinutes,
    ...
    routeGeometry,
    geometrySource: routeGeometry.length >= 2 ? "parsed_route_geometry" : connection.geometrySource || "stop_buffer",
    transfersUsed,
    reachedVia: "scheduled_bus",
  };
}

function isBodsSyntheticTimingConnection(connection) {
  const sourceText = [
    connection?.geometrySource,
    connection?.timingSource,
    connection?.source,
    connection?.connectionType,
    connection?.linkType,
  ].map((value) => String(value || "").toLowerCase()).join(" ");
  return /\bsynthetic\b|\bconnector\b|\banchor\b|\binjected\b/.test(sourceText);
}

function getBodsConnectionDrawableGeometryType(connection, routeGeometry = []) {
  if (isBodsSyntheticTimingConnection(connection)) {
    return "timing_only_synthetic_connector";
  }
  if (Array.isArray(routeGeometry) && routeGeometry.length >= 2) {
    return "actual_route_geometry";
  }
  const source = String(connection?.geometrySource || "").toLowerCase();
  if (source.includes("stop_to_stop")) {
    return "scheduled_stop_to_stop";
  }
  return "undrawable";
}

function buildBodsCandidateSegmentDiagnostic(connection, stopsById, context = {}) {
  if (!connection) {
    return null;
  }
  const from = stopsById?.get?.(connection.fromStopId);
  const to = stopsById?.get?.(connection.toStopId);
  const runtime = Number.isFinite(Number(context.segmentRuntimeMinutes))
    ? Number(context.segmentRuntimeMinutes)
    : Number(connection.arrivalMinutes) - Number(connection.departureMinutes);
  return {
    fromStopId: connection.fromStopId,
    fromStopName: from?.name || connection.fromStopName || "",
    toStopId: connection.toStopId,
    toStopName: to?.name || connection.toStopName || "",
    routeName: connection.routeName || connection.routeRef || "",
    departureMinutes: connection.departureMinutes,
    arrivalMinutes: connection.arrivalMinutes,
    bodsSegmentRuntimeMinutes: Number.isFinite(runtime) ? runtime : null,
    initialWalkTimeMinutes: Number.isFinite(Number(context.initialWalkTimeMinutes)) ? Number(context.initialWalkTimeMinutes) : null,
    accessCostMinutes: Number.isFinite(Number(context.accessCostMinutes)) ? Number(context.accessCostMinutes) : null,
    accessArrivalMinutes: Number.isFinite(Number(context.accessArrivalMinutes)) ? Number(context.accessArrivalMinutes) : null,
    elapsedCostAtEnd: Number.isFinite(Number(context.elapsedCostAtEnd)) ? Number(context.elapsedCostAtEnd) : null,
    rejectionReason: context.rejectionReason || null,
  };
}

function getBodsInitialWalkCostMinutes(previous, stops, currentIndex, transfersUsed) {
  const stopIndexById = new Map(stops.map((stop, index) => [stop.id, index]));
  const chain = buildBodsJourneyChainSnapshot(previous, stops, stopIndexById, currentIndex, transfersUsed, 0);
  const initialWalk = chain.find((step) => step.type === "initial_walk");
  return Number.isFinite(Number(initialWalk?.elapsedCostMinutes))
    ? Number(initialWalk.elapsedCostMinutes)
    : Number.isFinite(Number(initialWalk?.arrivalMinutes))
      ? Number(initialWalk.arrivalMinutes)
      : null;
}
...
function buildBodsTimetableIsochronesFromSearch(searchResult, configuredBands) {
  ...
  configuredBands.forEach((band) => {
    ...
    const bandSegments = searchResult.reachableSegments
      ...
      .filter((segment) => isDrawableBodsReachableSegment(segment))
      .filter(Boolean);
    ...
    let rings = bandSegments.length > 0
      ? buildBodsReachableLinkCorridorRings(routeMetrics, bandSegments, bandTime)
      : [];
    let geometryMode = rings.length > 0 ? "route_link_corridors" : "none";
    let usedSparseGeometryFallback = false;
    const scheduledStopMetrics = buildBodsReachableStopArrivalMetrics(
      searchResult,
      bandTime,
      bandSegments
    );
    const scheduledStopRings = buildBodsScheduledStopArrivalRings(
      scheduledStopMetrics,
      searchResult.originCoordinates,
      BODS_SCHEDULED_STOP_FALLBACK_MAX_RINGS_PER_BAND
    );
    if (scheduledStopRings.length > 0) {
      rings = [...rings, ...scheduledStopRings];
      geometryMode = geometryMode === "route_link_corridors"
        ? "route_link_corridors_plus_scheduled_stop_arrivals"
        : "scheduled_stop_arrival_buffers";
      usedSparseGeometryFallback = true;
    }
    routeMetrics.scheduledStopArrivalMetricCount = scheduledStopMetrics.length;
    ...
    if (rings.length > 0) {
      outputs.push({
        ...
        properties: {
          ...
          scheduledStopArrivalMetricCount: routeMetrics.scheduledStopArrivalMetricCount || 0,
          scheduledStopArrivalRingCount: scheduledStopRings.length,
          scheduledStopArrivalMaxRingsPerBand: BODS_SCHEDULED_STOP_FALLBACK_MAX_RINGS_PER_BAND,
          ...
          bodsWholeZoneOriginConnectedOnly: false,
...
function buildBodsScheduledStopArrivalMetrics(reachableSegments = [], bandTime, drawableSegments = []) {
  const seen = new Set();
  const metrics = [];
  const drawableStopIds = new Set();
  (drawableSegments || []).forEach((segment) => {
    if (segment?.fromStopId) {
      drawableStopIds.add(normaliseBodsStopIdentifier(segment.fromStopId));
    }
    if (segment?.toStopId) {
      drawableStopIds.add(normaliseBodsStopIdentifier(segment.toStopId));
    }
  });
  (reachableSegments || [])
    .filter((segment) => isBodsActualScheduledMovementSegment(segment))
    .filter((segment) => Number(segment.elapsedTimeAtEnd) <= Number(bandTime))
    .forEach((segment) => {
      [
        { coordinate: segment.startCoordinate, minutes: segment.elapsedTimeAtStart, stopId: segment.fromStopId, role: "from" },
        { coordinate: segment.endCoordinate || segment.coordinate, minutes: segment.elapsedTimeAtEnd, stopId: segment.toStopId, role: "to" },
      ].forEach((entry) => {
        if (!isValidCoordinateObject(entry.coordinate)) {
          return;
        }
        if (entry.stopId && drawableStopIds.has(normaliseBodsStopIdentifier(entry.stopId))) {
          return;
        }
        const key = `${entry.stopId || ""}|${entry.coordinate.latitude.toFixed(5)}|${entry.coordinate.longitude.toFixed(5)}|${entry.role}`;
        if (seen.has(key)) {
          return;
        }
        seen.add(key);
        metrics.push({
          coordinate: { latitude: entry.coordinate.latitude, longitude: entry.coordinate.longitude },
          minimumTotalBusAccessTimeMinutes: Math.min(Number(entry.minutes) || Number(bandTime), Number(bandTime)),
          source: "scheduled_bods_stop_arrival",
          stopId: entry.stopId || "",
          routeRef: segment.routeRef || "",
          routeName: segment.routeName || "",
...
function buildBodsReachableStopArrivalMetrics(searchResult, bandTime, drawableSegments = []) {
  const seen = new Set();
  const metrics = [];
  const stopIndexById = new Map((searchResult?.stops || []).map((stop, index) => [stop.id, index]));
  const drawableStopIds = new Set();
  (drawableSegments || []).forEach((segment) => {
    if (segment?.fromStopId) {
      drawableStopIds.add(normaliseBodsStopIdentifier(segment.fromStopId));
  ...
  (searchResult?.earliestByStop || [])
    .filter((entry) => Number(entry.elapsedCostMinutes) <= Number(bandTime))
    .forEach((entry) => {
      const stop = entry.stop;
      if (!stop?.id || !isValidCoordinateObject(stop)) {
        return;
      }
      const normalisedStopId = normaliseBodsStopIdentifier(stop.id);
      if (drawableStopIds.has(normalisedStopId)) {
        return;
      }
      const stopIndex = stopIndexById.get(stop.id);
      if (stopIndex === undefined || !doesBodsReachableStopRequireBusMovement(searchResult.previous, searchResult.stops, stopIndex, entry.transfersUsed)) {
        return;
      }
      const key = `${normalisedStopId}|${Number(stop.latitude).toFixed(5)}|${Number(stop.longitude).toFixed(5)}`;
      if (seen.has(key)) {
        return;
      }
      seen.add(key);
      metrics.push({
        ...
        minimumTotalBusAccessTimeMinutes: Math.min(Number(entry.elapsedCostMinutes) || Number(bandTime), Number(bandTime)),
        source: "reachable_bods_stop_arrival",
        stopId: stop.id,
        routeRef: "",
        routeName: "",
        tripId: "",
...
function doesBodsReachableStopRequireBusMovement(previous, stops, stopIndex, transfersUsed) {
  let index = stopIndex;
  let transferState = Number.isFinite(Number(transfersUsed)) ? Number(transfersUsed) : 0;
  const visited = new Set();
...
function buildBodsScheduledStopArrivalRings(metrics, originCoordinates = null, maxRings = BUS_ROUTE_MAX_BUFFERS_PER_BAND) {
  const cleanMetrics = (metrics || []).filter((metric) => isValidCoordinateObject(metric.coordinate));
  if (cleanMetrics.length === 0 || maxRings <= 0) {
    return [];
  }
  const coordinates = dedupeCoordinates(cleanMetrics.map((metric) => metric.coordinate), 5);
  if (coordinates.length === 0) {
    return [];
  }
  const rings = [];
  coordinates.forEach((coordinate) => {
    const originDistance = isValidCoordinateObject(originCoordinates)
      ? getDistanceMetres(originCoordinates.latitude, originCoordinates.longitude, coordinate.latitude, coordinate.longitude)
      : Infinity;
    if (Number.isFinite(originDistance) && originDistance <= 120) {
      return;
    }
    const ring = buildCoordinateBufferRing(coordinate, BODS_SCHEDULED_STOP_FALLBACK_BUFFER_METRES, 20);
    if (ring && ring.length >= 4 && rings.length < maxRings) {
      rings.push(ring);
    }
  });
  return capPolygonRings(rings, maxRings);
}

function isBodsActualScheduledMovementSegment(segment) {
  return Boolean(segment)
    && !segment.syntheticTimingOnly
    && segment.reachedVia === "scheduled_bus"
    && isBodsJourneyChainOriginAnchored(segment)
    && isValidCoordinateObject(segment.startCoordinate)
    && isValidCoordinateObject(segment.endCoordinate)
    && Number.isFinite(Number(segment.elapsedTimeAtStart))
    && Number.isFinite(Number(segment.elapsedTimeAtEnd))
    && Number(segment.elapsedTimeAtEnd) > Number(segment.elapsedTimeAtStart);
}
...
function buildBodsReachableRouteMetrics(stopMetrics, reachableSegments, bandTime) {
  ...
  reachableSegments
    ...
    const coordinates = routeCoordinates.length >= 2 ? routeCoordinates : [];
    if (coordinates.length < 2) {
      return;
    }
    const sampleSpacing = 260;
...
function isDrawableBodsReachableSegment(segment) {
  if (!segment || segment.syntheticTimingOnly || !isBodsJourneyChainOriginAnchored(segment) || !isBodsSegmentRouteContinuousFromWalkAccess(segment)) {
    return false;
  }
  return getBodsReachableSegmentGeometryCoordinates(segment).length >= 2;
}

function isBodsSegmentRouteContinuousFromWalkAccess(segment) {
  const chain = Array.isArray(segment?.journeyChain) ? segment.journeyChain : [];
  if (chain.length === 0) {
    return false;
  }
  const firstBusIndex = chain.findIndex((step) => step.type === "bus");
  if (firstBusIndex < 0) {
    return false;
  }
  const accessSteps = chain.slice(0, firstBusIndex);
  const hasInitialWalk = accessSteps.some((step) => step.type === "initial_walk");
  if (!hasInitialWalk) {
    return false;
  }
  const hasInternalConnectorBeforeFirstBus = accessSteps.some((step) => step.type === "initial_interchange_walk" || step.type === "transfer_walk");
  if (hasInternalConnectorBeforeFirstBus) {
    return false;
  }
  const firstBus = chain[firstBusIndex];
  const initialWalk = accessSteps.find((step) => step.type === "initial_walk");
  return Boolean(firstBus?.fromStopId && initialWalk?.toStopId && normaliseBodsStopIdentifier(firstBus.fromStopId) === normaliseBodsStopIdentifier(initialWalk.toStopId));
}
...
function getBodsReachableSegmentGeometryCoordinates(segment) {
  if (!segment || segment.syntheticTimingOnly || segment.drawableGeometryType === "timing_only_synthetic_connector") {
    return [];
  }
...
function summariseBodsOutputGeometryModes(isochrones = []) {
  ...
  const scheduledStopArrivalRingsByBand = {};
  const scheduledStopArrivalMetricsByBand = {};
  isochrones.forEach((isochrone) => {
    const properties = isochrone?.properties || {};
    const contour = Number(isochrone?.contour ?? properties.contour);
    ...
    if (Number.isFinite(contour)) {
      scheduledStopArrivalRingsByBand[contour] = Number(properties.scheduledStopArrivalRingCount || 0);
      scheduledStopArrivalMetricsByBand[contour] = Number(properties.scheduledStopArrivalMetricCount || 0);
    }
  });
  return {
    ...
    scheduledStopArrivalRingsByBand,
    scheduledStopArrivalMetricsByBand,
  };
}
...
function buildBodsTimetableSourceNote(metadata = {}) {
  ...
  const routeAuditText = metadata.bodsReachableRouteSummary?.top_routes?.length
    ...
    : "";
  const precisionWarningText = geometryModes.includes("scheduled_stop_arrival_buffers") || geometryModes.includes("route_link_corridors_plus_scheduled_stop_arrivals")
    ? " Warning: some BODS timetable links do not include reliable drawable route geometry, so actual scheduled stop arrivals are shown as small stop buffers rather than inferred route corridors."
    : geometrySummary.straightLineFallbackUsed || geometrySummary.estimatedRuntimeUsed || Number(metadata.bodsEstimatedRuntimeConnectionCount || 0) > 0
