...
const BODS_CACHE_SCHEMA_VERSION = "v78-bods-hybrid-naptan-route-graph-cache";
...
async function buildBodsHybridNaptanOsmRouteAccessLinks(timetable, eligiblePhysicalStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes, options = {}) {
  ...
  if (routes.length === 0) {
    result.warnings.push("No mapped OSM bus route relation could be linked to the walk-accessible physical stops.");
    return result;
  }
  ...
  if (result.syntheticConnectionCount === 0) {
    result.warnings.push("Mapped OSM routes were found from the access stops, but no catchable BODS timed anchor could be created after the selected time.");
  }
  return result;
}
  ...
  if (searchResult.initialStopsWithinMaxWalkCount === 0) {
    ...
    if (Number(searchResult.matchedBoardingStopCount || 0) > 0 && Number(searchResult.boardingStopsWithDeparturesAfterSelectedTime || 0) === 0) {
      return {
        ...
        message: `Nearby physical NaPTAN/OSM stops were found within the selected walk distance, and the app searched mapped OSM bus routes outward from those stops for BODS timed anchor stops; however no catchable scheduled BODS graph boarding node could be seeded after the selected time.${nearestText} Try a different departure time/date or check OSM route membership and BODS operator coverage.`,
        details,
      };
    }
    return {
