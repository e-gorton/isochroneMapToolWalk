...
const BODS_CACHE_SCHEMA_VERSION = "v75-bods-progress-cache";
...
function buildBodsBoardingStopContext(timetable, nearbyOriginStops, originCoordinates, maximumWalkToBusStopMetres, departureMinutes, maximumBandMinutes) {
  ...
  const directMatchedBoardingStopCount = boardingStops.filter((stop) => stop.boardingMatchType !== "access_linked_nearby_timetable_stop").length;
  const accessLinkedBoardingStopCount = boardingStops.filter((stop) => stop.boardingMatchType === "access_linked_nearby_timetable_stop").length;
...
function diagnoseBodsTimetableSearchFailure(searchResult) {
  ...
  if (searchResult.initialStopsWithinMaxWalkCount === 0) {
    ...
    if (Number(searchResult.matchedBoardingStopCount || 0) > 0 && Number(searchResult.boardingStopsWithDeparturesAfterSelectedTime || 0) === 0) {
      return {
        ...
        message: `Nearby physical stops were linked to local BODS timetable stops, but none of those linked boarding stops had scheduled departures after the selected time within the parsed timetable graph.${nearestText} Try a different departure time/date or check operator timetable coverage.`,
        details,
      };
    }
    return {
  ...
  if (searchResult.consideredConnectionCount === 0) {
    return {
      ...
      message: "Walk-accessible physical stops and local BODS timetable stops were linked, but no valid scheduled departures were found after the selected time within the configured look-ahead window. Try a different time or date.",
      details,
    };
  }
